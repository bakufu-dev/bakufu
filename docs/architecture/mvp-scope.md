...
| **Admin CLI**（最低限） | `bakufu admin retry-task <task_id>` / `cancel-task <task_id>` / `retry-event <event_id>` の 3 コマンド | LLM Adapter の `BLOCKED` Task と Outbox の dead-letter event を人手で救済する最終経路。UI は Phase 2 |
...
11. LLM Adapter が復旧不能エラー（`AuthExpired` 等）を返したとき Task が `BLOCKED` 状態に隔離され、`bakufu admin retry-task <task_id>` で再開できる
12. Notifier 失敗（Discord 障害等）が発生しても Outbox が retry し、最大 5 回失敗で dead-letter 化、`bakufu admin retry-event <event_id>` で再投入できる
