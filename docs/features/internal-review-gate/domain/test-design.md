...
| REQ-IRG-004（invalid_role）| required_gate_roles 非含有 role で提出 → raise | TC-UT-IRG-009 | ユニット | 異常系 | 2, 3 |
...
| MSG-IRG-004（invalid_role）| 無効 role → `[FAIL]...` + `Next:` | TC-UT-IRG-009 | ユニット | 異常系 | Q-1 |
| §確定 C（GateDecision 遷移 静的決定）| compute_decision 全条件 | TC-UT-IRG-004, TC-UT-IRG-005 | ユニット | 正常系 | 4, 5 |
| §確定 F（VerdictDecision 2 値のみ）| ambiguous 文字列は型エラー | TC-UT-IRG-011 | ユニット | 異常系 | Q-1 |
| §確定 G（comment NFC + strip しない）| 前後改行保持、NFC 合成形 / 分解形同一視 | TC-UT-IRG-012 | ユニット | 正常系 | Q-1 |
| §確定 H（2 行エラー構造）| 全 MSG-IRG で `Next:` hint 物理保証 | TC-UT-IRG-013〜016 | ユニット | 異常系 | Q-1 |
| §確定 I（責務分離）| 参照整合性は Aggregate 内で強制しない | TC-UT-IRG-017 | ユニット | 正常系 | Q-1 |
| frozen 不変性 | `gate.gate_decision = X` 直接代入拒否 | TC-UT-IRG-018 | ユニット | 異常系 | Q-1 |
| frozen 構造的等価 | 同一属性 Gate 2 インスタンスが `==` True | TC-UT-IRG-019 | ユニット | 正常系 | Q-1 |
| `extra='forbid'` | 未知フィールド拒否 | TC-UT-IRG-020 | ユニット | 異常系 | Q-1 |
...
### MSG 2 行構造 + Next: hint（全 4 MSG-IRG、Q-1 / §確定 H）
