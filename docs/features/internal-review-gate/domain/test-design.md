...
| REQ-IRG-004（invalid_role）| required_gate_roles 非含有 role で提出 → raise | TC-UT-IRG-009 | ユニット | 異常系 | — |
| REQ-IRG-004（不変条件 4 種）| gate_decision / verdicts 整合性違反（不正状態直接構築）| TC-UT-IRG-010 | ユニット | 異常系 | 2〜7 |
| MSG-IRG-001（role_already_submitted）| 重複提出 → `[FAIL]...` + `Next:` | TC-UT-IRG-006 | ユニット | 異常系 | — |
| MSG-IRG-002（gate_already_decided）| 確定後提出 → `[FAIL]...` + `Next:` | TC-UT-IRG-007 | ユニット | 異常系 | — |
| MSG-IRG-003（comment_too_long）| 5001文字 comment → `[FAIL]...` + `Next:` | TC-UT-IRG-008 | ユニット | 異常系 | 11 |
| MSG-IRG-004（invalid_role）| 無効 role → `[FAIL]...` + `Next:` | TC-UT-IRG-009 | ユニット | 異常系 | — |
| §確定 C（GateDecision 遷移 静的決定）| compute_decision 全条件 | TC-UT-IRG-004, TC-UT-IRG-005 | ユニット | 正常系 | 4, 5 |
| §確定 F（VerdictDecision 2 値のみ）| ambiguous 文字列は型エラー | TC-UT-IRG-011 | ユニット | 異常系 | — |
| §確定 G（comment NFC + strip しない）| 前後改行保持、NFC 合成形 / 分解形同一視 | TC-UT-IRG-012 | ユニット | 正常系 | — |
| §確定 H（2 行エラー構造）| 全 MSG-IRG で `Next:` hint 物理保証 | TC-UT-IRG-013〜016 | ユニット | 異常系 | — |
| §確定 I（責務分離）| 参照整合性は Aggregate 内で強制しない | TC-UT-IRG-017 | ユニット | 正常系 | — |
| frozen 不変性 | `gate.gate_decision = X` 直接代入拒否 | TC-UT-IRG-018 | ユニット | 異常系 | 内部品質基準 |
| frozen 構造的等価 | 同一属性 Gate 2 インスタンスが `==` True | TC-UT-IRG-019 | ユニット | 正常系 | 内部品質基準 |
| `extra='forbid'` | 未知フィールド拒否 | TC-UT-IRG-020 | ユニット | 異常系 | 内部品質基準 |
...
### MSG 2 行構造 + Next: hint（全 4 MSG-IRG、受入基準 — / §確定 H）
...
- C0 目標: `domain/internal_review_gate/` で **90% 以上**
    ...
    domain/
      internal_review_gate/
        ...
        test_internal_review_gate/             # 3 ファイル分割（最初から、500 行ルール）
          ...
          test_decision_logic.py               # compute_decision 全条件 + TC-IT-IRG-001/002（往復シナリオ）
```

## 人間が動作確認できるタイミング

- CI 統合後: `gh pr checks` で全ジョブ緑
- ローカル: `cd backend && uv run pytest tests/domain/internal_review_gate/test_internal_review_gate/ -v` → 全テスト緑
