...
> sub-features: [`domain/`](domain/)（M1）| [`repository/`](repository/)（M5-B #164）| [`application/`](application/)（M5-B #164）| http-api（将来）| ui（将来）
> 関連 Issue: [#65 feat(internal-review-gate): InternalReviewGate Aggregate Root (M1)](https://github.com/bakufu-dev/bakufu/issues/65) / [#164 feat(M5-B): InternalReviewGate infrastructure実装](https://github.com/bakufu-dev/bakufu/issues/164)
...
| repository | [`repository/`](repository/)（M5-B #164）| Gate の状態を再起動跨ぎで保持（永続化）、Verdict comment の secret 保護を担保（MaskedText TypeDecorator） |
| application | [`application/`](application/)（M5-B #164）| InternalReviewGateExecutorPort 実装・並列 GateRole LLM 実行・ALL_APPROVED → ExternalReviewGate 生成 / REJECTED → Task 差し戻し連携 |
...
|------------|-----------|------|
| A: domain | #65（M1）| 完了（マージ済み）|
| B: repository | #164（M5-B）| 進行中 |
| C: application | #164（M5-B）| 進行中（repository と同一 PR）|
| D: http-api | 将来起票 | 未着手 |
| E: ui | 将来起票 | 未着手 |
