...
| # | 論点 | 起票先 |
|---|---|---|
| Q-OPEN-1 | `VITE_EMPIRE_ID` が未設定の場合の UX（設定画面 vs エラーページ vs 自動検出）| **解決済み（R1-7 / `detailed-design.md §確定E` で凍結）**: 設定エラーメッセージを表示して Directive 投入フォームを非表示にする。自動検出・設定画面は Phase 2 以降 |
