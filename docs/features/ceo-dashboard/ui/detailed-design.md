...
| リクエストヘッダ | `Content-Type: application/json` を全リクエストに付与。`Authorization: Bearer <VITE_REVIEWER_ID>` は **Gate action POST 3本（approve / reject / cancel）のみ**に個別付与する。`defaultHeaders` には含めない |
