...
| `aggregate_type` | invalidate する queryKey |
|---|---|
| `"Task"` | `["task", aggregate_id]` + `["tasks"]`（一覧も再取得）|
| `"ExternalReviewGate"` | `["gate", aggregate_id]` + `["task", payload.task_id]`（Task 詳細も再取得）|
| `"Agent"` | `["task"]` 全件（Agent ステータス変化は Task 詳細に影響する可能性）|
| `"Directive"` | `["tasks"]`（Directive 完了 → Task 状態変化）|
...
4. 成功（201）: `queryClient.invalidateQueries(["tasks"])` + `navigate("/")`（Task 一覧へ遷移）
