...
### 確定 R1-4: 状態変更 API（POST / PUT / PATCH / DELETE）は `Origin` ヘッダを検証する。ただし MVP（Cookie セッションなし）では `Origin` ヘッダが存在しない呼び出し（AI エージェント・SDK 等）は通過させる

**理由**: threat-model.md §A3 の CSRF 対策（T2 攻撃者）。CSRF 攻撃はブラウザが Cookie を自動送信する場合に成立する。MVP では Cookie セッションを採用しないため `Origin` なしリクエストの CSRF リスクは低い。AI エージェント / curl / SDK はブラウザではないため `Origin` ヘッダを送信しないが、これらのクライアントをブロックしてしまうと後続 Issue B〜G の全 API が使用不能になる。したがって MVP では「`Origin` ヘッダが存在する場合のみ許可一覧と照合し、不一致なら 403」という段階的配線とする。Phase 2 で Cookie セッションを追加した際に「`Origin` なしも 403」に変更する（`detailed-design.md §確定 D` で実装判定ロジックを凍結）。
...
| 8 | 許可 Origin 以外の `Origin` ヘッダ付き POST リクエストが HTTP 403 を返す（MVP では `Origin` なしの POST は通過する） | （セキュリティ） | TC-IT-HAF-008 |
