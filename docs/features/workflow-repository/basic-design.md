...
> 関連: [requirements.md](requirements.md) / [`docs/features/empire-repository/`](../empire-repository/) **テンプレート真実源** / [`docs/features/workflow/`](../workflow/) （domain 設計済み）
...
|--------|----------|------------|------|
| REQ-WFR-001 | `WorkflowRepository` Protocol | `backend/src/bakufu/application/ports/workflow_repository.py` | Repository ポート定義（empire-repo と同パターン、§確定 R1-A） |
...
└── docs/
    ...
    └── features/
        └── workflow-repository/                       # 本 feature 設計書 4 本
```
...
2. service が Workflow のドメイン操作（例: `workflow.add_stage(stage_data)`）で新 Workflow を構築（pre-validate 方式、workflow #16 で凍結）
...
- 既存 feature への波及:
  ...
  - `feature/workflow`（PR #16）の domain 層 Workflow を import するのみ、workflow 設計書は変更しない
