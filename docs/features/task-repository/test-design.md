...
<!-- 対象範囲: REQ-TR-001〜006 / 詳細設計 §確定 R1-A〜R1-K / 9段階 save() / BUG-DRR-001 closure / 3 masking カラム物理保証 -->
...
1. **save() 9 段階**（§確定 R1-B）— 3 DELETE（CASCADE 活用）→ tasks UPSERT → 5 INSERT の順序強制と子テーブル完全往復
2. **3 masking カラム**（§確定 R1-E）— `tasks.last_error` / `conversation_messages.body_markdown` / `deliverables.body_markdown` に raw secret が DB に残らないことを raw SQL で物理確認
...
| **REQ-TR-002（masking、§確定 R1-E）** | raw `last_error` / `body_markdown` → DB に `<REDACTED:*>` 永続化（**3 masking カラム物理保証**）| TC-IT-TR-020-masking-* (9 経路) | 結合 | 正常系 | 5 |
...
| **§確定 R1-B（9 段階順序）** | deliverables UNIQUE(task_id, stage_id) 上書き / conversations CASCADE 再 INSERT | TC-UT-TR-005 / TC-UT-TR-005b | 結合 | 正常系 | 4 |
| **§確定 R1-J（_from_rows 子構造再組み立て）** | assigned_agent_ids 順序 / deliverables dict / conversations list 完全往復 | TC-UT-TR-003 | 結合 | 正常系 | — |
...
### save() 9 段階 DELETE+UPSERT+INSERT（§確定 R1-B）
...
| TC-UT-TR-005b | UNIQUE(task_id, stage_id) 制約: deliverables を更新しても重複行が発生しない（§確定 R1-B 段階 1 + 8）| 正常系 | `make_task_with_deliverables` を保存済み | 同 stage_id で内容を変更した deliverable で re-save → raw SQL で deliverables 行数確認 | `SELECT COUNT(*) FROM deliverables WHERE task_id = :id` = 1。古い行が段階 1 の DELETE で先行消去済みのため UNIQUE 違反なし。body_markdown が更新済み |
...
### 3 masking カラム物理保証（§確定 R1-E / §確定 R1-A、本 PR の核心テストファイル）
...
- [ ] **save() 9 段階**（§確定 R1-B）が TC-UT-TR-005 / 005b / 005c で DELETE 先行 + UPSERT（ON CONFLICT DO UPDATE）+ INSERT の 3 経路を物理確認
...
- [ ] **6 テーブル DDL + 2 INDEX**（REQ-TR-003）が TC-IT-TR-001 / 002 / 003 で物理確認
- [ ] **Alembic chain 一直線**: TC-IT-TR-006 で `0007.down_revision == "0006_directive_aggregate"` を物理確認
- [ ] **upgrade/downgrade idempotent**: TC-IT-TR-005 で双方向 migration を物理確認
...
- [ ] **_from_rows 全子構造**（§確定 R1-J）: TC-UT-TR-003 で assigned_agent_ids（order_index順）/ deliverables（dict）/ conversations（created_at 順）/ messages（timestamp 順）/ attachments（sha256 順）の復元順序が §確定 R1-H と一致
