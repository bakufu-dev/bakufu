...
<!-- 対象範囲: REQ-TR-001〜006 / 詳細設計 §確定 A〜F / 9段階 save() / BUG-DRR-001 closure / 3 masking カラム物理保証 -->

本 feature は M2 Repository **7 番目の Aggregate Repository PR**（empire / workflow / agent / room / directive 後）。Task Aggregate（M1、PR #42 マージ済み）に対する Repository 層を新規追加する。テンプレートは directive-repository (PR #50) を 100% 継承しつつ、**6-method Protocol**（`find_by_id` / `count` / `save(task)` / `count_by_status` / `count_by_room` / `find_blocked`）と **Task 固有の多段階 9-step save()** および **3 masking カラム**（`tasks.last_error` / `conversation_messages.body_markdown` / `deliverables.body_markdown`）の構造を確立する。

task-repository 固有の論点 5 件を**専用テストファイルで物理保証**する:

1. **save() 9 段階**（§確定 R1-B / §確定 B）— 3 DELETE（CASCADE 活用）→ tasks UPSERT → 5 INSERT の順序強制と子テーブル完全往復
2. **3 masking カラム**（§確定 R1-E / §確定 E）— `tasks.last_error` / `conversation_messages.body_markdown` / `deliverables.body_markdown` に raw secret が DB に残らないことを raw SQL で物理確認
...
4. **BUG-DRR-001 closure 物理確認**（§確定 R1-C / §確定 C）— 0007 適用後に `directives.task_id → tasks.id` FK が **存在すること**を `PRAGMA foreign_key_list('directives')` で確認（TC-IT-DRR-006 の反転）
5. **§BUG-TR-001 FK 未追加の申し送り確認**（§Known Issues §BUG-TR-001）— 0007 時点で `task_assigned_agents.agent_id` FK が **存在しないこと**を物理確認

**最初から 5 ファイル分割**（directive-repo PR #50 正規構成の継承。`test_masking_fields.py` が 3 masking カラム物理保証の核心、`test_find_blocked.py` が find_blocked + count_by_* の 3 新 method 専用ファイル）。
...
| REQ-TR-002（save round-trip） | `save(task)` 9 段階 → `find_by_id` round-trip | TC-UT-TR-003 | 結合 | 正常系 | 4 |
| REQ-TR-002（save 9段階 DELETE+UPSERT+INSERT） | 9 段階順序の物理確認（child table 完全往復） | TC-UT-TR-005 / TC-UT-TR-005b / TC-UT-TR-005c | 結合 | 正常系 | 4 |
...
| **REQ-TR-002（masking、§確定 R1-E / §確定 G）** | raw `last_error` / `body_markdown` → DB に `<REDACTED:*>` 永続化（**3 masking カラム物理保証**）| TC-IT-TR-020-masking-* (9 経路) | 結合 | 正常系 | 5 |
| REQ-TR-003（Alembic 0007 DDL）| 6 テーブル + INDEX + FK 群作成 | TC-IT-TR-001 / TC-IT-TR-002 / TC-IT-TR-003 | 結合 | 正常系 | 6 |
...
| REQ-TR-003（§BUG-TR-001 申し送り）| 0007 で `task_assigned_agents.agent_id` FK が存在しない | TC-IT-TR-009 | 結合 | 正常系 | 9 |
| REQ-TR-004（CI Layer 2）| arch test parametrize（3 カラム追加）| TC-UT-TR-arch | 結合 | 正常系 | 10 |
| REQ-TR-004（CI Layer 1）| grep guard で 3 カラムの `MaskedText` 必須 | （CI ジョブ） | — | — | 10 |
...
| **§確定 R1-B（save 9 段階）** | DELETE 逆順 → tasks UPSERT → 5 テーブル INSERT 順序の物理確認 | TC-UT-TR-005 | 結合 | 正常系 | 4 |
...
| **§確定 R1-E（CI 三層防衛 3 カラム）** | 正のチェック + 負のチェック 3 カラム分 | TC-UT-TR-arch + TC-DOC-TR-001 | 結合 / doc | 正常系 | 10 |
| **§確定 R1-H（ORDER BY 決定論性）** | 全子テーブルの ORDER BY + find_blocked の tiebreaker | TC-UT-TR-003 + TC-UT-TR-008e | 結合 | 正常系 | — |
| **§確定 A（TypeDecorator 信頼）** | UUIDStr / MaskedText 二重変換なし | TC-UT-TR-003 + TC-IT-TR-020-masking-* | 結合 | 正常系 | — |
| **§確定 B（9 段階順序）** | deliverables UNIQUE(task_id, stage_id) 上書き / conversations CASCADE 再 INSERT | TC-UT-TR-005 / TC-UT-TR-005b | 結合 | 正常系 | 4 |
| **§確定 D（_from_rows 子構造再組み立て）** | assigned_agent_ids 順序 / deliverables dict / conversations list 完全往復 | TC-UT-TR-003 | 結合 | 正常系 | — |
| **§BUG-TR-001（agent FK 申し送り）** | 0007 で task_assigned_agents.agent_id FK なし | TC-IT-TR-009 | 結合 | 正常系 | 9 |
...
- **save() 9 段階の順序確認**: TC-UT-TR-005 で child table DELETE → tasks UPSERT → child INSERT の順序違反が `IntegrityError` になることを物理確認（段階 4 より前に tasks UPSERT をしないことの確認）
- **3 masking カラム全経路**: TC-IT-TR-020-masking-* で `tasks.last_error` / `conversation_messages.body_markdown` / `deliverables.body_markdown` の各カラムに masked + passthrough + roundtrip を確認
...
- **§BUG-TR-001 申し送り**: TC-IT-TR-009 で `task_assigned_agents.agent_id` FK が 0007 時点で存在しないことを確認
...
|--------|-----|------------|---------|---------------------|
| **SQLite (sqlite+aiosqlite)** | engine / session / 6 テーブル / Alembic 0007 migration | 不要（実 DB を `tmp_path` 配下の bakufu.db で起動、テストごとに使い捨て）| 不要 | **済（M2 永続化基盤 conftest の `app_engine` / `session_factory` fixture を再利用）** |
...
| **MaskingGateway (`mask`)** | `MaskedText.process_bind_param` 経由で 3 カラムをマスキング | 不要（実 init を `_initialize_masking` autouse fixture で実施）| 不要 | **済（persistence-foundation #23 で characterization 完了、本 PR で配線実適用）** |

**factory（合成データ）の扱い**:

| factory | 出力 | `_meta.synthetic` 付与 | 備考 |
|--------|-----|------------------|------|
| `make_task`（**本 PR で追加**） | `Task`（valid デフォルト: `status=PENDING`, `last_error=None`, `assigned_agent_ids=[]`, `deliverables={}`, `conversations=[]`）| `True` | task PR #42 の Task Aggregate factory |
| `make_task_with_agents`（**本 PR で追加**） | `Task`（`assigned_agent_ids` に 2 Agent 付き）| `True` | save() 段階 5 の INSERT テスト用 |
| `make_task_with_conversations`（**本 PR で追加**） | `Task`（`conversations` に 1 Conversation + 複数 Message 付き）| `True` | save() 段階 6/7 の INSERT テスト用 |
| `make_task_with_deliverables`（**本 PR で追加**） | `Task`（`deliverables` に 1 Deliverable + 1 Attachment 付き）| `True` | save() 段階 8/9 の INSERT テスト用 |
| `make_task_full`（**本 PR で追加**） | `Task`（全子構造を持つ full Task。assigned_agents / conversations / deliverables / attachments すべて付き）| `True` | round-trip + masking テスト用 |
...
### 基本 CRUD — save round-trip / count（§確定 R1-A / §確定 B / §確定 D）
...
| TC-UT-TR-003 | `save(task)` → `find_by_id` round-trip 全属性（§確定 D）| `session_factory` + `make_task_full` + `seeded_task_context` | seeded room + seeded directive | `save(full_task)` → `find_by_id(task.id)` | 復元 Task が以下全属性と等価: `id` / `room_id` / `directive_id` / `current_stage_id` / `status` / `last_error` / `created_at`（UTC tz-aware）/ `updated_at`（UTC tz-aware）/ `assigned_agent_ids`（order_index 順）/ `deliverables`（dict[StageId, Deliverable]）/ `conversations`（created_at ASC 順）/ 各 Conversation の messages（timestamp ASC 順）/ 各 Deliverable の attachments（sha256 ASC 順）|
...
### save() 9 段階 DELETE+UPSERT+INSERT（§確定 R1-B / §確定 B）

**`test_protocol_crud.py` 内 `TestSave9StageSemantics`** — 9 段階順序の物理確認。

| テストID | 対象 | 種別 | 前提条件 | 操作 | 期待結果 |
|---------|-----|------|---------|------|---------|
| TC-UT-TR-005 | save(task) → re-save で child table が DELETE+再INSERT される（§確定 R1-B）| 正常系 | `make_task_with_conversations` を保存済み | conversations を変更した task を re-save → `find_by_id` | 再 save 後の conversations が更新済み。古い conversation_messages 行が残らない（DELETE+再 INSERT の物理確認）|
| TC-UT-TR-005b | UNIQUE(task_id, stage_id) 制約: deliverables を更新しても重複行が発生しない（§確定 B 段階 1 + 8）| 正常系 | `make_task_with_deliverables` を保存済み | 同 stage_id で内容を変更した deliverable で re-save → raw SQL で deliverables 行数確認 | `SELECT COUNT(*) FROM deliverables WHERE task_id = :id` = 1。古い行が段階 1 の DELETE で先行消去済みのため UNIQUE 違反なし。body_markdown が更新済み |
| TC-UT-TR-005c | 全子テーブル空 Task → 全子有り Task への更新（段階 4〜9 全実行）| 正常系 | empty assigned_agents / conversations / deliverables の Task を保存済み | `make_task_full` に相当する Task を同 id で re-save → `find_by_id` | 全子構造が正しく存在。`task_assigned_agents` / `conversations` / `conversation_messages` / `deliverables` / `deliverable_attachments` の行数が factory と一致 |
...
| TC-UT-TR-008d | `find_blocked()` 戻り値 Task が全属性 _from_rows で完全復元される（§確定 D）| 正常系 | BLOCKED Task（last_error 付き）を保存済み | `find_blocked()` | 戻り値 Task の全属性（id / status / last_error / room_id / directive_id 等）が保存値と等価 |
...
### 3 masking カラム物理保証（§確定 R1-E / §確定 A、本 PR の核心テストファイル）

**`test_masking_fields.py`** — `tasks.last_error` / `conversation_messages.body_markdown` / `deliverables.body_markdown` の 3 カラムに raw secret が DB に残らないことを raw SQL SELECT で byte-level 証明する。directive-repo `test_masking_text.py` のテンプレート継承。
...
| TC-IT-TR-020-masking-last_error-roundtrip | `tasks.last_error` — 不可逆性（§確定 A §不可逆性）| 正常系 | Discord Bot Token を含む `last_error` で save → `find_by_id` | 復元 Task の `last_error` が `<REDACTED:DISCORD_TOKEN>` を含む。raw token が `find_by_id` 経由で復元不能 |
| TC-IT-TR-020-masking-message-masked | `conversation_messages.body_markdown` — Anthropic API key マスキング | 正常系 | `body_markdown` に `sk-ant-api03-XXX...` を含む Message を持つ Task を save | raw SQL `SELECT body_markdown FROM conversation_messages WHERE id = :id` で `<REDACTED:ANTHROPIC_KEY>` を含む。raw key が残らない |
| TC-IT-TR-020-masking-message-plain | `conversation_messages.body_markdown` — secret なし passthrough | 正常系 | `body_markdown` に plain text（"作業を開始します。"）を持つ Message を save | raw SQL SELECT で文字列が改変されない |
...
| TC-IT-TR-020-masking-multiple-columns | 3 カラム同時 masking — 1 Task に 3 種 secret（§確定 R1-E 全層確認）| 正常系 | `last_error` に Discord token、`body_markdown`（message）に Anthropic key、`body_markdown`（deliverable）に GitHub PAT を含む full Task を save | 各カラムの raw SQL SELECT で各 `<REDACTED:*>` センチネルが存在。3 つすべての raw secret が残らない |

### Alembic 0007 + FK CASCADE + BUG-DRR-001 closure + §BUG-TR-001（受入基準 6〜9）

**`test_alembic_task.py`** — directive-repo `test_alembic_directive.py` のテンプレート継承。

| テストID | 対象 | 使用 fixture | 前提条件 | 操作 | 期待結果 |
|---------|-----|--------------|---------|------|---------|
| TC-IT-TR-001 | 0007 が 6 テーブルを作成（受入基準 6）| `empty_engine`（clean DB） | — | `alembic upgrade head` → `SELECT name FROM sqlite_master WHERE type='table'` | `tasks` / `task_assigned_agents` / `conversations` / `conversation_messages` / `deliverables` / `deliverable_attachments` の全 6 テーブルが存在 |
| TC-IT-TR-002 | 0007 が INDEX 3 件を作成（§確定 F）| `empty_engine` | — | `upgrade head` → `SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='tasks'` | `ix_tasks_status` / `ix_tasks_room_id` / `ix_tasks_updated_at_id` の全 3 INDEX が存在 |
...
| TC-IT-TR-005 | upgrade head → downgrade base → upgrade head が idempotent（受入基準 6）| `empty_engine` | — | 双方向サイクル実行 | 最終状態で 6 テーブルが存在。downgrade base 後は全テーブル消滅。再 upgrade 後に再出現 |
...
| TC-IT-TR-009 | §BUG-TR-001: `task_assigned_agents.agent_id` FK が 0007 時点で存在しない（受入基準 9）| `empty_engine` | — | `upgrade head` → `PRAGMA foreign_key_list('task_assigned_agents')` | FK 参照テーブル一覧に `agents` が**存在しない**（forward reference problem — agent-repository PR で closure 申し送り）|

### CI 三層防衛 Task 拡張（受入基準 10、§確定 R1-E / §確定 E）

| テストID | 対象 | 使用 fixture | 前提条件 | 操作 | 期待結果 |
|---------|-----|--------------|---------|------|---------|
| TC-UT-TR-arch | Layer 2: `tests/architecture/test_masking_columns.py` の Task parametrize 拡張（3 カラム）| `Base.metadata` | M2 永続化基盤の arch test に masking 検証構造あり | parametrize に `("tasks", "last_error", MaskedText)` / `("conversation_messages", "body_markdown", MaskedText)` / `("deliverables", "body_markdown", MaskedText)` を追加 | pass（3 カラムは MaskedText、その他カラムは masking なし）。後続 PR が誤ってカラム型を変更した瞬間に落下して PR ブロック |
| TC-DOC-TR-001 | storage.md §逆引き表 Task 行存在（受入基準 11）| repo root | `docs/architecture/domain-model/storage.md` 編集済み | `tests/docs/test_storage_md_back_index.py` で Task 行検証 | (a) `tasks.last_error: MaskedText` が §逆引き表に存在、(b) `conversation_messages.body_markdown: MaskedText` が存在、(c) `deliverables.body_markdown: MaskedText` が存在、(d) Task 残カラム（masking 対象なし）行が存在 |
...
- REQ-TR-001〜006 すべてに最低 1 件のテストケース
- **save() 9 段階**: TC-UT-TR-005 / 005b / 005c で DELETE 先行 + UPSERT + INSERT 順序・child table 完全往復・UNIQUE 制約 3 経路すべてに証拠
- **3 masking カラム（9 経路）**: TC-IT-TR-020-masking-* で `tasks.last_error` / `conversation_messages.body_markdown` / `deliverables.body_markdown` の各カラムに masked + passthrough + roundtrip（last_error のみ）+ NULL safe + 3 カラム同時を確認
...
- **6 テーブル DDL**: TC-IT-TR-001 で全 6 テーブルの存在を物理確認
- **INDEX**: TC-IT-TR-002 で 3 INDEX（ix_tasks_status / ix_tasks_room_id / ix_tasks_updated_at_id）の存在を物理確認
...
- ローカル: `bash scripts/setup.sh` → `cd backend && uv run pytest tests/infrastructure/persistence/sqlite/repositories/test_task_repository tests/infrastructure/persistence/sqlite/test_alembic_task.py tests/architecture/test_masking_columns.py tests/docs/test_storage_md_back_index.py -v` → 全テスト緑
- Backend 実起動: `cd backend && uv run python -m bakufu`（環境変数 `BAKUFU_DATA_DIR=/tmp/bakufu-test` を設定）
  ...
  - `sqlite3 <DATA_DIR>/bakufu.db ".tables"` で 6 テーブルが存在することを目視
  - `sqlite3 <DATA_DIR>/bakufu.db "PRAGMA foreign_key_list(directives)"` で `tasks.id` への FK が存在することを目視（BUG-DRR-001 closure 確認）
  - `sqlite3 <DATA_DIR>/bakufu.db "PRAGMA foreign_key_list(task_assigned_agents)"` で `agents` への FK が存在しないことを目視（§BUG-TR-001 申し送り確認）
- masking 物理確認: `uv run pytest tests/.../test_masking_fields.py -v` → 9 ケース緑、raw token が DB に残らないことを目視
  ...
  tests/
    factories/
      task.py                                         # 新規（make_task / make_task_with_agents /
                                                      #        make_task_with_conversations /
                                                      #        make_task_with_deliverables / make_task_full）
    architecture/
      test_masking_columns.py                         # 既存更新: Task 3 カラム parametrize 拡張
                                                      # TC-UT-TR-arch
    infrastructure/
      persistence/
        sqlite/
          repositories/
            test_task_repository/                     # 新規ディレクトリ（5 ファイル分割）
              ...
              test_protocol_crud.py                    # TC-UT-TR-001〜009 + TC-IT-TR-LIFECYCLE
              test_find_blocked.py                     # TC-UT-TR-006 / 007 / 008 / 008b / 008c / 008d / 008e
              test_masking_fields.py                   # TC-IT-TR-020-masking-* (9 ケース、3 masking カラム核心)
...
_read_persisted_message_body(session_factory, message_id) -> str
  raw SQL: SELECT body_markdown FROM conversation_messages WHERE id = :id
...
| # | タスク | 起票先 | 備考 |
|---|-------|--------|------|
| Task 後続申し送り #1 | §BUG-TR-001: `task_assigned_agents.agent_id → agents.id` / `deliverables.committed_by → agents.id` FK closure | `feature/task-agent-fk-closure`（未 Issue）| agent-repository PR マージ後に `batch_alter_table` で FK 追加。BUG-DRR-001 / BUG-EMR-001 と同パターン |
| Task 後続申し送り #2 | `tasks.current_stage_id → workflow_stages.id` FK closure（§確定 R1-G 循環参照問題）| `feature/task-workflow-binding`（未 Issue）| workflow §確定 J と同方針。task-workflow-binding PR で `batch_alter_table` で FK 追加 |
...
- [ ] **save() 9 段階**（§確定 R1-B）が TC-UT-TR-005 / 005b / 005c で DELETE 先行 + UPSERT + INSERT の 3 経路を物理確認
- [ ] **3 masking カラム（9 経路）**（§確定 R1-E）が TC-IT-TR-020-masking-* で `tasks.last_error` / `conversation_messages.body_markdown` / `deliverables.body_markdown` 各カラムに raw SQL SELECT での物理確認
...
- [ ] **§BUG-TR-001 申し送り**（§Known Issues）が TC-IT-TR-009 で `task_assigned_agents.agent_id` FK が 0007 時点で存在しないことを物理確認
- [ ] **6 テーブル DDL + 3 INDEX**（REQ-TR-003）が TC-IT-TR-001 / 002 / 003 で物理確認
- [ ] **Alembic chain 一直線**: TC-IT-TR-006 で `0007.down_revision == "0006_directive_aggregate"` を物理確認
- [ ] **upgrade/downgrade idempotent**: TC-IT-TR-005 で双方向 migration を物理確認
- [ ] **CI 三層防衛**（§確定 R1-E）: Layer 1 grep（CI）+ Layer 2 arch（TC-UT-TR-arch）+ Layer 3 storage.md（TC-DOC-TR-001）の 3 つすべてに証拠
- [ ] **TypeDecorator 信頼**（§確定 A）: TC-UT-TR-003 で UUIDStr 二重ラップなし / MaskedText 手動 mask なしの round-trip を確認
- [ ] **_from_rows 全子構造**（§確定 D）: TC-UT-TR-003 で assigned_agent_ids（order_index順）/ deliverables（dict）/ conversations（created_at 順）/ messages（timestamp 順）/ attachments（sha256 順）の復元順序が §確定 R1-H と一致
- [ ] **save(task) 1 引数 + UPSERT セマンティクス**（empire §確定 B 継承）: TC-UT-TR-005 で UPSERT の ON CONFLICT + child table DELETE+re-INSERT を確認
- [ ] **テストファイル分割（5 ファイル）が basic-design.md §モジュール構成と整合**
- [ ] §BUG-TR-001 の申し送り先（feature/task-agent-fk-closure）が detailed-design.md §Known Issues に明記されている
