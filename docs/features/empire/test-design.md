...
<!-- 対象範囲: REQ-EM-001〜005 / MSG-EM-001〜005 / 脅威 T1, T2 / 受入基準 1〜10 / 詳細設計 確定 A〜D -->
...
| REQ-EM-002（容量） | `Empire.hire_agent` + `model_validator` | TC-UT-EM-007 | ユニット | 境界値 | 確定 C |
...
| REQ-EM-003（容量） | `Empire.establish_room` + `model_validator` | TC-UT-EM-010 | ユニット | 境界値 | 確定 C |
...
| AC-9（lint/typecheck） | `pyright --strict` / `ruff check` | （CI ジョブ） | — | — | 9 |
| AC-10（カバレッジ） | `pytest --cov=bakufu.domain.empire` | （CI ジョブ） | — | — | 10 |
...
- 受入基準 1〜8 すべてにユニットケース（受入基準 9, 10 は CI ジョブで担保）
- T1 / T2 すべてに有効性確認ケース
...
- [`requirements-analysis.md`](requirements-analysis.md) §前提条件・制約 で「ネットワーク: 該当なし — Empire は domain 層のため外部通信なし」と凍結
...
| factory | 出力 | `_meta.synthetic` |
|--------|-----|------------------|
| `EmpireFactory` | `Empire`（valid デフォルト + override 可） | `True` |
| `RoomRefFactory` | `RoomRef`（valid デフォルト + override 可） | `True` |
| `AgentRefFactory` | `AgentRef`（valid デフォルト + role=DEVELOPER） | `True` |
| `RoleFactory` | `Role` enum 全列挙からランダム | `True` |

`_meta.synthetic = True` は factory が返すインスタンスにラップするモジュール属性として注入する（Pydantic frozen 内には埋め込まない、生成元を識別する目印）。
...
| TC-UT-EM-019 | MSG-EM-001 文言照合 | 異常系 | `name='a'*81` で構築失敗 | 例外 `message` が `[FAIL] Empire name must be 1-80 characters (got 81)` と完全一致 |
...
- 受入基準 1 〜 8 の各々が**最低 1 件のユニットテストケース**で検証されている（E2E 不在のため受入基準は unit/integration で代替、戦略ガイド準拠）
- 受入基準 9（pyright/ruff）/ 10（カバレッジ 80%）は CI ジョブ（lint / typecheck / test-backend）で担保
...
- [ ] factory に `_meta.synthetic = True` が付与される設計になっている
