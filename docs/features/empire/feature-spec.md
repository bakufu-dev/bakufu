...
> 関連 Issue: [#8 feat(empire): Empire Aggregate Root (M1)](https://github.com/bakufu-dev/bakufu/issues/8) / [#25 feat(empire-repository): Empire SQLite Repository (M2)](https://github.com/bakufu-dev/bakufu/issues/25)
...
| http-api | (将来) | UI / 外部クライアントから組織状態を操作・取得する経路 |
...
E2E（受入基準 10, 11）は [`system-test-design.md`](system-test-design.md) で詳細凍結。受入基準 1〜9 は domain sub-feature の IT / UT で検証（[`domain/test-design.md`](domain/test-design.md)）。
