...
| メソッド名 | 引数 | 戻り値 | raises |
|---|---|---|---|
| `create` | `empire_id: EmpireId, name: str, description: str, workflow_id: WorkflowId, prompt_kit_prefix_markdown: str` | `Room` | `EmpireNotFoundError` / `WorkflowNotFoundError` / `RoomNameAlreadyExistsError` / `RoomInvariantViolation` |
