...
> 関連: [basic-design.md](basic-design.md) / [`docs/features/empire-repository/detailed-design.md`](../../empire-repository/detailed-design.md) **テンプレート真実源** / [`docs/features/workflow-repository/detailed-design.md`](../../workflow-repository/detailed-design.md) **2 件目テンプレート** / [`docs/features/agent-repository/detailed-design.md`](../../agent-repository/detailed-design.md) **3 件目テンプレート** / [`domain/detailed-design.md`](../domain/detailed-design.md)
...
| `save(room: Room, empire_id: EmpireId)` | Room + EmpireId | None | 同一 Tx 内で rooms + room_members を delete-then-insert（empire-repo §確定 B 踏襲）。**`empire_id` は引数経由**（§確定 R1-H、Room Aggregate に empire_id 属性がないため呼び出し側 service が渡す） |
| `find_by_name(empire_id: EmpireId, name: str)` | EmpireId + name 文字列 | `Room \| None` | **第 4 method**、Empire スコープでの一意検査（agent §R1-C 継承、§確定 R1-F）。不在時 None。`INDEX(empire_id, name)` 経由 |
...
| `save(room, empire_id)` | Room + EmpireId | None | §確定 R1-B の delete-then-insert（3 段階手順、後述） |
| `find_by_name(empire_id, name)` | EmpireId + str | `Room \| None` | `SELECT id FROM rooms WHERE empire_id = :empire_id AND name = :name LIMIT 1` で RoomId 取得 → 存在すれば `find_by_id(found_id)` で子テーブル含めて Room 復元（§確定 R1-F）|
| `_to_row(room, empire_id)` | Room + EmpireId | `tuple[dict, list[dict]]` | (rooms_row, member_rows) に分離（empire-repo §確定 C、§確定 R1-H で `empire_id` 引数経由を凍結） |
| `_from_row(room_row, member_rows)` | dict, list[dict] | Room | VO 構造で復元（§確定 R1-J で `prompt_kit_prefix_markdown` の masked 復元を凍結） |

### Tables（既存 M2 永続化基盤の table モジュール群に追加）

| テーブル | モジュール | カラム |
|----|----|----|
| `rooms` | `infrastructure/persistence/sqlite/tables/rooms.py`（新規）| `id: UUIDStr PK` / `empire_id: UUIDStr FK CASCADE` / **`workflow_id: UUIDStr FK RESTRICT NULLABLE`**（業務ルール R1-12、§確定 R1-M）/ `name: String(80) NOT NULL`（DB UNIQUE なし）/ `description: String(500) NOT NULL DEFAULT ''` / **`prompt_kit_prefix_markdown: MaskedText NOT NULL DEFAULT ''`** / `archived: Boolean NOT NULL DEFAULT FALSE` / **INDEX(empire_id, name) 非 UNIQUE** |
...
### 確定 R1-A: empire-repo / workflow-repo / agent-repo テンプレート 100% 継承（再凍結）

empire-repository の §確定 A〜F + §Known Issues §BUG-EMR-001 規約 + workflow-repository の §確定 E（CI 三層防衛 正のチェック）+ agent-repository の §確定 R1-C（find_by_name 第 4 method）/ §確定 R1-D（DB レベル制約二重防衛）を**そのまま継承**。本 PR で再議論しない項目:
...
### 確定 R1-B: `save()` 3 段階手順（empire-repo §確定 B の Room 適用）
...
`Room` Aggregate は `empire_id` 属性を持たない（domain §確定で凍結、basic-design.md §クラス設計概要 §`Room` Aggregate に `empire_id` を含めない理由 参照）。Repository 永続化では `rooms.empire_id` カラムが必要なため、`save(room, empire_id)` で呼び出し側 service が渡す形（§確定 R1-H 詳述）。

### 確定 R1-C: domain ↔ row 変換契約（empire-repo §確定 C の Room 適用）
...
戻り値: `Room(id=..., workflow_id=..., name=..., description=..., prompt_kit=PromptKit(prefix_markdown=masked_str), members=[AgentMembership(...) for ...], archived=...)`。Aggregate Root の不変条件は構築時の `model_validator(mode='after')` で再走（domain PR で凍結済み）。
...
##### masked `prompt_kit_prefix_markdown` 復元の不可逆性凍結（§確定 R1-J）
...
### 確定 R1-D: `count()` SQL 契約（empire-repo §確定 D 踏襲）
...
### 確定 R1-E: CI 三層防衛 Room 拡張（**正/負のチェック併用**、workflow-repo §確定 E パターン）
...
`docs/design/domain-model/storage.md` §逆引き表に Room 関連 2 行追加（既存 `PromptKit.prefix_markdown` 行は本 PR で実適用済みに更新）。

### 確定 R1-F: `find_by_name(empire_id, name)` 第 4 method 契約（agent §R1-C 継承詳細）
...
### 確定 R1-H: `save(room, empire_id)` の `empire_id` 引数経路凍結

##### 背景: `Room` Aggregate に `empire_id` 属性がない

[`domain/detailed-design.md`](../domain/detailed-design.md) §クラス設計（詳細）で `Room` Aggregate Root の属性は `id` / `name` / `description` / `workflow_id` / `members` / `prompt_kit` / `archived` の 7 つ。**`empire_id` は含まれない**（Room の所属 Empire は Empire Aggregate の `rooms: list[RoomRef]` 経由で表現される）。

##### Repository 永続化での `empire_id` 必須性

`rooms.empire_id` カラムは:
- `find_by_name(empire_id, name)` の `WHERE` 句に必須（§確定 R1-F）
...
| (b) `Room` Aggregate に `empire_id: EmpireId` 属性を追加 | ✗ 不採用 | room domain 設計（マージ済み）の破壊的変更、room §確定の「Room の所属 Empire は Empire Aggregate の rooms 経由」凍結に違反 |
...
「Aggregate に外部 Aggregate ID 属性を持たない場合、Repository `save(aggregate, scope_id)` で引数経由」パターンを本 §確定 R1-H で凍結。後続 PR で同パターンが必要になった場合、本 §確定 R1-H を真実源として参照。

### 確定 R1-I: `workflow_id` FK ON DELETE RESTRICT（NULLABLE 対応）

##### Alembic での生成（0005 初期 migration — 修正前）

0005 では `nullable=False` で生成していたが、Issue #183（ブートストラップ循環バグ）の修正として §確定 R1-M の 0006 migration で `NULLABLE` へ変更する。

**確定後の正式定義**:

`sa.Column('workflow_id', sa.CHAR(32), sa.ForeignKey('workflows.id', ondelete='RESTRICT'), nullable=True)`

##### NULLABLE FK の挙動（SQLite FK 規約）

| `workflow_id` の値 | FK チェック | 挙動 |
|---|---|---|
| `NULL` | スキップ | Room 作成成功。Workflow は後付けで紐付け可能 |
| 非 `NULL` — 存在する Workflow ID | 通過 | Room 作成成功 |
| 非 `NULL` — 存在しない Workflow ID | 違反（RESTRICT）| `sqlalchemy.IntegrityError`（FOREIGN KEY constraint failed）|

application 層は `workflow_id` が非 `None` の場合のみ `WorkflowRepository.find_by_id` で存在検査を行い、不在時は `WorkflowNotFoundError` を raise する（§確定 G `create` raises 参照）。DB FK は application 層検査が壊れた場合の最終防衛線。

##### 違反時の挙動（参照整合性）

| 経路 | 違反検出層 | 例外 |
|---|---|---|
| application 層が Workflow を削除しようとする時に Room が残存（workflow_id 非 NULL）| DB FK RESTRICT | `sqlalchemy.IntegrityError`（FOREIGN KEY constraint failed）|
...
**追加しない**。本 PR の Protocol は 4 method 確定（§確定 R1-F）。`count_by_workflow` は `feature/workflow-application` 等で Workflow 削除前検査が必要になった時点で追加（YAGNI）。本 PR ではその時点で Repository 第 5 method 追加が容易な拡張ポイントを残すのみ。

### 確定 R1-J: `prompt_kit_prefix_markdown` masking の不可逆性凍結（room §確定 G 実適用の副作用）
...
### 確定 R1-K: `empire_room_refs.room_id → rooms.id` FK closure 詳細（§確定 R1-C 詳述）
...
| 順 | 操作 | 対象 |
|---|---|---|
| 1 | `op.create_table('rooms', ...)` | rooms（workflow_id FK RESTRICT NULLABLE 込み — §確定 R1-M 参照）|
...
### 確定 R1-L: テスト責務の 4 ファイル分割（empire-repo / workflow-repo / agent-repo 教訓を最初から反映）

empire-repo で 506 行 → 500 行ルール違反でディレクトリ分割した教訓、agent-repo で最初から 4 分割した教訓を踏襲。本 PR は**最初から 4 ファイル分割**:

| ファイル | 責務 |
|---|---|
| `test_room_repository/test_protocol_crud.py` | save → find_by_id 経路の正常系、count() の SQL 契約、find_by_name の Empire スコープ検索（§確定 R1-F） |
| `test_room_repository/test_save_semantics.py` | delete-then-insert の物理保証、ORDER BY 規約（§BUG-EMR-001 継承）、`empire_id` 引数経路（§確定 R1-H）|
...
[`domain/detailed-design.md`](../domain/detailed-design.md) §クラス設計（詳細）で「`members[*].agent_id` が指す Agent の存在は application 層 `RoomService.add_member` が `AgentRepository.find_by_id` で確認」と application 層責務として凍結済み。
...
| `workflow_id` | `UUIDStr` | **FK → `workflows.id` ON DELETE RESTRICT**, NULLABLE | 採用 Workflow（後付け紐付け可、§確定 R1-I・§確定 R1-M）|
| `name` | `String(80)` | NOT NULL（DB UNIQUE なし、application 層責務）| 部屋名 |
| `description` | `String(500)` | NOT NULL DEFAULT '' | 用途説明 |
| `prompt_kit_prefix_markdown` | **`MaskedText`** | NOT NULL DEFAULT '' | PromptKit.prefix_markdown（**room §確定 G 実適用、§確定 R1-J 不可逆性凍結**）|
| `archived` | `Boolean` | NOT NULL DEFAULT FALSE | アーカイブ状態 |
| INDEX | `(empire_id, name)` 非 UNIQUE | name `ix_rooms_empire_id_name` | **§確定 R1-F**: find_by_name の Empire スコープ検索（左端プリフィックス）|
...
| 操作 | 対象 |
|----|----|
| `op.create_table('rooms', ...)` | 7 カラム + 2 FK（empire_id CASCADE / workflow_id RESTRICT **nullable=False**（元々の 0005 定義、§確定 R1-M の 0006 で NULLABLE に変更））|
...
**Alembic append-only 原則**: 0005 は不変。`rooms.workflow_id` の NULLABLE 化は §確定 R1-M の 0006 migration が担う。既存環境・新規構築環境ともに 0005→0006 の順で migrate する（0006 が ALTER を適用）。

### 確定 R1-M: `rooms.workflow_id` NULLABLE 化 Alembic migration（Issue #183 Fix）

**背景**: 0005 が `rooms.workflow_id NOT NULL` で生成されたため、`workflow_id` を省略した Room 作成リクエストが `sqlalchemy.exc.IntegrityError: NOT NULL constraint failed: rooms.workflow_id` で 500 を返すブートストラップ循環バグ（Issue #183）が発生していた。Domain 層・Pydantic スキーマ・Frontend 仕様が `workflow_id` を任意としているため、DB 側を Domain 層の意図に揃える（DDD 原則）。

**方針 (ジェンセン確定: 2026-05-07)**: 案A — DB 側を NULLABLE に統一。

| 項目 | 値 |
|----|----|
| revision id | `0006_fix_room_workflow_id_nullable`（固定）|
| down_revision | `0005_room_aggregate`（単一チェーン維持）|

| 操作 | 内容 |
|---|---|
| upgrade | `with op.batch_alter_table('rooms', recreate='always') as batch_op: batch_op.alter_column('workflow_id', nullable=True)` |
| downgrade | `with op.batch_alter_table('rooms', recreate='always') as batch_op: batch_op.alter_column('workflow_id', nullable=False)` |

`batch_alter_table(recreate='always')` を使用する理由: SQLite は `ALTER TABLE ... ALTER COLUMN` を直接サポートしないため（[SQLite ALTER TABLE 制約](https://www.sqlite.org/lang_altertable.html)）、Alembic batch mode での内部 table 再作成（CREATE TABLE new → INSERT SELECT → DROP old → RENAME）が必要。§確定 R1-K での `empire_room_refs` FK closure と同一パターン。

**downgrade 時の挙動（非可逆確定）**: `workflow_id = NULL` な Row が存在する状態での downgrade（`nullable=False` 戻し）は `IntegrityError` で失敗するため、**downgrade は非可逆とする**。誤って downgrade が必要な開発環境では `downgrade base && just db:seed` で DB を再構築する。NULL 行の手動削除後の downgrade は操作コストと事故リスクが高く設計確定として採用しない。

##### Alembic chain 一直線の物理保証

CI で head が分岐していないことを検査する既存スクリプト（M2 永続化基盤で凍結）が単一 chain を assert。本 PR 以降は `0001 → 0002 → 0003 → 0004 → 0005 → 0006` の 6 ステップを assert 対象に追加すること（実装 PR 担当タスク）。

## API エンドポイント詳細

該当なし — 理由: 本 sub-feature は infrastructure 層のみ。HTTP API は `feature/http-api` で凍結する。

## Known Issues

### 申し送り #1: masked `prompt_kit_prefix_markdown` の LLM Adapter 配送経路（§確定 R1-J）
...
- [SQLite — Foreign Key Actions](https://www.sqlite.org/foreignkeys.html#fk_actions) — §確定 R1-I の RESTRICT 挙動
- [SQLite — ALTER TABLE](https://www.sqlite.org/lang_altertable.html) — ADD CONSTRAINT 未サポート、`op.batch_alter_table` 必要性の根拠
- [SQLite — Query Planner](https://www.sqlite.org/queryplanner.html) — INDEX 左端プリフィックス原則（§確定 R1-F）
- [Alembic — `batch_alter_table`](https://alembic.sqlalchemy.org/en/latest/batch.html) — §確定 R1-K の SQLite FK 追加方法
- [`docs/features/persistence-foundation/`](../../persistence-foundation/) — M2 永続化基盤（`MaskedText` + room §確定 G hook 構造）
...
- [`domain/`](../domain/) — Room domain 設計（room §確定 F で `(agent_id, role)` 重複禁止 / room §確定 G で PromptKit Repository マスキング申し送り）
- [`docs/design/domain-model/aggregates.md`](../../../design/domain-model/aggregates.md) — Room 凍結済み設計
- [`docs/design/domain-model/storage.md`](../../../design/domain-model/storage.md) — 逆引き表（本 PR で Room 行追加 + 既存 `PromptKit.prefix_markdown` 行を実適用済みに更新）
