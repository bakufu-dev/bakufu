...
| 操作 | 対象 |
|----|----|
| `op.create_table('rooms', ...)` | 7 カラム + 2 FK（empire_id CASCADE / workflow_id RESTRICT **nullable=True**（§確定 R1-M による訂正）） |
...
**注意（Issue #183 Fix）**: 0005 を `nullable=False` で既に migrate 済みの開発環境では §確定 R1-M の `0006_fix_room_workflow_id_nullable` を適用すること。新規構築時は 0005 を `nullable=True` で実行する（0006 は no-op となる設計）。
...
**downgrade 時の挙動**: `workflow_id = NULL` な Row が存在する場合、`nullable=False` に戻す downgrade は `IntegrityError` で失敗する。データが NULL の行を先に削除するか、downgrade は非可逆とする（開発環境では `downgrade base && just db:seed` が標準手順）。
