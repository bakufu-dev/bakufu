...
> feature: `room`
> 関連: [basic-design.md](basic-design.md) / [`docs/architecture/domain-model/aggregates.md`](../../architecture/domain-model/aggregates.md) §Room
...
`model_config.frozen = True`。`prefix_markdown` の永続化前マスキングは Repository 層で適用（[`storage.md`](../../architecture/domain-model/storage.md) §シークレットマスキング規則 — 本 PR で適用先一覧に追記）。
...
[`value-objects.md`](../../architecture/domain-model/value-objects.md) §AgentMembership で凍結済み。本 feature では再定義せず import のみ。
...
| ID | 出力先 | 文言 |
|----|------|----|
| MSG-RM-001 | 例外 message | `[FAIL] Room name must be 1-80 characters (got {length})` — `{length}` は §確定 B の正規化パイプライン（NFC + `strip()`）適用後の `len()` 値 |
| MSG-RM-002 | 例外 message | `[FAIL] Room description must be 0-500 characters (got {length})` — `{length}` は §確定 B の NFC + strip 適用後 |
| MSG-RM-003 | 例外 message | `[FAIL] Duplicate member: agent_id={agent_id}, role={role}` |
| MSG-RM-004 | 例外 message | `[FAIL] Room members capacity exceeded (got {count}, max 50)` |
| MSG-RM-005 | 例外 message | `[FAIL] Member not found: agent_id={agent_id}, role={role}` |
| MSG-RM-006 | 例外 message | `[FAIL] Cannot modify archived Room: room_id={room_id}` |
| MSG-RM-007 | 例外 message | `[FAIL] PromptKit.prefix_markdown must be 0-10000 characters (got {length})` — `{length}` は §確定 B の通り `prefix_markdown` のみ NFC 正規化後の `len()`（strip は行わない） |
...
- [`docs/architecture/domain-model/storage.md`](../../architecture/domain-model/storage.md) — シークレットマスキング規則（`PromptKit.prefix_markdown` の Repository 永続化時に適用）
