...
<!-- feature: room / sub-feature: domain -->
<!-- 配置先: docs/features/room/domain/test-design.md -->
<!-- 対象範囲: REQ-RM-001〜006 / MSG-RM-001〜007 / 脅威 T1, T2 / 受入基準 1〜14 / 詳細設計 確定 A〜H / archive 冪等性 / leader 必須性 と Workflow 参照整合性 と Agent 存在検証 と name 一意の application 層責務境界 -->

本 sub-feature は domain 層の Aggregate Root（Room）と内部 VO（PromptKit）と例外（RoomInvariantViolation）に閉じる。HTTP API / CLI / UI の公開エントリポイントは持たないため、E2E は本 sub-feature 範囲外（親 [`system-test-design.md`](../system-test-design.md) が受入基準 16, 17, 18 の E2E を担当）。本 sub-feature のテストは **ユニット主体 + 結合は Aggregate 内 module 連携（Room + PromptKit + AgentMembership + RoomInvariantViolation 往復シナリオ）+ 責務境界の物理確認**で構成する。
...
| 確定 G（PromptKit VO 維持） | PromptKit が単一属性 VO のまま frozen + 構造的等価 | TC-UT-RM-024 | ユニット | 正常系 | — |
| 確定 H（webhook auto-mask） | `RoomInvariantViolation` の auto-mask（T2 防御） | TC-UT-RM-014 | ユニット | 異常系 | 14 |
| frozen 不変性 | `Room` / `PromptKit` 属性代入拒否 | TC-UT-RM-025 | ユニット | 異常系 | — |
| frozen 構造的等価 | VO の `__eq__` / `__hash__` | TC-UT-RM-024 | ユニット | 正常系 | — |
...
| MSG-RM-001（2 行構造） | `[FAIL] Room name must be 1-80 characters (got {length})` + `Next:` hint | TC-UT-RM-031 | ユニット | 異常系 | 2 |
| MSG-RM-002（2 行構造） | `[FAIL] Room description must be 0-500 characters (got {length})` + `Next:` hint | TC-UT-RM-032 | ユニット | 異常系 | 3 |
| MSG-RM-003（2 行構造） | `[FAIL] Duplicate member: agent_id={agent_id}, role={role}` + `Next:` hint | TC-UT-RM-033 | ユニット | 異常系 | 5 |
| MSG-RM-004（2 行構造） | `[FAIL] Room members capacity exceeded (got {count}, max 50)` + `Next:` hint | TC-UT-RM-034 | ユニット | 異常系 | 9 |
| MSG-RM-005（2 行構造） | `[FAIL] Member not found: agent_id={agent_id}, role={role}` + `Next:` hint | TC-UT-RM-035 | ユニット | 異常系 | 8 |
| MSG-RM-006（2 行構造） | `[FAIL] Cannot modify archived Room: room_id={room_id}` + `Next:` hint | TC-UT-RM-036 | ユニット | 異常系 | 13 |
| MSG-RM-007（2 行構造、`pydantic.ValidationError` 経由） | `[FAIL] PromptKit.prefix_markdown must be 0-10000 characters (got {length})` + `Next:` hint | TC-UT-RM-037 | ユニット | 異常系 | （文言照合） |
| Next: hint 物理保証（確定 I） | 全 MSG で `assert "Next:" in str(exc)` を CI 強制 | TC-UT-RM-031〜037 | ユニット | 異常系 | （Norman R1 対応） |
| 開発者品質基準（lint/typecheck） | `pyright --strict` / `ruff check` | （CI ジョブ） | — | — | — |
| 開発者品質基準（カバレッジ） | `pytest --cov=bakufu.domain.room` | （CI ジョブ） | — | — | — |
...
- 受入基準 1〜14 すべてに unit/integration ケース（16/17 は system-test-design.md の E2E、18 は repository/test-design.md が担当）
...
- [`../feature-spec.md`](../feature-spec.md) §前提条件・制約 で「ネットワーク: 該当なし」と凍結
...
- 受入基準 16, 17（restart 跨ぎ保持 / Empire 内 name 一意 E2E）は親 [`system-test-design.md`](../system-test-design.md) が担当（TC-E2E-RM-001, 002）
- 受入基準 18（`PromptKit.prefix_markdown` masking）は repository IT（TC-IT-RR-008）と system-test E2E（TC-E2E-RM-003）の両方で検証（[`repository/test-design.md`](../repository/test-design.md) / [`system-test-design.md`](../system-test-design.md)）
- 受入基準 1〜14 はすべて unit/integration テストで検証可能
...
domain 層単独の本 sub-feature では「結合」を **Aggregate 内 module 連携（Room + PromptKit + AgentMembership + RoomInvariantViolation の往復シナリオ）+ pre-validate 連続シナリオ**と定義する。外部 LLM / Discord / GitHub / DB は本 sub-feature では使わないためモック不要。
...
**注**: 本 sub-feature では結合テストも `tests/integration/test_room.py` ではなく `tests/domain/test_room.py` 内の「往復シナリオ」セクションとして実装してよい（empire / workflow / agent と同方針）。

## ユニットテストケース

`tests/factories/room.py` の factory 経由で入力を生成する。raw fixture は本 sub-feature では外部 I/O ゼロのため存在しない。
...
### PromptKit Value Object（確定 G）

| テストID | 対象 | 種別 | 入力 | 期待結果 |
|---------|-----|------|------|---------|
| TC-UT-RM-019 | `PromptKit(prefix_markdown)` 境界値（T1 関連 / 確定 B / G） | 境界値 | prefix_markdown 0 / 10000 / 10001 文字、前後改行を含む文字列 | 0/10000 は成功、10001 で **`pydantic.ValidationError`**（PromptKit VO 構築時に Pydantic field validator が Fail Fast、MSG-RM-007 形式の文言）。`RoomInvariantViolation(kind='prompt_kit_too_long')` 経路は採用しない（Norman R2 指摘を受け確定 I で `kind` Literal から削除済み）。**前後改行は strip されないことを assert**（NFC のみ適用、agent §確定 E と同方針） |
| TC-UT-RM-024 | PromptKit / Room 構造的等価 / hash | 正常系 | 全属性同値の `PromptKit` を 2 インスタンス + 全属性同値の `Room` を 2 インスタンス | `==` True、`hash()` 一致（確定 G）。frozen + `model_config.frozen=True` 起因 |
...
**T1 申し送り**: `PromptKit.prefix_markdown` の secret マスキング（OAuth トークン / API key 等）は本 sub-feature では**長さ上限のみ**を検証する（TC-UT-RM-019 で 10001 文字拒否）。実際の永続化前マスキング（`<REDACTED:ANTHROPIC_KEY>` 等の置換）は `repository` sub-feature の Repository 層で適用される責務として残す（[`../../../design/domain-model/storage.md`](../../../design/domain-model/storage.md) §シークレットマスキング規則）。本 sub-feature では「domain VO は raw 保持、Repository 層で適用」という適用先指定を凍結する。
...
### MSG 文言照合（受入基準 14、文言の静的照合 + Next: hint 物理保証）

**フィードバック原則の物理保証**（Norman R1 / 確定 I 対応）: 全 MSG は **2 行構造**（1 行目 = 失敗事実 `[FAIL] ...`、2 行目 = 次の行動 `Next: ...`）。test 側で **`assert "Next:" in str(exc)`** を必須とし、hint 行が抜けたら CI で落ちる構造で凍結する。これにより「エラーメッセージは"何が失敗したか"ではなく"次に何をすべきか"を伝える」原則が文書宣言（detailed-design.md §確定 I）に留まらず実装でも担保される。

| テストID | MSG ID | 例外型 | 入力 | 期待結果（1 行目: failure 完全一致 / 2 行目: hint 部分一致） |
|---------|--------|--------|------|---------|
| TC-UT-RM-031 | MSG-RM-001 | `RoomInvariantViolation(kind='name_range')` | name='a'*81 | 1 行目: `[FAIL] Room name must be 1-80 characters (got 81)` で始まる、2 行目: **`assert "Next:" in str(exc)`** + `assert "1-80 NFC-normalized" in str(exc)` で `Next: Provide a name with 1-80 NFC-normalized characters; trim leading/trailing whitespace.` を含む |
| TC-UT-RM-032 | MSG-RM-002 | `RoomInvariantViolation(kind='description_too_long')` | description='a'*501 | 1 行目: `[FAIL] Room description must be 0-500 characters (got 501)` で始まる、2 行目: **`assert "Next:" in str(exc)`** + `assert "PromptKit.prefix_markdown" in str(exc)`（長文は PromptKit に逃がす hint） |
| TC-UT-RM-033 | MSG-RM-003 | `RoomInvariantViolation(kind='member_duplicate')` | 重複 `(agent_id, role)` | 1 行目: `[FAIL] Duplicate member: agent_id=<id>, role=LEADER` 形式、2 行目: **`assert "Next:" in str(exc)`** + `assert "different role" in str(exc)`（leader + reviewer の兼任 hint） |
| TC-UT-RM-034 | MSG-RM-004 | `RoomInvariantViolation(kind='capacity_exceeded')` | members 51 件 | 1 行目: `[FAIL] Room members capacity exceeded (got 51, max 50)` で始まる、2 行目: **`assert "Next:" in str(exc)`** + `assert "Remove unused members" in str(exc)` または `"split the work" in str(exc)` |
| TC-UT-RM-035 | MSG-RM-005 | `RoomInvariantViolation(kind='member_not_found')` | `remove_member(unknown_id, role)` | 1 行目: `[FAIL] Member not found: agent_id=<id>, role=DEVELOPER` 形式、2 行目: **`assert "Next:" in str(exc)`** + `assert "GET /rooms/" in str(exc)` または `"already been removed" in str(exc)` |
| TC-UT-RM-036 | MSG-RM-006 | `RoomInvariantViolation(kind='room_archived')` | archived=True の Room に `add_member` | 1 行目: `[FAIL] Cannot modify archived Room: room_id=<id>` 形式、2 行目: **`assert "Next:" in str(exc)`** + `assert "Create a new Room" in str(exc)` + `assert "Phase 2" in str(exc)`（unarchive は Phase 2 hint） |
| TC-UT-RM-037 | MSG-RM-007 | **`pydantic.ValidationError`**（`PromptKit` 構築時に field validator で Fail Fast、Norman R2 / 確定 I 対応） | prefix_markdown='a'*10001 | 1 行目: `[FAIL] PromptKit.prefix_markdown must be 0-10000 characters (got 10001)` を含む、2 行目: **`assert "Next:" in str(exc)`** + `assert "variables" in str(exc)` または `"role_specific_prefix" in str(exc)` または `"sections" in str(exc)`（Phase 2 構造拡張 hint） |
...
- MSG-RM-001 〜 007 の各文言が**静的文字列で照合**されており、加えて **`assert "Next:" in str(exc)` の hint 物理保証**を全 7 ケースで実施（TC-UT-RM-031 〜 037、Norman R1 / 確定 I 対応）。「hint が抜けたら CI で落ちる」物理層でフィードバック原則を凍結
- MSG-RM-007 の例外型を `pydantic.ValidationError` 経由に統一（Norman R2 指摘で `kind='prompt_kit_too_long'` が確定 I の Literal から削除されたデッドコードの除去に同期）
- 受入基準 1 〜 14 の各々が**最低 1 件のユニット/結合ケース**で検証されている（E2E 不在のため戦略ガイドの「結合代替可」に従う）
- 受入基準 16, 17（E2E）は親 system-test-design.md が担当、受入基準 18（masking）は repository/test-design.md が担当
- T1（PromptKit 経由 secret 漏洩）/ T2（webhook URL 例外漏洩）の各脅威に対する対策が**最低 1 件のテストケース**で有効性を確認されている
- 確定 A〜I すべてに証拠ケース（確定 I は MSG 2 行構造 + `prompt_kit_too_long` Literal 削除で TC-UT-RM-019 / 031〜037 が同期済み）
- C0 目標: `domain/room/` 配下で **95% 以上**（domain 層基準）
...
後段で `feature/admin-cli`（`bakufu admin room create`）/ `feature/http-api`（Room CRUD）が完成したら、本 sub-feature の Room を経由して `curl` 経由の手動シナリオで E2E 観測可能になる。
...
- 本 sub-feature では agent と同じく helper module-level 独立化相当の検査関数群を `aggregate_validators.py` に持つ。test 側は `Room` を構築 / メソッド呼び出しすることで間接的に helper を経路網羅する（empire / workflow / agent と同方針）

## 未決課題・要起票 characterization task

| # | タスク | 起票先 | 備考 |
|---|-------|--------|------|
| （N/A） | 該当なし — 外部 I/O ゼロのため characterization 不要 | — | 後続 `repository` sub-feature の Repository 実装（DB 永続化、`rooms.UNIQUE(empire_id, name)` 制約 + `room_members.UNIQUE(room_id, agent_id, role)` 制約）/ `feature/room-ui`（Web UI）が起票時に Room 起点の characterization が発生する見込み |

**Schneier から前回申し送りされた件への対応状況**（empire / workflow / agent レビューより継承）:

- **DB UNIQUE 二重防御**: 本 sub-feature では対象外（domain 層）。`detailed-design.md §データ構造（永続化キー）` に「`rooms.UNIQUE(empire_id, name)` + `room_members.UNIQUE(room_id, agent_id, role)`」が明記されており、`repository` sub-feature の Repository 実装時にこの DB 制約が basic-design.md / detailed-design.md と整合していることを必ず確認する申し送り
- **Unicode 不可視文字 / 同形異字符**: 本 sub-feature では対象外（`feature/http-api` の入力境界責務）。Empire / Workflow / Agent / Room 共通で `name` の機密レベルは低のため認可バイパス経路ではないが、後段で抜けないこと
- **`PromptKit.prefix_markdown` の secret マスキング**: 本 sub-feature では文字列保持のみ。永続化前マスキング規則（[`../../../design/domain-model/storage.md`](../../../design/domain-model/storage.md) §シークレットマスキング規則）の適用は `repository` sub-feature 責務として明示申し送り済み。本 sub-feature では「domain VO は raw 保持、Repository 層で適用」という適用先指定を凍結
- **本 sub-feature 固有の申し送り**:
  ...
  - prompt injection 対策（敵対的プロンプト検出 / ユーザー入力境界 escape）は `feature/llm-adapter` および `feature/http-api` の入力境界責務として残す。本 sub-feature は長さ制約（10000 文字）のみを Pydantic field validator で Fail Fast
...
- [ ] 脅威 T1（PromptKit 経由 secret 漏洩）/ T2（webhook URL 例外漏洩）への有効性確認ケースが含まれ、サニタイズ・マスキングは別 sub-feature 責務として明示申し送りされている
- [ ] 外部 I/O ゼロの主張が basic-design.md / feature-spec.md と整合している（実 ファイルシステム I/O は発生させない、`workflow_id` / `agent_id` は VO 型として保持のみ）
...
- [ ] Schneier 申し送り（DB UNIQUE 二重防御 / `repository` sub-feature でのマスキング規則適用 / LEADER 必須性の `RoomService` 押し出し / prompt injection の `feature/llm-adapter` 責務）が次レビュー時に確認可能な形で test-design および設計書に記録されている
