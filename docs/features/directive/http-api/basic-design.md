...
| 処理 | `DirectiveService.issue(room_id, raw_text)` → 1) Room 存在確認（不在 → `RoomNotFoundError` 404）2) Room archived 確認（archived → `RoomArchivedError` 409）3) `$` プレフィックス正規化（R1-A: `text = raw_text if raw_text.startswith('$') else '$' + raw_text`）4) `Directive(id=uuid4(), text=text, target_room_id=room_id, created_at=now())` 構築（不変条件検査 → `DirectiveInvariantViolation` 422）5) `Task(id=uuid4(), room_id=room_id, directive_id=directive.id, current_stage_id=room.workflow.entry_stage_id, status=PENDING, ...)` 構築 6) `async with session.begin()`: `DirectiveRepository.save(directive)` + `directive.link_task(task.id)` + `DirectiveRepository.save(updated_directive)` + `TaskRepository.save(task)`（アトミック UoW）7) `DirectiveWithTaskResponse` を構築して返す |
...
6. `$` プレフィックス正規化（業務ルール R1-A: 未付加なら自動付加）
7. `Directive(...)` 構築（R1-B〜G 不変条件検査 → `DirectiveInvariantViolation` → 422）
8. `Task(status=PENDING, assigned_agent_ids=[], deliverables={}, ...)` 構築（`TaskInvariantViolation` → 422）
...
10. `DirectiveWithTaskResponse` を構築して HTTP 201 で返す
...
| 観点 | 設計決定 |
|---|---|
| Directive.text の HTTP レスポンスマスキング | `DirectiveResponse.text` は `field_serializer` で `mask()` を適用（defense-in-depth）。DB 永続化時は MaskedText TypeDecorator によりマスキング済み。GET レスポンスでも masked 値を返す（R1-F 準拠）|
| DirectiveInvariantViolation の auto-mask | domain 層で webhook URL を `<REDACTED:DISCORD_WEBHOOK>` に伏字化済み。HTTP レスポンスの `detail` にも masked 値が通過する（R1-D 準拠）|
| UoW のロールバック保証 | `async with session.begin()` は例外時に自動ロールバック。Directive のみ保存されて Task が失われる inconsistent 状態を防ぐ |
| Room 存在確認 | Directive 発行前に Room の存在・archived 状態を確認。存在しない Room への発行は拒否 |
