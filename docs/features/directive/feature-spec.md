...
> 関連 Issue: [#24 feat(directive): Directive Aggregate Root (M1)](https://github.com/bakufu-dev/bakufu/issues/24) / [#34 feat(directive-repository): Directive SQLite Repository (M2)](https://github.com/bakufu-dev/bakufu/issues/34) / [#60 feat(task-http-api): Directive + Task lifecycle HTTP API (M3)](https://github.com/bakufu-dev/bakufu/issues/60)
...
| http-api | [`http-api/`](http-api/) | UI / 外部クライアントから Directive を発行・Task を同時起票する経路（Issue #60）|
...
| UC-DR-005 | CEO | HTTP API 経由で Directive を発行し、同時に Task が起票される（POST /api/rooms/{room_id}/directives → 201 DirectiveWithTaskResponse） | 必須 | http-api |
| UC-DR-006 | CEO | Directive 発行時に Room が存在しないまたは archived の場合、拒否される | 必須 | http-api |

## 6. スコープ

### In Scope

- Directive 業務概念全体で観察可能な業務ふるまい（UC-DR-001〜006）
...
- `target_room_id` の参照整合性検証 → `DirectiveService.issue()` 責務（application 層、http-api sub-feature で実装）
...
| 12 | HTTP API 経由で Directive を発行すると、HTTP 201 レスポンスとして `DirectiveWithTaskResponse`（Directive + 同時起票された Task）が返される（業務ルール R1-A 正規化を含む）| UC-DR-005 | TC-E2E-DR-003（[`system-test-design.md`](system-test-design.md)）|
| 13 | `DirectiveWithTaskResponse` の `directive.text` は `<REDACTED:*>` 形式でマスキングされた値が返される（HTTP レスポンスでの defense-in-depth マスキング、業務ルール R1-F 準拠）| UC-DR-005 | TC-E2E-DR-003 |
| 14 | 存在しない Room への Directive 発行は 404 で拒否される | UC-DR-006 | TC-E2E-DR-003 |
| 15 | archived 済み Room への Directive 発行は 409 で拒否される | UC-DR-006 | TC-E2E-DR-003 |

E2E（受入基準 10）は [`system-test-design.md`](system-test-design.md) で詳細凍結。受入基準 1〜7, 9 は domain sub-feature の IT / UT で検証（[`domain/test-design.md`](domain/test-design.md)）。受入基準 11 は repository sub-feature の IT で検証。受入基準 12〜15 は http-api sub-feature の TC-E2E-DR-003 で検証（[`system-test-design.md`](system-test-design.md)）。
