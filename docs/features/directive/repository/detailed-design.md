# 詳細設計書 — directive / repository

> feature: `directive`
> sub-feature: `repository`
> 親 spec: [`../feature-spec.md`](../feature-spec.md)
> 関連: [basic-design.md](basic-design.md) / [`docs/features/empire-repository/detailed-design.md`](../../empire-repository/detailed-design.md) **テンプレート真実源** / [`docs/features/room-repository/detailed-design.md`](../../room-repository/detailed-design.md) **直近テンプレート** / [`../domain/detailed-design.md`](../domain/detailed-design.md)
...
| `find_by_room(room_id: RoomId)` | RoomId | `list[Directive]` | **第 4 method**、Room スコープ検索 ORDER BY `created_at DESC, id DESC`（BUG-EMR-001 規約: 複合 key で決定論的順序、§確定 R1-D）。空の場合 `[]` |
...
| `find_by_room(room_id)` | RoomId | `list[Directive]` | `SELECT * FROM directives WHERE target_room_id = :room_id ORDER BY created_at DESC, id DESC`。BUG-EMR-001 規約（複合 key で決定論的順序）適用。INDEX(target_room_id, created_at) が複合クエリを最適化 |
...
### 確定 R1-D: `find_by_room` method 設計と `find_by_task_id` 申し送り

#### `find_by_room(room_id: RoomId) -> list[Directive]`

| 項目 | 値 | 根拠 |
|---|---|---|
| クエリ | `SELECT * FROM directives WHERE target_room_id = :room_id ORDER BY created_at DESC, id DESC` | Room 内 directive を最新順で返却、同時刻の場合 id（PK）で決定論的 |
| ORDER BY 方向 | **`created_at DESC, id DESC`** | BUG-EMR-001 規約「複合 key で決定論的順序」適用。`created_at` 単独では同時刻 Directive の順序が非決定的。`id`（PK、UUIDv4、一意）を tiebreaker として加えることで完全決定論化 |
| INDEX 活用 | `INDEX(target_room_id, created_at)` 複合 INDEX の左端プリフィックスが `WHERE target_room_id = :room_id ORDER BY created_at DESC` をカバー。`id` 追加は SQLite が PK lookup で補完 | フルスキャンなし |
| 空 Room | `[]` 返却（NOT Found 扱いではなく空リスト） | 空配列と「Room 不在」は application 層が区別する責務 |
| 子テーブル | なし（flat な 1 行 SELECT のみ）| Directive は子テーブルを持たない |

#### `find_by_task_id` — task-repository PR への申し送り

`find_by_task_id(task_id: TaskId) -> Directive | None` は **本 PR のスコープ外**（YAGNI）。

| 理由 | 詳細 |
|---|---|
| 呼び出し側ゼロ | task-application / task-repository は後続 PR。Protocol に method を先行追加しても呼び出し経路がない |
| INDEX 矛盾 | `find_by_task_id` を追加しても `task_id` INDEX を「YAGNI で追加しない」ことになる矛盾（「method は今、INDEX は将来」は設計内部矛盾） |
| PR #47 前例 | room-repository で `count_by_empire` が YAGNI 違反として却下された直接の前例 |

**task-repository PR で同時追加**: `find_by_task_id` Protocol method + `directives.task_id` INDEX + `directives.task_id → tasks.id` FK closure（§確定 R1-C）の 3 点を一括追加する。設計書は task-repository 設計時に更新する。
...
Directive は `target_room_id` を自身の属性として保持する（[`../domain/detailed-design.md`](../domain/detailed-design.md)）。DB 永続化に必要な `target_room_id` は `directive.target_room_id` から直接取得できる。

[storage.md §Repository save() インターフェース設計パターン §判断ルール](../../../design/domain-model/storage.md) 適用:
...
|---|---|---|
| `row.id` | `id: DirectiveId` | `DirectiveId(row.id)`（**TypeDecorator-trust パターン**: `UUIDStr` TypeDecorator が UUID インスタンスを返すため、防衛的 `UUID()` ラップ不要 — PR #48 v2 確立済み。`DirectiveId(UUID(row.id))` は禁止） |
| `row.text` | `text: str` | `MaskedText.process_result_value(row.text, ...)` でデマスキング（TypeDecorator が SQLAlchemy result 取得時に自動適用）|
| `row.target_room_id` | `target_room_id: RoomId` | `RoomId(row.target_room_id)`（TypeDecorator-trust パターン、PR #48 v2 と同一） |
| `row.created_at` | `created_at: datetime` | `DateTime(timezone=True)` が aware datetime として返却（`pytz.utc` または `timezone.utc`） |
| `row.task_id` | `task_id: TaskId \| None` | `None` or `TaskId(row.task_id)`（TypeDecorator-trust パターン、PR #48 v2 と同一） |
...
**TypeDecorator-trust パターン（PR #48 v2 確立）**: `UUIDStr` は `process_result_value` で Python `UUID` インスタンスを返す TypeDecorator。`row.id` の時点で既に `UUID` 型。`DirectiveId` は `UUID` を受け取るコンストラクタを持つため `DirectiveId(row.id)` で十分。`UUID(row.id)` の二重ラップは TypeDecorator を信頼しない防衛的コーディングであり禁止。

## 設計判断の補足

### なぜ `save(directive, room_id)` の非対称パターンを採用しないか

Room は `empire_id` 属性を持たないため `save(room, empire_id)` の非対称パターンが必要だった。一方 Directive は `target_room_id` を Aggregate 自身が属性として保持する（[`../domain/detailed-design.md`](../domain/detailed-design.md)）。`directive.target_room_id` を Repository が参照できるため、非対称パターン不要。[storage.md §確定 H](../../../design/domain-model/storage.md) 判断ルール「Aggregate が DB 永続化に必要な外部 ID を属性として持つ場合は標準 1 引数パターン」を適用。

### なぜ `find_by_room` の ORDER BY が `created_at DESC, id DESC` か（単独 DESC ではない理由）

BUG-EMR-001 規約「複合 key で決定論的順序」の適用。`created_at` 単独では、同一 Room に同時刻（例: application 層で batch 処理した Directive 群）の Directive が複数存在する場合、SQLite の返却順が非決定的になる。`id`（PK、UUIDv4、一意）を tiebreaker として加えることで完全決定論化。UI / CLI での表示順の安定性とテスト再現性を同時に確保する。`INDEX(target_room_id, created_at)` は ASC / DESC どちらにも対応しており INDEX 変更不要。
...
### なぜ `task_id` INDEX と `find_by_task_id` method を 0006 で追加しないか

YAGNI 原則（PR #47 で `count_by_empire` が YAGNI 違反として却下された直接の前例）。task-application / task-repository は後続 PR であり、0006 時点で `find_by_task_id` の呼び出し側はゼロ。「method だけ先に追加して INDEX は後で（YAGNI）」という分割は、Protocol が性能保証のない method を宣言する矛盾を生む。task-repository PR で method + INDEX + FK closure を**一括**追加するのが整合的。§確定 R1-D 参照。
...
該当なし — 理由: Repository は内部 API。ユーザー向けメッセージは application 層（directive feature-spec.md §MSG-DR-NNN）で管理する。
...
| ID | 出力先 | 文言 |
|----|------|------|
| 該当なし | — | Repository 層。MSG-DR-NNN 系（directive feature-spec.md）が application 層で管理 |
...
該当なし — 理由: 本 sub-feature は infrastructure 層のみ。API は `directive/http-api/` sub-feature（将来）で凍結する。
...
- [`../domain/detailed-design.md`](../domain/detailed-design.md) — Directive Aggregate 凍結済み設計
- [`docs/design/domain-model/storage.md`](../../../design/domain-model/storage.md) §シークレットマスキング規則 — `MaskedText` の配線方式と CI 三層防衛の根拠
