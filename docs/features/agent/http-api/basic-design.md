...
| masking utility | `bakufu.infrastructure.security.masking.mask` | infrastructure/security 確定（persistence-foundation）| POST/PATCH レスポンスの prompt_body 伏字化（R1-9 / A02 防御）|
...
| GET レスポンスの `persona.prompt_body` | DB 復元済み masked 値（`<REDACTED:*>`）が返る（R1-9）|
| POST / PATCH レスポンスの `persona.prompt_body` | `PersonaResponse` field_serializer による masked 値が返る（raw token 非露出、R1-9 / A02）|
...
| **T4: prompt_body 経由での秘密情報漏洩（A02）** | CEO が POST / PATCH で API key / GitHub PAT を `prompt_body` に含めた場合、HTTP レスポンスに raw token が露出 | API key / GitHub PAT | `PersonaResponse.prompt_body` が `infrastructure.security.masking.mask()` で伏字化（POST/PATCH）。GET では DB 復元の masked 値（`<REDACTED:*>`）をそのまま返す。find_by_id は成功する — `prompt_body` は pydantic regex 制約なしのフリーテキストのため、workflow R1-16 の pydantic.ValidationError は発生しない（R1-9 業務ルール凍結）|
...
| A02 | Cryptographic Failures | **Persona.prompt_body**: POST/PATCH レスポンスで `PersonaResponse` の field_serializer が `infrastructure.security.masking.mask()` で伏字化。GET レスポンスでは DB 復元の masked 値（`<REDACTED:*>`）がそのまま返る（R1-8 永続化 masking + R1-9 HTTP レスポンス masking で二重防御）|
