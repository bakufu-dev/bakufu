...
<!-- feature: agent -->
<!-- 配置先: docs/features/agent/domain/test-design.md -->
<!-- 対象範囲: REQ-AG-001〜006 / MSG-AG-001〜008 / 脅威 T1, T2 / feature-spec.md §9 受入基準 1〜9 / 詳細設計 確定 A〜E + SkillRef.path 防御規則 / `is_default` ちょうど 1 件強制 / archive 冪等性 / name 一意の application 層責務 / SkillRef.path traversal 防御 7 規則 -->
...
| REQ-AG-003（重複） | `add_skill` で skill_id 重複 | TC-UT-AG-008 | ユニット | 異常系 | 7 |
| REQ-AG-003（容量） | `len(skills) <= 20` | TC-UT-AG-018 | ユニット | 境界値 | （確定 C） |
| REQ-AG-004 | `Agent.remove_skill` 正常系 | TC-UT-AG-009 | ユニット | 正常系 | 8 |
| REQ-AG-004（未登録） | `remove_skill` で skill_id 未登録 | TC-UT-AG-019 | ユニット | 異常系 | （MSG-AG-008） |
| REQ-AG-005 | `Agent.archive()` 正常系 | TC-UT-AG-010 | ユニット | 正常系 | 9 |
...
| REQ-AG-006-⑤ | `skill_id` 重複なし、skills ≤ 20 件 | TC-UT-AG-008, TC-UT-AG-018 | ユニット | 異常系/境界値 | 7 |
...
| frozen 不変性 | `Agent` / `Persona` / `ProviderConfig` / `SkillRef` 属性代入拒否 | TC-UT-AG-026 | ユニット | 異常系 | — |
| frozen 構造的等価 | VO の `__eq__` / `__hash__` | TC-UT-AG-011 | ユニット | 正常系 | — |
...
| 結合シナリオ 1 | `Agent` + `Persona` + `ProviderConfig` + `SkillRef` + `AgentInvariantViolation` 往復 | TC-IT-AG-001 | 結合 | 正常系/異常系 | 1, 5, 7, 8, 9 |
...
- 受入基準 1〜9 すべてに unit/integration ケース（lint/typecheck・カバレッジは CI ジョブ）
...
該当なし — 理由: domain 層のため公開エントリポイントなし。E2E は親 [`../system-test-design.md`](../system-test-design.md) が専管（TC-E2E-AG-001〜003）。sub-feature の test-design.md には IT / UT のみ記載する。
...
### frozen / extra='forbid' / VO 構造的等価（設計不変条件）

| テストID | 対象 | 種別 | 入力 | 期待結果 |
|---------|-----|------|------|---------|
| TC-UT-AG-011 | VO 構造的等価 / hash | 正常系 | 全属性同値の Persona / ProviderConfig / SkillRef を 2 インスタンス | `==` True、`hash()` 一致（frozen VO 設計不変条件） |
...
- 受入基準 1〜9 の各々が**最低 1 件のユニット/結合ケース**で検証されている（E2E は親 system-test-design.md 担当）
- lint/typecheck（pyright/ruff）・カバレッジは CI ジョブで担保（feature-spec.md §10）
...
- [ ] 外部 I/O ゼロの主張が basic-design.md / ../feature-spec.md と整合している（`is_relative_to()` 検査は文字列上の構造照合であり、本 feature では実 ファイルシステム I/O は発生させない方針が確定 — base directory は Path 定数として扱う）
