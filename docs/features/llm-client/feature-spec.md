...
**理由**: [`docs/design/threat-model.md §主要資産`](../../design/threat-model.md)「`BAKUFU_ANTHROPIC_API_KEY` / `BAKUFU_OPENAI_API_KEY`」行の機密性（高）対策、および §A4 管理操作（T1 / T6 対策）の秘密情報管理方針と同一。`tech-stack.md §subprocess の環境変数ハンドリング` と同一方針の HTTP API 版。
