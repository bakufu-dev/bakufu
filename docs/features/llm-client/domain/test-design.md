...
| REQ-LC-003（空文字拒否: §確定D Fail Fast — min_length=1 制約）| `LLMResponse` | TC-UT-RESP-002 | ユニット | 境界値 | §確定D |
...
| TC-UT-LM-006 | extra フィールド処理（実装: frozen=True のみ設定。extra='forbid' は未設定）| 境界値 | `LLMMessage.model_validate({..., 'extra_field': 'x'})` | extra フィールドは無視される（`hasattr(msg, 'extra_field')` が `False`）。`pydantic.ValidationError` は raise されない。frozen のみが model_config に設定されている |
...
| TC-UT-RESP-002 | `LLMResponse` は空文字を拒否する（§確定D Fail Fast: min_length=1）| 境界値 | `content=''` | `pydantic.ValidationError`（`min_length=1` 制約違反）。infrastructure フォールバック責務は §確定D で廃止され Fail Fast に変更された |
...
| TC-UT-MSG-006 | MSG-LC-006（§確定D Fail Fast 変更後: [WARN] → [FAIL]）| `str` 文言検証 | `anthropic_llm_client.py` ソースを直接検査して MSG-LC-006 定数の文言を物理確認 | `'[FAIL] LLM returned no text content'` がモジュールソース内に存在する（フォールバック廃止に伴い [WARN] から [FAIL] に変更済み）|
...
- **LLMResponse 空文字拒否（§確定D Fail Fast: min_length=1）**: TC-UT-RESP-002 で `pydantic.ValidationError` raise を物理確認
...
- [ ] LLMResponse 空文字拒否（TC-UT-RESP-002）で §確定D Fail Fast（min_length=1 → `pydantic.ValidationError`）が domain VO に正しく実装されていることを確認
