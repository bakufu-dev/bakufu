...
| 項目 | 内容 |
|---|---|
| 状態 | **RESOLVED（`feature/issue-35-task-repository` Alembic 0007 で closure 済み）** |
| 発生経緯 | 0006 時点で `tasks` テーブルは未存在（task-repository は後続 PR）。empire_room_refs の BUG-EMR-001 と同パターンの forward reference 問題 |
| 影響（closure 前）| `directives.task_id` は nullable UUIDStr として存在するが、DB レベルの FK 参照整合性が保証されない。存在しない TaskId が task_id に保存された場合でも IntegrityError が発生しない |
| 対策（closure 前）| application 層 `DirectiveService.issue()` が `TaskRepository.find_by_id(task_id)` で存在確認してから `directive.link_task(task_id)` → `save()` を呼ぶ（application 層の参照整合性検査で補完） |
| **closure 内容** | `feature/issue-35-task-repository` の `0007_task_aggregate.py` `upgrade()` 末尾で `op.batch_alter_table('directives')` → `create_foreign_key('fk_directives_task_id', 'tasks', ['task_id'], ['id'], ondelete='RESTRICT')` を追加。詳細は [`docs/features/task-repository/detailed-design.md`](../task-repository/detailed-design.md) §確定 C 参照 |
