...
<!-- 対象範囲: REQ-DRR-001〜005 / 受入基準 1〜13 / 詳細設計 §確定 R1-A〜G / directive §確定 G 実適用物理確認 + §BUG-DRR-001 申し送り確認 -->

本 feature は M2 Repository **6 番目の Aggregate Repository PR**（empire / workflow / agent / room 後）。directive Aggregate（M1、PR #24 マージ済み）に対する Repository 層を新規追加する。テンプレートは room-repository (PR #48) を 100% 継承し、**5-method Protocol**（`find_by_id` / `count` / `save(directive)` / `find_by_room` / `find_by_task_id`）と **full-mask テーブル**（`directives.text`）の構造を確立する。

room-repo のテンプレートを継承しつつ、directive-repository 固有の論点 4 件を**専用テストファイルで物理保証**する:

1. **`text` MaskedText**（directive §確定 G 実適用）— Schneier 多層防御 3 件目を Repository 経由で物理確認、不可逆性確認含む
2. **`find_by_room(room_id)` ORDER BY created_at DESC**（§確定 R1-D）— Room スコープ検索・最新順・INDEX(target_room_id, created_at) 活用
3. **`find_by_task_id(task_id)`**（§確定 R1-D）— Task から発行元 Directive 逆引き・`find_by_id` 委譲パターン
4. **`target_room_id` FK ON DELETE CASCADE + §BUG-DRR-001 `task_id` FK 申し送り**（§確定 R1-B / R1-C）— Room 削除で Directive 自動削除 / task_id FK は task-repository PR に申し送り

**最初から 4 ファイル分割**（room-repo PR #48 正規構成を継承。`test_masking_text.py` が directive §確定 G 実適用の物理確認を担う本 PR 固有のテストファイル）。

## テストマトリクス

| 要件ID | 実装アーティファクト | テストケースID | テストレベル | 種別 | 受入基準 |
|--------|-------------------|---------------|------------|------|---------|
| REQ-DRR-001 | `DirectiveRepository` Protocol **5 method** 定義 | TC-UT-DRR-001 | 結合 | 正常系 | 1, 2 |
| REQ-DRR-002（find_by_id） | `find_by_id` 存在 / 不在 | TC-UT-DRR-002 | 結合 | 正常系 | 3 |
| REQ-DRR-002（save round-trip） | `save(directive)` UPSERT → `find_by_id` round-trip | TC-UT-DRR-003 | 結合 | 正常系 | 4 |
| REQ-DRR-002（save UPSERT 更新） | 既存 Directive を再 save で UPDATE | TC-UT-DRR-007 | 結合 | 正常系 | 4 |
| REQ-DRR-002（save task_id 更新） | `link_task(task_id)` → re-save で `task_id` 更新 | TC-UT-DRR-008 | 結合 | 正常系 | 4 |
| REQ-DRR-002（count SQL） | `count()` が SQL `COUNT(*)` を発行 | TC-UT-DRR-006 | 結合 | 正常系 | 8 |
| REQ-DRR-002（find_by_room） | Room 内 Directive 一覧 ORDER BY created_at DESC | TC-UT-DRR-004 / TC-UT-DRR-004b | 結合 | 正常系 | 6 |
| REQ-DRR-002（find_by_task_id） | Task 紐付け Directive 逆引き / 未紐付けは None | TC-UT-DRR-005 | 結合 | 正常系 | 7 |
| REQ-DRR-002（Tx boundary） | commit path 永続化 / rollback path 破棄 | TC-UT-DRR-009 | 結合 | 正常系 / 異常系 | — |
| **REQ-DRR-002（masking、§確定 R1-B / directive §確定 G）** | raw `text` → DB に `<REDACTED:*>` 永続化（**directive §確定 G 実適用**）| TC-IT-DRR-010-masking-* (7 経路) | 結合 | 正常系 | 5 |
| REQ-DRR-003（Alembic 0006 DDL）| 1 テーブル + INDEX + FK 1 件作成 | TC-IT-DRR-001 | 結合 | 正常系 | 9 |
| REQ-DRR-003（Alembic chain） | 0001→0002→...→0006 単一 head | TC-IT-DRR-002 | 結合 | 正常系 | — |
| REQ-DRR-003（upgrade/downgrade） | 双方向 migration が idempotent | TC-IT-DRR-003 | 結合 | 正常系 | 9 |
| REQ-DRR-003（down_revision） | `0006.down_revision == "0005_room_aggregate"` | TC-IT-DRR-004 | 結合 | 正常系 | — |
| REQ-DRR-003（CASCADE FK 物理確認） | Room 削除で Directive 自動削除（§確定 R1-B） | TC-IT-DRR-005 | 結合 | 正常系 | 10 |
| REQ-DRR-003（task_id FK なし確認） | 0006 時点で `task_id` FK が存在しないことを物理確認（§BUG-DRR-001）| TC-IT-DRR-006 | 結合 | 正常系 | 11 |
| REQ-DRR-004（CI Layer 2）| arch test parametrize（`_FULL_MASK_TABLES` or 相当に `directives` 追加）| TC-UT-DRR-arch | 結合 | 正常系 | 12 |
| REQ-DRR-004（CI Layer 1）| grep guard で `directives.text` の `MaskedText` 必須 | （CI ジョブ） | — | — | 12 |
| REQ-DRR-005（storage.md） | §逆引き表更新（`directives.text: MaskedText` 追加）| TC-DOC-DRR-001 | doc 検証 | 正常系 | 13 |
...
| **§確定 R1-B（CASCADE FK）** | target_room_id → rooms.id ON DELETE CASCADE 物理確認 | TC-IT-DRR-005 | 結合 | 正常系 | 10 |
| **§確定 R1-C（BUG-DRR-001 申し送り）** | 0006 時点で task_id FK なし（PRAGMA foreign_key_list 確認）| TC-IT-DRR-006 | 結合 | 正常系 | 11 |
| **§確定 R1-D（find_by_room ORDER BY）** | created_at DESC ORDER BY の SQL ログ物理確認 | TC-UT-DRR-004 | 結合 | 正常系 | 6 |
| **§確定 R1-D（find_by_task_id 委譲）** | find_by_id 委譲パターンの SQL ログ確認 | TC-UT-DRR-005 | 結合 | 正常系 | 7 |
| **§確定 R1-E（CI 三層防衛）** | 正のチェック + 負のチェック | TC-UT-DRR-arch + TC-DOC-DRR-001 | 結合 / doc | 正常系 | 12 |
| **§確定 R1-F（save 1 引数）** | `save(directive)` が `directive.target_room_id` から直接 FK 解決 | TC-UT-DRR-003 + TC-UT-DRR-LIFECYCLE | 結合 | 正常系 | — |
| **§確定 R1-G（_to_row / _from_row）** | `created_at` timezone-aware 往復 / `task_id` None↔UUID 往復 | TC-UT-DRR-003 + TC-UT-DRR-008 | 結合 | 正常系 | — |
| **directive §確定 G（masking 不可逆性、Repository 実適用）** | masked `text` 復元時に raw 戻らない | TC-IT-DRR-010-masking-roundtrip | 結合 | 正常系 | 5 |
| **§BUG-DRR-001（task_id FK 申し送り）** | 0006 で task_id FK が `directives` テーブルに存在しないことを確認 | TC-IT-DRR-006 | 結合 | 正常系 | 11 |
| **Lifecycle 統合** | save → find_by_room → find_by_task_id → save（更新）の 5 method 連携 | TC-IT-DRR-LIFECYCLE | 結合 | 正常系 | 1, 4, 6, 7 |

**マトリクス充足の証拠**:

- REQ-DRR-001〜005 すべてに最低 1 件のテストケース
- **directive §確定 G 実適用 7 経路**（discord / anthropic / github / bearer / no-secret / roundtrip / multiple）すべてに TC-IT-DRR-010-masking-* で物理確認
- **find_by_room ORDER BY created_at DESC**（§確定 R1-D）: TC-UT-DRR-004 で複数 Directive の順序 + SQL ログ `ORDER BY created_at DESC` を物理確認
- **find_by_task_id → find_by_id 委譲**（§確定 R1-D）: TC-UT-DRR-005 で SQL ログ `LIMIT 1` + 存在 / 不在の 2 経路確認
...
- 受入基準 1〜11 すべてに unit/integration ケース、12〜13 は CI / doc 確認
- 孤児要件ゼロ

## 外部 I/O 依存マップ

本 feature は infrastructure 層の Repository 実装。empire-repo / workflow-repo / agent-repo / room-repo と同方針で本物の SQLite + 本物の Alembic + 本物の SQLAlchemy AsyncSession を使う。
...
- 後続 `feature/directive-application` / `feature/admin-cli` / `feature/http-api` が公開 I/F を実装した時点で E2E を起票
...
### Protocol 定義 + 充足（受入基準 1, 2、§確定 R1-A）

| テストID | 対象モジュール連携 | 使用 fixture | 前提条件 | 操作 | 期待結果 |
|---------|------------------|--------------|---------|------|---------|
| TC-UT-DRR-001 | `DirectiveRepository` Protocol が **5 method** を宣言 | — | `application/ports/directive_repository.py` がインポート可能 | `from bakufu.application.ports.directive_repository import DirectiveRepository` | Protocol が `find_by_id` / `count` / `save(directive)` / `find_by_room` / `find_by_task_id` の **5 method** を宣言、すべて `async def`、`@runtime_checkable` なし（empire §確定 A）|
| （TC-UT-DRR-001 内）| `SqliteDirectiveRepository` の Protocol 充足 | `session_factory` | engine + Alembic 適用済み | `repo: DirectiveRepository = SqliteDirectiveRepository(session)` で型代入が pyright で通る | pyright strict pass、duck typing で 5 method 全 `hasattr` 確認 |

### 基本 CRUD（受入基準 3, 4, 8、§確定 R1-B / R1-F / R1-G）

| テストID | 対象モジュール連携 | 使用 fixture | 前提条件 | 操作 | 期待結果 |
|---------|------------------|--------------|---------|------|---------|
| TC-UT-DRR-002 | `find_by_id` 存在 / 不在（受入基準 3） | `session_factory` + `make_directive` + `seeded_room_id` | seeded room | (1) `save(directive)` → `find_by_id(directive.id)` / (2) `find_by_id(uuid4())` | (1) 保存済み Directive を返す / (2) None を返す |
| TC-UT-DRR-003 | `save(directive)` → `find_by_id` round-trip 等価（受入基準 4）| `session_factory` + `make_directive` + `seeded_room_id` | empty directives テーブル + seeded room | `save(directive)` → `find_by_id(directive.id)` | 復元 Directive が `id` / `text` / `target_room_id` / `created_at` / `task_id` すべて元 Directive と等価。`created_at` が UTC tz-aware datetime で往復（§確定 R1-G） |
| TC-UT-DRR-006 | `count()` SQL `COUNT(*)` 契約（受入基準 8）| `session_factory` + `make_directive` + `seeded_room_id` + `before_cursor_execute` event | DB に複数 Directive 保存済 | `count()` 呼び出し + SQL ログ観測 | `SELECT count(*) FROM directives` が発行される、全行ロード経路（`SELECT id FROM directives`）が**ない**ことを assert |
...
### `find_by_room` ORDER BY created_at DESC（受入基準 6、§確定 R1-D）
...
### `find_by_task_id` Task 逆引き（受入基準 7、§確定 R1-D）

**`test_find_by_task_id.py`** — §確定 R1-D `find_by_task_id` 専用テストファイル。

| テストID | 対象 | 種別 | 前提条件 | 操作 | 期待結果 |
|---------|-----|------|---------|------|---------|
| TC-UT-DRR-005 | `find_by_task_id(task_id)` が紐付け済み Directive を返す | 正常系 | seeded room + `directive.link_task(task_id)` で task_id 付き Directive を保存 | `find_by_task_id(task_id)` | 対応する Directive を返す。SQL ログに `WHERE task_id = :task_id LIMIT 1` が含まれる（委譲パターン確認、§確定 R1-D）|
| TC-UT-DRR-005b | `find_by_task_id(task_id)` が未紐付けの場合 None を返す | 正常系 | seeded room + Directive 存在 (task_id=None) | 存在しない `task_id` で `find_by_task_id(unknown_task_id)` | None 返却 |
| TC-UT-DRR-005c | `find_by_task_id` の `find_by_id` 委譲経路で `_from_row` が完全復元する | 正常系 | task_id 付き Directive が保存済み | `find_by_task_id(task_id)` の戻り値属性確認 | 全属性（id / text / target_room_id / created_at / task_id）が保存値と等価 |

### directive §確定 G 実適用 7 経路（受入基準 5、本 PR の核心テストファイル）
...
| TC-IT-DRR-005 | `target_room_id` FK ON DELETE CASCADE（§確定 R1-B、受入基準 10）| `session_factory` + `seeded_room_id` + `make_directive` | seeded room に Directive を保存済み | raw SQL `DELETE FROM rooms WHERE id = :room_id` 実行 → `SELECT * FROM directives WHERE target_room_id = :room_id` | Directive 行が自動削除（CASCADE 発火）。`SELECT` 結果が空になる |
| TC-IT-DRR-006 | §BUG-DRR-001: 0006 で `task_id → tasks.id` FK が存在しないことを物理確認（受入基準 11）| `session_factory` | Alembic 0006 適用済み | `PRAGMA foreign_key_list('directives')` で FK 一覧取得 | FK 参照テーブル一覧に `tasks` が**含まれない**。0006 時点での forward reference 問題回避（task-repository PR で FK closure 申し送り）|

### CI 三層防衛 Directive 拡張（受入基準 12、§確定 R1-E）
...
| テストID | 対象 | 使用 fixture | 前提条件 | 操作 | 期待結果 |
|---------|-----|--------------|---------|------|---------|
| TC-IT-DRR-LIFECYCLE | 5 method 全経路 — save → find_by_room → find_by_task_id → save（更新）連携 | `session_factory` + `seeded_room_id` + `make_directive` | empty directives テーブル + seeded room | (1) `save(d1)` / `save(d2)` / `save(d3)` → (2) `find_by_room(room_id)` → 3 件、created_at DESC 順 → (3) `d1.link_task(task_id)` → `save(d1_updated)` → (4) `find_by_task_id(task_id)` → d1_updated 返却 → (5) `count()` → 3 → (6) `find_by_id(d2.id)` → d2 | 各段階で valid Directive を返す。link_task → re-save が UPSERT で task_id を更新。masking なし（plain text）なので round-trip equality 成立。`count()` は `SELECT count(*)` 単独発行 |
...
- domain layer のテストは directive feature #24 で完了済み（本 PR スコープ外）

## カバレッジ基準

- REQ-DRR-001〜005 すべてに最低 1 件のテストケース
- **directive §確定 G 実適用 7 経路**（discord / anthropic / github / bearer / no-secret / roundtrip / multiple）すべてに正常系ケース、TC-IT-DRR-010-masking-* で物理確認
- **find_by_room ORDER BY created_at DESC**（§確定 R1-D）: TC-UT-DRR-004 で 4 経路（降順確認 / 空リスト / Room スコープ分離 / _from_row 往復）すべてに証拠
- **find_by_task_id → find_by_id 委譲**（§確定 R1-D）: TC-UT-DRR-005 で 3 経路（存在 / 不在 / 属性往復）すべてに証拠 + SQL ログで LIMIT 1 物理確認
- **target_room_id FK ON DELETE CASCADE**（§確定 R1-B）: TC-IT-DRR-005 で Room 削除時の Directive 自動削除を物理確認
  ...
  tests/
    ...
    infrastructure/
      persistence/
        sqlite/
          repositories/
            test_directive_repository/              # 新規ディレクトリ（4 ファイル分割）
              __init__.py
              conftest.py                            # seed_room helper（FK 解決用: empire + workflow + room をシード）
              test_protocol_crud.py                  # TC-UT-DRR-001〜009 + TC-IT-DRR-LIFECYCLE
              test_find_by_room.py                   # TC-UT-DRR-004 / 004b / 004c / 004d
              test_find_by_task_id.py                # TC-UT-DRR-005 / 005b / 005c
...
- `test_find_by_room.py` / `test_find_by_task_id.py` を独立ファイルにするのは 5-method Protocol の拡張 2 method が本 PR 固有の論点であり、500 行ルール（empire-repo PR #29 / agent-repo PR #45 教訓）に従い最初から分割するため
...
| Directive 後続申し送り #3 | `find_by_task_id` の `task_id` INDEX 追加 | `feature/task-repository`（後続 Issue #35）| 現状は FK なし + INDEX なし（YAGNI、MVP 数十 Directive でフルスキャン許容）。task-repository PR で FK closure と同時に判断 |
...
- [ ] **directive §確定 G 実適用 7 経路**（discord / anthropic / github / bearer / no-secret / roundtrip / multiple）すべてに TC-IT-DRR-010-masking-* で物理確認
- [ ] **find_by_room ORDER BY created_at DESC**（§確定 R1-D）が TC-UT-DRR-004 で SQL ログ `ORDER BY created_at DESC` + 降順一覧 + Room スコープ分離を物理確認
- [ ] **find_by_task_id → find_by_id 委譲**（§確定 R1-D）が TC-UT-DRR-005 で SQL ログ `LIMIT 1` + 存在 / 不在 2 経路を確認
...
- [ ] **テストファイル分割（4 ファイル）が basic-design.md §モジュール構成と整合**
