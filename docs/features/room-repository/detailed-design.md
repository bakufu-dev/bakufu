...
classDiagram
    class RoomRepository {
        ...
        +async count_by_empire(empire_id: EmpireId) int
    }
...
| `find_by_name(empire_id: EmpireId, name: str)` | EmpireId + name 文字列 | `Room \| None` | **第 4 method**、Empire スコープでの一意検査（agent §R1-C 継承）。不在時 None。`INDEX(empire_id, name)` 経由 |
| `count_by_empire(empire_id: EmpireId)` | EmpireId | `int` | **第 5 method**、Empire 内 Room 数（§確定 R1-F、本 PR で初導入）。SQL `COUNT(*) WHERE empire_id=?` 契約、全行ロード+ Python `len()` 禁止 |
...
| `find_by_name(empire_id, name)` | EmpireId + str | `Room \| None` | `SELECT id FROM rooms WHERE empire_id = :empire_id AND name = :name LIMIT 1` で RoomId 取得 → 存在すれば `find_by_id(found_id)` で子テーブル含めて Room 復元（§確定 G）|
| `count_by_empire(empire_id)` | EmpireId | int | `select(func.count()).select_from(RoomRow).where(RoomRow.empire_id == empire_id)` で SQL `COUNT(*) WHERE` 発行、`scalar_one()` で int 取得 |
...
### 確定 D: `count()` / `count_by_empire()` SQL 契約（empire-repo §確定 D 踏襲 + 拡張）
...
| **`count_by_empire(empire_id)`: `select(func.count()).select_from(RoomRow).where(RoomRow.empire_id == empire_id)` で SQL `COUNT(*) WHERE` 発行**、`scalar_one()` で int 取得（**INDEX(empire_id, name) 左端プリフィックスが効く**）| `find_by_id` を全件呼んで Empire でフィルタ後 `len()` | N+1、後続 PR が真似すると数千 Room でメモリ枯渇 |
...
### 確定 G: `count_by_empire(empire_id)` 第 5 method 契約（§確定 R1-F 詳細、本 PR で初導入）

##### 採用方針: SQL `COUNT(*) WHERE empire_id=?` で Empire 内 Room 数を返す

| 候補 | 採否 | 理由 |
|---|---|---|
| **(a) `select(func.count()).select_from(RoomRow).where(RoomRow.empire_id == empire_id)`** | ✓ 採用 | SQL レベル `COUNT(*) WHERE`、O(log N) で `INDEX(empire_id, name)` 左端プリフィックスが効く |
| (b) `find_by_id` を全件呼んで filter 後 `len()` | ✗ 不採用 | N+1、メモリ枯渇 |
| (c) `count()` で全 Empire 合計のみ提供、Empire スコープは廃止 | ✗ 不採用 | Empire admin CLI / UI で Empire 単位の Room 数表示が後続 PR で必要、SQL レベルで吸収すべき |

##### 後続 Repository PR への申し送り（テンプレート責務）

本 §確定 G で「Aggregate 集合カウントの SQL レベル吸収（`count_by_*(scope_id)` パターン）」を凍結。後続 PR が同パターンを採用する場合:

| 後続 PR | 想定される追加 method |
|---|---|
| `feature/task-repository`（Issue #35） | `count_by_room(room_id)` / `count_by_directive(directive_id)` |
| `feature/external-review-gate-repository`（Issue #36） | `count_by_decision(decision)` / `count_by_task_id(task_id)`（既に Issue #36 で凍結済み） |
...
- `count_by_empire(empire_id)` の `WHERE` 句に必須（§確定 G）
...
**追加しない**。本 PR の Protocol は 5 method 確定（§確定 R1-F）。`count_by_workflow` は `feature/workflow-application` 等で Workflow 削除前検査が必要になった時点で追加（YAGNI）。本 PR ではその時点で Repository 第 6 method 追加が容易な拡張ポイントを残すのみ。
...
| ファイル | 責務 |
|---|---|
| `test_room_repository/test_protocol_crud.py` | save → find_by_id 経路の正常系、count() の SQL 契約、find_by_name の Empire スコープ検索、count_by_empire の SQL `COUNT(*) WHERE` 契約 |
...
### なぜ `count_by_empire` を Protocol に追加するか（empire-repo の 3 method、agent-repo の 4 method を超える初の Repository）

empire-repo §確定 D で `count()` は SQL `COUNT(*)` 限定と凍結したが、これは**全 Aggregate の合計**を返す method。Empire 単位での Room 数 / Workflow 単位での Room 数等の **集合カウント by scope** が後続 PR で必要になる経路は room §確定の「Empire ダッシュボードで Room 一覧表示」「admin CLI list-rooms」で見えている。

application 層が `find_by_id` を全件呼んで filter する実装は N+1 / メモリ枯渇のテンプレを後続 PR に伝染させる経路（empire-repo §確定 D の `count()` 教訓と同方針）。

`count_by_empire(empire_id)` を Repository に追加することで、**集合カウントを SQL レベルで吸収**する責務分離テンプレートを確立。後続 PR（task / external-review-gate-repository 等）が同パターン（`count_by_room` / `count_by_decision` 等）を真似できる。
...
| INDEX | `(empire_id, name)` 非 UNIQUE | name `ix_rooms_empire_id_name` | **§確定 R1-F**: find_by_name + count_by_empire の左端プリフィックス |
