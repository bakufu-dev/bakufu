...
<!-- 対象範囲: REQ-RR-001〜005 / 受入基準 1〜13 / 詳細設計 §確定 A〜H / room §確定 G 実適用物理確認 + BUG-EMR-001 Stage 2 closure -->
...
| REQ-RR-003（BUG-EMR-001 closure）| Alembic 0005 で `empire_room_refs.room_id → rooms.id` FK が追加（§確定 R1-C）| TC-IT-RR-011 | 結合 | 正常系 | 11 |
| REQ-RR-003（Alembic 0005）| 2 テーブル + UNIQUE + INDEX(empire_id, name) + 3 FK + empire_room_refs FK closure | TC-IT-RR-012 | 結合 | 正常系 | 11 |
| REQ-RR-004（CI Layer 1）| grep guard で `rooms.prompt_kit_prefix_markdown` の `MaskedText` 必須 | （CI ジョブ） | — | — | 12 |
| REQ-RR-004（CI Layer 2）| arch test parametrize（`_PARTIAL_MASK_TABLES` に `rooms` 追加） | TC-UT-RR-013-arch | ユニット | 正常系 | 12 |
| REQ-RR-005（storage.md） | §逆引き表更新（`rooms` partial-mask + `room_members` no-mask） | TC-DOC-RR-001 | doc 検証 | 正常系 | （Layer 3） |
| AC-12（依存方向） | `domain` 層から `application` / `infrastructure` への import がゼロ件 | （既存 CI script） | — | — | 12 |
| AC-13（lint/typecheck/coverage）| pyright strict / ruff / coverage 90% | （CI ジョブ）| — | — | 13 |
...
| 確定 E（CI 三層防衛 partial-mask 拡張） | 正のチェック + 負のチェック併用 | TC-UT-RR-013-arch + TC-DOC-RR-001 | ユニット / doc | 正常系 | 12 |
...
| **§BUG-EMR-001 Stage 2 closure** | Alembic 0005 で empire_room_refs.room_id FK 追加、empire 削除で room_members CASCADE 削除 | TC-IT-RR-011 + TC-IT-RR-012 | 結合 | 正常系 | 11 |
| **lifecycle 統合** | save → find_by_name → find_by_id → save（更新）の 5 method 連携 | TC-IT-RR-LIFECYCLE | 結合 | 正常系 | 1, 6, 7 |

**マトリクス充足の証拠**:

- REQ-RR-001〜005 すべてに最低 1 件のテストケース
...
- 受入基準 1〜13 のうち 1〜11 は unit/integration ケース、12（CI 三層防衛 + storage.md）/ 13（CI ジョブ）は CI 担保
- 確定 A〜H すべてに証拠ケース
- 孤児要件ゼロ
...
### Alembic 0005 + BUG-EMR-001 Stage 2 closure（受入基準 11、§確定 R1-C）
...
### CI 三層防衛 partial-mask 拡張（受入基準 12、§確定 E）
