...
<!-- 対象範囲: REQ-RR-001〜006 / 受入基準 1〜18 / 詳細設計 §確定 A〜F, H〜L / room §確定 G 実適用物理確認 + BUG-EMR-001 Stage 2 closure -->
...
2. **`find_by_name(empire_id, name)` Empire スコープ強制**（§確定 F）— IDOR 防御、INDEX(empire_id, name) 利用
...
| REQ-RR-002（find_by_name）| Empire スコープ検索（4th method、§確定 F） | TC-UT-RR-005 | ユニット | 正常系 / 異常系 | 6 |
| REQ-RR-002（save 引数 §確定 H）| `save(room, empire_id)` で empire_id 引数経由（Room に empire_id 属性なし） | TC-UT-RR-006 | ユニット | 正常系 | （責務境界） |
...
| 確定 D（count SQL）| `select(func.count())` の物理確認 | TC-UT-RR-004 | ユニット | 正常系 | 5 |
...
| **確定 H（save(room, empire_id) signature）** | Room に empire_id 属性なし、引数で受け取る | TC-UT-RR-006 + TC-UT-RR-013-roundtrip | ユニット | 正常系 | （責務境界） |
| 確定 L（4 ファイル分割） | test_*.py 全ファイル 500 行未満 | （静的確認） | — | — | — |
...
- **find_by_name Empire スコープ強制**（§確定 F、IDOR 防御）: TC-UT-RR-005 で 3 経路（ヒット / 不在 / 異 Empire 同 name）+ SQL ログ観測で `WHERE empire_id=?` + `LIMIT 1` の物理確認
...
- **save(room, empire_id) signature**（§確定 H）: TC-UT-RR-006 で empire_id 引数経由の保存 + Room domain の empire_id 属性不在を確認
...
| TC-UT-RR-006 | `save(room, empire_id)` 引数経由の保存（§確定 H） | `session_factory` + `make_room` | Room domain に empire_id 属性なし | (1) `save(room, empire_id_a)` 保存、(2) raw SQL `SELECT empire_id FROM rooms WHERE id=?` で確認 | empire_id 引数の値が `rooms.empire_id` カラムに永続化、Room VO 自体は empire_id を持たない |
...
- **`save(room, empire_id)` signature（§確定 H）**: TC-UT-RR-006 で empire_id 引数経由保存 + Room VO に empire_id 属性不在を確認
...
- [ ] **`save(room, empire_id)` signature（§確定 H）** が TC-UT-RR-006 で物理確認（empire_id 引数経由保存 + Room VO に empire_id 属性不在）
