...
<!-- 対象範囲: REQ-RR-001〜006 / 受入基準 1〜18 / 詳細設計 §確定 R1-A〜F, R1-H〜L / room §確定 G 実適用物理確認 + BUG-EMR-001 Stage 2 closure -->

本 feature は M2 Repository **5 番目の Aggregate Repository PR**（empire / workflow / agent 後）。room Aggregate（M1、PR #22 マージ済み）に対する Repository 層を新規追加する。テンプレートは agent-repository (PR #45) を 100% 継承し、**4 method Protocol**（`find_by_id` / `count` / `save(room, empire_id)` / `find_by_name`）と **partial-mask テーブル**（`rooms.prompt_kit_prefix_markdown`）の構造を確立する。
...
|--------|-------------------|---------------|------------|------|---------|
| REQ-RR-001 | `RoomRepository` Protocol 4 method 定義 | TC-UT-RR-001 | ユニット | 正常系 | 1, 2 |
...
| REQ-RR-002（find_by_name）| Empire スコープ検索（4th method、§確定 R1-F） | TC-UT-RR-005 | ユニット | 正常系 / 異常系 | 6 |
...
| 確定 R1-B（save 3 段階） | DML 順序の物理確認 | TC-UT-RR-002 | ユニット | 正常系 | 3 |
| 確定 R1-C（_to_row / _from_row）| 双方向変換 + empire_id 引数経由 | TC-UT-RR-014-roundtrip | ユニット | 正常系 | — |
| 確定 R1-D（count SQL）| `select(func.count())` の物理確認 | TC-UT-RR-004 | ユニット | 正常系 | 5 |
| 確定 R1-E（CI 三層防衛 partial-mask 拡張） | 正のチェック + 負のチェック併用 | TC-UT-RR-013-arch + TC-DOC-RR-001 | ユニット / doc | 正常系 | 15 |
| 確定 R1-F（find_by_name 契約） | Empire スコープで RoomId → find_by_id 委譲 | TC-UT-RR-005 | ユニット | 正常系 / 異常系 | 6 |
...
| **lifecycle 統合** | save → find_by_name → find_by_id → save（更新）の 4 method 連携 | TC-IT-RR-LIFECYCLE | 結合 | 正常系 | 1, 6 |
...
- 受入基準 1〜11 は unit/integration ケース、12〜14（Migration/DB 制約物理確認）は TC-IT-RR-011/012 で担保、15〜16（CI 三層防衛・doc・empire-repo sync）は CI/コードレビュー担保、17〜18（lint/typecheck/coverage）は CI ジョブ担保
...
### 基本 CRUD（受入基準 3〜6、§確定 R1-B / R1-D / R1-F / R1-H）
...
### CI 三層防衛 partial-mask 拡張（受入基準 15、§確定 R1-E）
...
- **CI 三層防衛 partial-mask 拡張（§確定 R1-E）**: Layer 1 grep（CI ジョブ）+ Layer 2 arch（TC-UT-RR-013-arch）+ Layer 3 storage.md（TC-DOC-RR-001）3 つすべてに証拠
...
- [ ] **CI 三層防衛 partial-mask 拡張（§確定 R1-E）**: Layer 1 grep（CI）+ Layer 2 arch（TC-UT-RR-013-arch）+ Layer 3 storage.md（TC-DOC-RR-001）の 3 つすべてに証拠
