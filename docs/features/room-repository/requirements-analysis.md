...
4. agent-repo §確定 R1-C で「Empire 内一意 Aggregate には Repository 第 4 method `find_by_name(empire_id, name)` を追加」テンプレを確立。Room の `name` は room §確定（[aggregates.md §Room](../../design/domain-model/aggregates.md) L23-31）で「Empire 内で一意」と凍結済みで、本 PR が **agent-repo §R1-C パターン適用 2 件目** として後続 directive / task / external-review-gate-repository の真似元になる
...
| `count_by_empire(empire_id)` を Protocol に追加する | **YAGNI 違反（却下）**。呼び出し側（application / CLI / HTTP API）が現時点で不在。Empire admin CLI / UI での「Empire 内 Room 数」表示は後続 PR の要件であり、現時点で Protocol に追加すると「呼ばれない method が Protocol を汚す」形になる。後続 PR で `count()` を拡張するか新 method を追加する設計が正しい。|
...
| ON DELETE | **`RESTRICT`** | Workflow は Room の参照先であって所有者ではない（[aggregates.md §Workflow](../../design/domain-model/aggregates.md) L46-72 で凍結）。`CASCADE` は Workflow 削除で Room が勝手に消える副作用、データ消失リスク大。`RESTRICT` で Workflow 削除を物理的に拒否し、application 層が「先に全 Room を archive 又は別 Workflow に移行」する経路を強制（Fail Fast） |
...
3. `docs/design/domain-model/storage.md` §逆引き表 を更新（後述 R1-G）
...
#### 確定 R1-F: `find_by_name(empire_id, name)` 第 4 method 追加 + INDEX 設計

empire-repo の Protocol は 3 method（find_by_id / count / save）、agent-repo で第 4 method `find_by_name` を追加。本 PR では **agent-repo §R1-C パターンを継承** して `find_by_name(empire_id, name)` を第 4 method として追加する:
...
| | INDEX なし | `find_by_name` の `WHERE empire_id=? AND name=?` がフルテーブルスキャンになり、Empire 内 Room 数が増えると O(N) で遅くなる。左端 prefix で empire_id を含む複合 INDEX により `find_by_name` の SELECT を効率化（[SQLite — Query Planner](https://www.sqlite.org/queryplanner.html) 左端プリフィックスの原則）|
| | 別々の INDEX(empire_id) と INDEX(name) | 複合 INDEX `(empire_id, name)` の方が `find_by_name` の `WHERE empire_id=? AND name=?` に完全一致で効率的。別々 INDEX は Planner が組み合わせる分のオーバーヘッドがある |

| INDEX | 効く SELECT |
|---|---|
| `INDEX(empire_id, name)` 非 UNIQUE | `WHERE empire_id=? AND name=?`（find_by_name） |

##### 後続 Repository PR への申し送り（テンプレート責務）

本 §確定 R1-F は agent-repo §R1-C の直接継承。「Empire 内一意 Aggregate に `find_by_name(empire_id, name)` を Protocol 第 4 method として追加し、`INDEX(empire_id, name)` 非 UNIQUE で SELECT を効率化する」パターンを確立する。後続 PR が同パターンを採用する場合:
...
| `feature/task-repository`（Issue #35）| `find_blocked() -> list[Task]`（admin CLI `list-blocked`） |
| `feature/external-review-gate-repository`（Issue #36）| `find_pending_by_reviewer(reviewer_id)` / `find_by_task_id(task_id)` |

#### 確定 R1-G: storage.md 逆引き表更新

`docs/design/domain-model/storage.md` §逆引き表に Room 関連 2 行追加:
...
4. **永続化**: `RoomRepository.save(room, empire_id)` → SQLite に書き込み（`prompt_kit_prefix_markdown` は `MaskedText` 経由で masking 適用、CEO が PromptKit に webhook URL を含めても `<REDACTED:DISCORD_WEBHOOK>` 化）
...
| 区分 | 要求 |
|-----|------|
| パフォーマンス | `count()` は O(1) SQL `COUNT(*)`。`find_by_id` は子テーブル含めて O(1) Tx で 2 SELECT。`find_by_name` は `INDEX(empire_id, name)` 経由で O(log N) |
...
| セキュリティ | `rooms.prompt_kit_prefix_markdown` の Discord webhook URL / API key を `MaskedText` で永続化前マスキング（room §確定 G 実適用）。CI 三層防衛で物理保証。`workflow_id` FK RESTRICT で Workflow 削除を物理拒否、参照整合性保証。詳細は [`threat-model.md`](../../design/threat-model.md) §A02 / §A04 / §A08 / §A09 |

## 受入基準

| # | 基準 | 検証方法 |
|---|------|---------|
| 1 | `RoomRepository` Protocol が **4 method**（find_by_id / count / `save(room, empire_id)` / find_by_name）を `async def` で定義、`@runtime_checkable` なし | TC-UT-RR-001 |
| 2 | `SqliteRoomRepository` が Protocol を型レベルで満たす（pyright strict） | CI typecheck |
| 3 | `save(room, empire_id)` が rooms UPSERT + room_members DELETE/INSERT を同一 Tx 内で実行 | TC-UT-RR-002 |
...
| 8 | `rooms.prompt_kit_prefix_markdown` が `MaskedText` で宣言、raw webhook URL を保存しても DB には `<REDACTED:DISCORD_WEBHOOK>` で永続化（room §確定 G 実適用） | TC-IT-RR-008-masking |
| 9 | `room_members` の `UNIQUE(room_id, agent_id, role)` が DB レベルで一意性を保証（同 (agent_id, role) ペアが 2 件あったら IntegrityError） | TC-IT-RR-009 |
| 10 | `rooms.workflow_id` の FK が `workflows.id` への ON DELETE RESTRICT で宣言、Workflow 削除時に Room が残っていれば IntegrityError | TC-IT-RR-010 |
| 11 | `rooms.empire_id` の FK が `empires.id` への ON DELETE CASCADE で宣言、Empire 削除で Room も削除される | TC-IT-RR-012 |
...
| 14 | Alembic 0005 revision で 2 テーブル + UNIQUE + INDEX + workflow_id FK + empire_room_refs FK closure が SQLite に作成される | TC-IT-RR-012 |
