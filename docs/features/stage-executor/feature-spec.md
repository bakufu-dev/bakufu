...
各エラー種別に対応する **実装クラス名**（`LLMProviderAuthError` 等）および既存クラスとのマッピングは [`application/detailed-design.md §確定 H`](application/detailed-design.md) で凍結する。本書では業務記述レベルの種別名（SessionLost / AuthExpired 等）のみを使用する。

**理由**: tech-stack.md §LLM Adapter 運用方針で確定済みのリトライ戦略を業務ルールとして参照する。実装クラス名は設計書の責務分離原則に従い detailed-design に委ねる。
