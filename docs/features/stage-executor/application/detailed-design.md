...
| メソッド | シグネチャ（概要）| 用途 | 状態 |
|---------|---------------|------|------|
| `chat(messages, system, session_id)` | 既存シグネチャを維持 | WORK Stage の LLM 実行 | M5-A 定義済み。変更なし |
| `chat_with_tools(messages, tools, system, session_id)` | `tools` パラメータ（tool schema リスト）を追加 | GateRole 審査の `submit_verdict` ツール呼び出し（M5-B）| M5-B で新規追加 |

**後方互換保証**: `chat()` の既存シグネチャ・呼び出し元・テストは変更しない。`chat_with_tools()` は Protocol の新 method として追加するのみ（既存実装クラスに `NotImplementedError` の default 実装を追加することで後方互換を保つ）。
...
**根拠**: `LLMProviderPort` は M5-A で定義した凍結契約だが、M5-B が新機能（ツール呼び出し）を必要とするため Boy Scout Rule に従い同一 PR で設計書を更新する（ヘルスバーグ指摘 §却下4）。
