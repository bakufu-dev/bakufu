...
**ふるまい**:
- `execute(task_id, stage_id, required_gate_roles) -> None`: InternalReviewGate の実行を開始する。完了・失敗は M5-B の実装が Task 状態を操作する（非同期コールバック方式）。詳細シグネチャは M5-B 設計時に確定（feature-spec.md Q-OPEN-1）
...
### なぜ INTERNAL_REVIEW Semaphore の release を M5-B に委ねるのか

InternalReviewGate は並列 Agent 審査であり、全 Agent が判定を完了するまで StageWorker の concurrency slot を占有すべきである。M5-B の実装が全 GateRole の判定完了を検知した時点で Semaphore を release することで、並行数の意味が「同時に実行中の Stage 数」と一致する。詳細な委譲方式（コールバック / asyncio.Event）は M5-B 設計時に確定（feature-spec.md Q-OPEN-1）。
