...
4. **TaskStatus 6 種 + 8 遷移**を Aggregate 内で正しく扱える state machine の表現方式が未確定。state machine が複雑なため「ふるまい毎に if 分岐を書く」実装になると遷移ルールが散逸し、後続 PR で「ある経路だけ遷移検査が抜けている」という退行が発生しやすい
...
#### 確定 R1-A: state machine は **enum-based decision table** で凍結

`TaskStatus` 6 種 + 8 遷移のうち実際に許可されるものを `state_machine.py` の dict / Mapping で**明示列挙**する。具体的には:
...
| `(IN_PROGRESS, 'advance_to_done')` | `DONE`（terminal） |
| `(IN_PROGRESS, 'block')` | `BLOCKED` |
| `(BLOCKED, 'unblock_retry')` | `IN_PROGRESS` |
| `(BLOCKED, 'cancel')` | `CANCELLED`（terminal） |
| `(*, 'cancel')` | `CANCELLED`（PENDING / IN_PROGRESS / AWAITING_EXTERNAL_REVIEW / BLOCKED 任意時点、terminal） |
| `(DONE, *)` | **エラー**（terminal、任意の遷移を Fail Fast、§確定 R1-B） |
| `(CANCELLED, *)` | **エラー**（terminal、同上） |

「(*, 'cancel')」は便宜上の表記で、実装上は PENDING / IN_PROGRESS / AWAITING_EXTERNAL_REVIEW / BLOCKED の 4 状態 × 'cancel' の 4 行を decision table に明示列挙する。DONE / CANCELLED を起点とする遷移は **table に存在しない** ことで明示禁止（lookup 失敗 → `TaskInvariantViolation(kind='terminal_violation')`）。

理由:

- 8 遷移を表で読めるため設計書 / 実装 / テストの 3 箇所で同じ表を写経でき、退行検出が容易
- 後続 `feature/external-review-gate` の 4 種 state machine、`feature/agent` の archive 遷移などで同パターンが流用できる
- room §確定 I の例外型統一規約（`*InvariantViolation` の `kind` で違反種別を識別）と相性が良い

#### 確定 R1-B: `status == DONE` / `CANCELLED` は terminal、**全ふるまいで先頭 Fail Fast**

`assign` / `commit_deliverable` / `request_external_review` / `advance` / `cancel` / `block` / `unblock_retry` の **すべてのふるまい入口**で:
...
6. **承認 / 差し戻し**: CEO が Gate を `approve()` → Gate APPROVED イベント → `task.advance(transition_id, by_owner_id)` → 次 Stage へ。`reject()` の場合は差し戻し先 Stage へ
7. **DONE**: 終端 Stage + APPROVED → `status=IN_PROGRESS → DONE`（terminal、以後変更不可）
...
- ジャーニー 6: `advance(transition_id, by_owner_id)` で AWAITING_EXTERNAL_REVIEW → IN_PROGRESS、`current_stage_id` 更新
- ジャーニー 7: 終端 Stage で `advance` 呼び出し → DONE 遷移、以後の `assign` / `commit_deliverable` / `advance` / `cancel` / `block` / `unblock_retry` 全 7 ふるまいが Fail Fast（terminal）
...
| REQ-TS-005 | Stage 進行 | `advance(transition_id, by_owner_id)` で次 Stage へ遷移、終端 Stage で DONE | 必須 |
...
| 5 | `status == DONE` の Task に対する全 7 ふるまい呼び出しが `TaskInvariantViolation(kind='terminal_violation')` | TC-UT-TS-005 |
| 6 | `status == CANCELLED` の Task に対する全 7 ふるまい呼び出しが同上 | TC-UT-TS-006 |
