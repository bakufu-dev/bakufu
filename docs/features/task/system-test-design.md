...
| TC-E2E-TS-003 | HTTP API 経由 Task lifecycle（受入基準 18〜23 複合） | 1) GET /api/tasks/{task_id}（Directive 経由で起票済み Task）→ 200 TaskResponse（last_error=null / status=PENDING）2) GET /api/rooms/{room_id}/tasks → 200 3) POST /api/tasks/{task_id}/assign — agent_ids=[agent_id] → 200（status=IN_PROGRESS）4) POST /api/tasks/{task_id}/deliverables/{stage_id} — secret を含む body_markdown → 200 TaskResponse（body_markdown masked）5) GET /api/tasks/{task_id} → status=IN_PROGRESS、deliverables[stage_id].body_markdown masked 6) PATCH /api/tasks/{task_id}/cancel → 200（status=CANCELLED）7) PATCH /api/tasks/{task_id}/cancel（再試行）→ 409 8) GET /api/tasks/{not_exist_id} → 404 | GET 200 + last_error null / list 200 / assign 後 IN_PROGRESS / deliverable commit 後 body_markdown masked / cancel 200 + CANCELLED / 再 cancel 409 / 不在 404 | 18, 19, 20, 22, 23 |
...
| TC-E2E-TS-005 | Room.members 外 assign 拒否 | 1) Task の Room に所属しない Agent を用意 2) POST /api/tasks/{task_id}/assign — agent_ids=[outsider_agent_id] | HTTP 403 | 19 |
| TC-E2E-TS-006 | 未担当 Agent の成果物提出拒否 | 1) Room member だが Task 未担当の Agent を用意 2) POST /api/tasks/{task_id}/deliverables/{stage_id} — submitted_by=unassigned_agent_id | HTTP 403 | 22 |

将来追加予定:

- TC-E2E-TS-007: UI 経由での Task 進行 + External Review 承認（`task/ui/` 完成後、Playwright）
...
| 永続化層 | 実 SQLite（テスト用 in-memory または tempfile） |
| domain 層 | 実 Task / Deliverable / Attachment Aggregate |
| application 層 | 直接呼び出し（test harness）または HTTP API 経由（TC-E2E-TS-003〜006）|
| HTTP API 層（TC-E2E-TS-003〜006）| FastAPI TestClient（`httpx.AsyncClient` + `ASGITransport`）— 実 DB 使用 |
| 外部 LLM / Discord / GitHub | 本 E2E では未使用（task 業務概念に外部 I/O なし）|
...
- 受入基準 18〜23 が TC-E2E-TS-003〜006 として **E2E で最低 1 件** ずつ検証される
...
└── test_directive_task_http_api.py # TC-E2E-TS-003〜006（Issue #60 実装済み）
```

## 未決課題

- TC-E2E-TS-003〜006 は http-api sub-feature（Issue #60）で実装済み
- TC-E2E-TS-007（UI Playwright）は将来の `task/ui/` sub-feature 追加時に本書を更新する別 PR で起票
