...
| REQ-TS-005（advance） | `Task.advance` 終端でない | TC-UT-TS-032 | ユニット | 正常系 | （AWAITING → IN_PROGRESS） |
| REQ-TS-005（advance to DONE） | `Task.advance(is_terminal=True)` | TC-UT-TS-033 | ユニット | 正常系 | （IN_PROGRESS → DONE） |
...
| REQ-TS-009（terminal） | DONE / CANCELLED から全 7 ふるまい | TC-UT-TS-005, TC-UT-TS-006 | ユニット | 異常系 | 5, 6 |
...
- **DONE / CANCELLED terminal 物理確認**: TC-UT-TS-005（DONE 起点で 7 ふるまい全部 raise）/ TC-UT-TS-006（CANCELLED 起点で同上）の 2 × 7 = 14 経路すべて
...
| TC-IT-TS-002 | Task lifecycle 完走（PENDING → IN_PROGRESS → AWAITING_EXTERNAL_REVIEW → IN_PROGRESS → DONE） | factory + `DeliverableFactory` | task_id=t_1, status=PENDING の Task | 1) `assign([agent_a])` → IN_PROGRESS → 2) `commit_deliverable(stage_a, deliverable, agent_a)` → 3) `request_external_review()` → AWAITING → 4) `advance(transition_id, owner_id, next_stage_id, is_terminal=False)` → IN_PROGRESS（次 Stage） → 5) `commit_deliverable` → 6) `advance(..., is_terminal=True)` → DONE → 7) DONE 状態で全 7 ふるまい呼び出しが MSG-TS-001 で raise | 5 段階の lifecycle が完走、最終 status=DONE、deliverables 2 件、DONE が terminal で全変更不可 |
| TC-IT-TS-003 | BLOCKED 経路と復旧（IN_PROGRESS → BLOCKED → IN_PROGRESS → DONE） | factory + webhook URL を含む last_error 文字列 | task_id=t_1, status=IN_PROGRESS の Task | 1) `block(reason, last_error='AuthExpired: https://discord.com/api/webhooks/123/secret')` → BLOCKED → 2) Task の `last_error` が NFC 正規化済みで保持されることを確認（auto-mask は永続化前のため Aggregate 内では raw 保持）→ 3) `unblock_retry()` → IN_PROGRESS → 4) `last_error=None` であることを確認（確定 D）→ 5) `commit_deliverable` → `advance(is_terminal=True)` → DONE | BLOCKED 隔離 → 復旧 → DONE 完走、確定 D の last_error クリア物理確認 |
...
### Task 構築（test_construction.py、受入基準 1, 2, 14, 15）
...
| TC-UT-TS-004 | state machine 不正遷移（一覧網羅） | 異常系 | 6 status × 7 ふるまい = 42 経路のうち 13 許可遷移を除く 29 経路 | 各々で `TaskInvariantViolation(kind='state_transition_invalid')`、MSG-TS-002 |
| TC-UT-TS-005 | DONE terminal 全 7 ふるまい raise（受入基準 5） | 異常系 | DoneTaskFactory + 各ふるまい呼び出し | 全 7 ふるまいが `TaskInvariantViolation(kind='terminal_violation')`、MSG-TS-001 |
| TC-UT-TS-006 | CANCELLED terminal 全 7 ふるまい raise（受入基準 6） | 異常系 | CancelledTaskFactory + 各ふるまい呼び出し | 同上 |
...
| TC-UT-TS-032 | `advance` 終端でない（AWAITING → IN_PROGRESS、gate_approved/rejected）| 正常系 | AwaitingReviewTaskFactory + transition_id + next_stage_id + is_terminal=False | 新 Task の status=IN_PROGRESS、current_stage_id=next_stage_id |
| TC-UT-TS-033 | `advance` 終端（IN_PROGRESS → DONE、advance_to_done）| 正常系 | IN_PROGRESS の Task + is_terminal=True | 新 Task の status=DONE（terminal） |
...
- **DONE / CANCELLED terminal 物理確認**: TC-UT-TS-005（DONE × 7 ふるまい）/ TC-UT-TS-006（CANCELLED × 7 ふるまい）の 14 経路すべて
