...
- `current_stage_id` の Workflow 内存在 — `TaskService.advance()` 系が `WorkflowRepository.find_by_id` で確認
- `assigned_agent_ids` の各 AgentId が Room.members 内 — 同上、`RoomRepository.find_by_id` で確認
- `transition_id` の Workflow 内存在 — `TaskService.advance()` で確認
...
**ふるまい**（全 7 種、すべて新インスタンス返却）:
- `assign(agent_ids: list[AgentId]) -> Task`: PENDING → IN_PROGRESS、`assigned_agent_ids` 更新
- `commit_deliverable(stage_id: StageId, deliverable: Deliverable, by_agent_id: AgentId) -> Task`: IN_PROGRESS の状態で `deliverables[stage_id] = deliverable` を更新
- `request_external_review() -> Task`: IN_PROGRESS → AWAITING_EXTERNAL_REVIEW
- `advance(transition_id: TransitionId, by_owner_id: OwnerId, next_stage_id: StageId, is_terminal: bool) -> Task`: AWAITING_EXTERNAL_REVIEW → IN_PROGRESS（次 Stage へ）/ IN_PROGRESS → DONE（is_terminal=True）
...
state machine の決定表（§確定 R1-A）。**`Mapping[tuple[TaskStatus, str], TaskStatus]`** として module-level 定数で凍結。
...
| `(IN_PROGRESS, 'advance_to_done')` | `DONE` |
| `(IN_PROGRESS, 'block')` | `BLOCKED` |
| `(IN_PROGRESS, 'cancel')` | `CANCELLED` |
| `(AWAITING_EXTERNAL_REVIEW, 'gate_approved')` | `IN_PROGRESS` |
| `(AWAITING_EXTERNAL_REVIEW, 'gate_rejected')` | `IN_PROGRESS` |
...
`action` は `Literal['assign', 'commit_deliverable', 'request_external_review', 'advance_to_next', 'advance_to_done', 'block', 'unblock_retry', 'cancel', 'gate_approved', 'gate_rejected']` で型レベル制約。
...
2. state machine table lookup（lookup 失敗 → `state_transition_invalid` で raise）
...
| `current_stage_id` の Workflow 内存在 | ✗ | ✓（`TaskService.advance()` で `WorkflowRepository.find_by_id`） |
| `assigned_agent_ids` 各 AgentId が Room.members 内 | ✗ | ✓（`TaskService.assign()` で `RoomRepository.find_by_id`） |
| `transition_id` の Workflow 内存在 | ✗ | ✓（同上） |
...
- 6 status × 7 ふるまい = **42 経路** のうち実際に許可されるのは **13 遷移** のみ
- 残り 29 経路は「許可されない」を if 分岐で書くと暗黙の拒否経路になり、後続 PR で「`unblock_retry` を IN_PROGRESS でも呼べる」等の誤実装が紛れ込む退行リスクが高い
...
### なぜ state machine の `commit_deliverable` を自己遷移として明示列挙するか

`commit_deliverable` は `IN_PROGRESS` のままで `deliverables` を更新するふるまい。state を変えないなら state machine table に載せる必要がないように見えるが、**「terminal でないこと + IN_PROGRESS のみで許可されること」を表明**するために明示列挙する。

`(IN_PROGRESS, 'commit_deliverable') -> IN_PROGRESS` を table に書くことで:
1. PENDING / AWAITING_EXTERNAL_REVIEW / BLOCKED / DONE / CANCELLED で `commit_deliverable` を呼ぶと lookup 失敗 → `state_transition_invalid`
2. 「commit_deliverable は IN_PROGRESS でのみ可能」を表で読める

`advance_to_next` も同様の自己遷移として明示列挙（IN_PROGRESS → IN_PROGRESS、current_stage_id 更新のみ）。
