...
| 確定E | `TaskService` のコンストラクタ | `__init__(self, task_repo: TaskRepository, session: AsyncSession) -> None`（Room 参照整合性は task の business 操作では不要。Room 存在確認は Directive 発行時に `DirectiveService` が担う）|
...
| `last_error` | `str \| None` | `@field_serializer('last_error')` で `mask(value)` 適用。BLOCKED 以外では None |
...
| `body_markdown` | `str` | `@field_serializer('body_markdown')` で `mask(value)` 適用（確定A）|
...
| 関数名 | シグネチャ概要 | 責務 |
|-------|-------------|------|
| `get_task_service` | `(session=Depends(get_session)) -> TaskService` | `TaskRepository`（SQLite 実装）/ `session` を注入して `TaskService` を生成する |
...
| `assign(task_id, agent_ids)` | `find_by_id` → `task.assign([AgentId(a) for a in agent_ids])` → `TaskInvariantViolation` を kind 別に処理 → `session.begin()` 内で `task_repo.save(updated_task)` |
...
| `commit_deliverable(task_id, stage_id, deliverable_create)` | `find_by_id` → `Deliverable(...)` 構築 → `task.commit_deliverable(stage_id, deliverable, submitted_by)` → `task_repo.save` |
