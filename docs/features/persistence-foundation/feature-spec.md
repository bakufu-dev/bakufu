...
| 2 | `BAKUFU_DATA_DIR` 相対パス指定時に起動 Fail Fast（`ValueError`） | UC-PF-005 | TC-UT-PF-002 |
...
| 5 | `audit_log` への DELETE が SQLite トリガで `RAISE(ABORT)` される | UC-PF-002 | TC-IT-PF-005 |
...
| 13 | `domain` 層から `bakufu.infrastructure.*` への import がゼロ件（CI で検査） | UC-PF-002 | TC-CI-PF-001 |
