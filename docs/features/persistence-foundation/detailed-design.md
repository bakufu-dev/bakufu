...
| [`detailed-design/triggers.md`](detailed-design/triggers.md) | event listener + SQLite トリガ | 確定 B（listener 登録方式）+ 確定 C（`audit_log` 不変性トリガ） |
...
classDiagram
    ...
    SqliteEngine --> MaskingGateway : (event listener wiring)
...
| 確定 B | SQLAlchemy event listener の登録方式（table モジュール内 module-level、デコレータ） | [`triggers.md`](detailed-design/triggers.md) |
