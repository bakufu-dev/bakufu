...
### なぜ `StageExecutorService.retry_blocked_task()` を使わないのか（ジェンセン再決定）

**背景（透明性のための明示）**: M5-C 設計当初、Bill Gates が `Task.unblock_retry()` 直接呼び出し方式を技術的判断として採用した。ヘルスバーグのレビュー（PR #174 却下 Critical 1）で「設計者が変更した判断をジェンセン決定として偽装している」という透明性違反が指摘された。ジェンセンが技術判断を確認・再承認し、本 §で**ジェンセン再決定**として明示凍結する。

**技術的根拠（変更なし）**: `StageExecutorService.retry_blocked_task()` は `enqueue_fn: Callable` 経由でサーバーの `asyncio.Queue` に Task を再投入する in-process 専用の実装である。admin-cli は bakufu サーバーとは独立した別プロセスであり、サーバーの asyncio Queue インスタンスへのアクセス手段が存在しない。したがって `StageExecutorService` を admin-cli から呼び出すことは物理的に不可能である。

**採用方式（ジェンセン再決定、2026-05-02）**: AdminService は `Task.unblock_retry()`（domain メソッド）を直接呼んで BLOCKED → IN_PROGRESS に変更し、DB に保存して終了する。CLI プロセスは LLM 実行責務を持たない。LLM 実行の再開は、StageWorker の**起動時リカバリスキャン**（Q-OPEN-2 解決策）によって保証される。bakufu サーバーが起動または再起動した際に、StageWorker が IN_PROGRESS 状態の孤立 Task を DB スキャンしてキューに再投入することで、end-to-end での実行継続が成立する。詳細設計は `docs/features/stage-executor/application/detailed-design.md §確定J` に凍結。

**Q-OPEN-2 解決（2026-05-02 ジェンセン決定）**: Option A（起動時リカバリスキャン + サーバー再起動）を M5-C スコープで採用。`retry-task` → DB 更新（CLI プロセス） → サーバー再起動 → StageWorker 起動時スキャン → キュー投入 → LLM 実行再開 という全体フローで end-to-end が完結する。`feature-spec.md §11 Q-OPEN-2` は本 PR で解決済みとしてクローズする。
