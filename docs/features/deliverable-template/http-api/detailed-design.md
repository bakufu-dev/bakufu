...
classDiagram
    class DeliverableTemplateService {
        ...
        -_check_dag(refs: tuple~DeliverableTemplateRef~, root_id: DeliverableTemplateId, depth: int, visited: set) None
    }
...
**ふるまい**:
- `create(...)`: uuid4() で id を生成し `model_validate` で Aggregate 構築 → `_check_dag` → `save`
- `find_by_id(id)`: `_dt_repo.find_by_id(id)` → None なら `DeliverableTemplateNotFoundError(template_id=str(id))` raise
- `update(id, ...)`: find_by_id → version 降格チェック（§確定 B）→ version 超なら `create_new_version` 経由 → `model_validate` で全フィールド再構築 → `_check_dag` → `save`
- `delete(id)`: find_by_id（不在なら 404 相当）→ `_dt_repo.delete(id)`
- `_check_dag(refs, root_id, depth, visited)`: BFS で refs を走査し `visited` セットに追加。depth が §確定 D の上限超で `CompositionCycleError` raise。各 ref の `template_id` が `root_id` と一致するか `visited` に含まれる場合も `CompositionCycleError`
...
| クラス | 属性 | 意図 |
|-------|------|------|
| `DeliverableTemplateNotFoundError` | `message: str` / `template_id: str \| None` | find_by_id が None の時 / ref 存在確認失敗時に raise |
| `RoleProfileNotFoundError` | `message: str` / `empire_id: str` / `role: str` | find_by_empire_and_role が None の時に raise |
| `CompositionCycleError` | `message: str` / `cycle_path: list[str]` | DAG 走査で推移的循環を検出した時に raise。cycle_path は循環を示す template_id の列 |
...
DAG 走査の **最大深度は 10**、**最大訪問ノード数は 100** とする。これらの上限を超えた場合は `CompositionCycleError` を raise する（`cycle_path=["depth_limit_exceeded"]` または `["node_limit_exceeded"]`）。

根拠: 悪意ある DoS（T5 脅威）の緩和。実用的なテンプレート合成の深度・規模はこれらの上限に達しない（feature-spec §14 に「composition 深さ ≤ 5」の性能目標あり）。
...
全エラーは http-api-foundation の `ErrorResponse` フォーマット（`{"error": {"code": str, "message": str, "detail": dict | None}}`）を使用する。`code` は snake_case の短縮名（例: `not_found` / `composition_cycle` / `version_downgrade`）。`detail` には `template_id` / `cycle_path` 等の非機密情報のみを含める。スタックトレースは絶対に含めない（A09 対策）。
...
| MSG-DT-HTTP-003 | `composition_cycle` | JSON レスポンス body | `[FAIL] composition に推移的な循環参照が検出されました（{cycle_path}）。` / `Next: 循環を引き起こす DeliverableTemplateRef を composition から除去してください。` |
...
| 失敗レスポンス | `404`（MSG-DT-HTTP-001）/ `422`（version 降格 MSG-DT-HTTP-004 / DAG 循環 MSG-DT-HTTP-003 / ref 不在 MSG-DT-HTTP-002 / 不変条件違反）|
...
`DeliverableTemplateResponse.schema` の型は `dict | str`。type=JSON_SCHEMA/OPENAPI の場合は dict 形式で返却し、それ以外は str 形式で返却する。FastAPI の `response_model` は `Union[dict, str]` として扱い、Pydantic v2 の `model_config` に `arbitrary_types_allowed=True` が不要なことを確認する。リクエスト側も同様に type の値に応じて dict / str を受け付ける（§確定 D の repository sub-feature との整合）。

根拠: domain `DeliverableTemplate.schema: dict | str` の型をそのまま HTTP レスポンスに反映する（無駄な変換なし、DRY）。
