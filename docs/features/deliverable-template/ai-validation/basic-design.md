...
│   │       ├── application/
│   │       │   ├── ports/
│   │       │   │   └── llm_provider_port.py                # LLMProviderPort (llm-client feature 提供)
│   │       │   └── services/
...
classDiagram
    class ValidationService {
        ...
        -_build_prompt(content, criterion) tuple~list~dict~, str~
    ...
    ValidationService --> LLMProviderPort : uses（DI）
...
2. `ValidationService` が各 `AcceptanceCriterion` について `_build_prompt(content, criterion)` で `tuple[list[dict], str]`（messages, system）を構築
3. `await self._llm_provider.chat(messages=messages, system=system)` を呼び `ChatResult` を取得
4. `_parse_response(result.response)` で `(ValidationStatus, reason: str)` を抽出し `CriterionValidationResult` を収集
...
1. `self._llm_provider.chat()` が `LLMProviderError` サブクラスを raise（`LLMProviderTimeoutError` / `LLMProviderAuthError` / `LLMProviderProcessError` / `LLMProviderEmptyResponseError`）
...
1. `self._llm_provider.chat()` は成功するが `result.response` が期待 JSON 形式でない
...
sequenceDiagram
    ...
    loop each AcceptanceCriterion
        VS->>VS: _build_prompt(content, criterion) → (messages, system)
        VS->>LLM: chat(messages=[...], system=..., session_id=None)
        LLM-->>VS: ChatResult(response=..., session_id=None, compacted=False)
        VS->>VS: _parse_response(result.response) → CriterionValidationResult
    end
...
| A04 | Insecure Design | **適用**: Fail Fast（LLMProviderError は即 LLMValidationError に変換）/ domain port パターン（依存逆転）/ UNCERTAIN/FAILED の escalation 責務分離（D-3 確定）|
...
|---------|------|---------|----------------|
| `LLMProviderError` サブクラス（`LLMProviderTimeoutError` / `LLMProviderAuthError` / `LLMProviderProcessError` / `LLMProviderEmptyResponseError`）| `LLMProviderPort.chat()` | `ValidationService` で catch → `LLMValidationError(kind='llm_call_failed', llm_error_kind=<サブクラス識別>, provider=exc.provider)` に変換して raise | MSG-AIVM-001 |
| JSON パース失敗（`result.response` が期待 JSON 形式でない）| `ValidationService._parse_response()` | `LLMValidationError(kind='parse_failed', llm_error_kind='', provider=<現在のプロバイダ>)` を raise | MSG-AIVM-002 |
