...
| REQ-AIVM-002 | LLMValidationAdapter（AbstractLLMValidationPort 実装・構造化プロンプト・LLM 応答パース）| `content: str`（DeliverableRecord.content）/ `criterion: AcceptanceCriterion` | ① 構造化プロンプト（System / User / Criterion の役割分離）を構築 ② `BAKUFU_LLM_VALIDATION_PROVIDER` で指定した LLM API にリクエスト（HTTPS / Bearer Token）③ LLM 応答を `status: ValidationStatus` + `reason: str` として JSON パース ④ `CriterionValidationResult(criterion_id=criterion.id, status=status, reason=reason)` を返す | `CriterionValidationResult` | HTTP / API エラー: `LLMValidationError`（MSG-AIVM-001）/ JSON パース失敗: `LLMValidationError`（MSG-AIVM-002）| §9 AC#16, 17 |
...
| REQ-AIVM-002 | `LLMValidationAdapter` | `backend/src/bakufu/infrastructure/llm_validation/adapter.py`（新規）| `AbstractLLMValidationPort` の concrete 実装。構造化プロンプト構築・LLM API 呼び出し・応答 JSON パース |
| REQ-AIVM-002 | `LLMValidationConfig` | `backend/src/bakufu/infrastructure/llm_validation/config.py`（新規）| 環境変数（`BAKUFU_LLM_VALIDATION_PROVIDER` / `BAKUFU_LLM_VALIDATION_MODEL` / `BAKUFU_LLM_VALIDATION_API_KEY`）を Pydantic Settings で読み込む |
...
| REQ-AIVM-001〜002 | `LLMValidationError` 例外 | `backend/src/bakufu/domain/exceptions/deliverable_template.py`（既存ファイル更新）| LLM API 呼び出し失敗 / 応答パース失敗の共通例外型（2 行エラー構造、兄弟例外と同パターン）|
...
│   │           │   └── config.py                            # LLMValidationConfig (新規)
│   │           ├── orm/
...
classDiagram
    ...
    LLMValidationAdapter --> LLMValidationConfig : reads
...
1. `LLMValidationAdapter.evaluate` が LLM API から HTTP エラーまたはタイムアウトを受ける
2. `LLMValidationError(kind='llm_call_failed')` を即 raise（握り潰し禁止、**§確定 F**）
3. `DeliverableRecord.validate_criteria` がキャッチせずに伝播
4. `ValidationService` が catch → ログ出力（API Key はマスク）→ `LLMValidationError` を再 raise
...
sequenceDiagram
    ...
    loop each AcceptanceCriterion
        ...
        Adapter-->>Adapter: LLM API HTTPS Request
        Adapter->>Adapter: JSON応答パース → CriterionValidationResult
...
| 環境変数 | `BAKUFU_LLM_VALIDATION_MODEL` | 必須 | LLM モデル名（例: `claude-opus-4-5` / `gpt-4o`）|
| 環境変数 | `BAKUFU_LLM_VALIDATION_API_KEY` | 必須 | LLM API キー（`SecretStr` でマスク）|
...
|-------|------|----------|-----|-------------------|
| Anthropic Claude API（`BAKUFU_LLM_VALIDATION_PROVIDER=anthropic` 時）| AcceptanceCriterion 単位の LLM 評価 | HTTPS | Bearer Token（`BAKUFU_LLM_VALIDATION_API_KEY`）| タイムアウト 30 秒 / リトライなし（Fail Fast）|
| OpenAI API（`BAKUFU_LLM_VALIDATION_PROVIDER=openai` 時）| 同上 | HTTPS | Bearer Token | タイムアウト 30 秒 / リトライなし |
...
| **T2: API Key 漏洩** | `BAKUFU_LLM_VALIDATION_API_KEY` のログ出力・例外 detail への混入 | LLM API クレデンシャル | `LLMValidationConfig.api_key` を `pydantic.SecretStr` として管理。`LLMValidationError.detail` に API Key を含めない。CI / ログ出力時の自動マスキング |
...
| A02 | Cryptographic Failures | **適用**: `BAKUFU_LLM_VALIDATION_API_KEY` は `SecretStr` 管理。ログ出力時の自動マスキング |
...
| A05 | Security Misconfiguration | **適用**: 起動時 `LLMValidationConfig` バリデーション（必須環境変数未設定を即検出）|
| A06 | Vulnerable and Outdated Components | `anthropic` SDK / `openai` SDK は `dev-workflow/audit` ジョブで横断管理 |
...
| A10 | SSRF | **適用**: SDK 固定エンドポイント使用。PROVIDER allowlist 制限（T4 参照）|
