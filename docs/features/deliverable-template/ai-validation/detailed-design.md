...
classDiagram
    ...
    class LLMValidationAdapter {
        ...
        -_build_prompt(content: str, criterion: AcceptanceCriterion) tuple~str, str~
...
| メソッド | 説明 |
|---------|------|
| `evaluate(content, criterion)` | `_build_prompt` → LLM API call → `_parse_response` → `CriterionValidationResult` 返却 |
| `_build_prompt(content, criterion)` | §確定 B の構造化プロンプトを構築。`(system_prompt, user_prompt)` のタプルを返す |
| `_parse_response(raw)` | LLM 応答 JSON から `status: ValidationStatus` + `reason: str` を抽出。失敗時に `LLMValidationError(kind='parse_failed')` raise |

**不変条件**:
- API call 失敗（HTTP エラー / タイムアウト）は即 `LLMValidationError(kind='llm_call_failed')` raise（握り潰し禁止、§確定 F: Fail Secure）
...
LLM API へのリクエストは以下の役割分離構造で構築する（`LLMValidationAdapter._build_prompt` が担当）:

| ブロック | ロール | 内容 | Injection 対策 |
|---------|-------|------|---------------|
| System prompt | `system` | 評価者としての役割指示 + 出力フォーマット指定（JSON: `{"status": "PASSED\|FAILED\|UNCERTAIN", "reason": "<str>"}` のみ）| ユーザー入力を含まない固定テキスト |
| Criterion block | `user`（先行）| `criterion.description`（評価基準の説明）+ `criterion.required`（必須フラグ）| content より前に配置。content は criterion の後の別スロット |
| Content block | `user`（後続）| `--- BEGIN CONTENT ---` / `{content}` / `--- END CONTENT ---` の delimiter で囲む | delimiter によるスコープ限定。content 内の命令テキストは delimiter 外に影響しない |
...
| MSG-AIVM-002 | `stderr` | `[FAIL] LLM validation response could not be parsed: expected JSON with 'status' and 'reason' fields.` / `Next: Check LLM model compatibility or update prompt structure in LLMValidationAdapter._build_prompt.` |

**注意**: `{provider}` には `BAKUFU_LLM_VALIDATION_PROVIDER` の値（`anthropic` / `openai`）を展開。`{model}` には `BAKUFU_LLM_VALIDATION_MODEL` を展開。`{error_type}` には例外クラス名のみ（スタックトレース・API Key は含めない、§確定 A09 ログ制約）。
