...
classDiagram
    class ValidationService {
        ...
        -_build_messages(content: str, criterion: AcceptanceCriterion) tuple~LLMMessage, LLMMessage~
    ...
    ValidationService --> AbstractLLMClient : DI
    ...
    LLMClientError <|-- LLMTimeoutError
    LLMClientError <|-- LLMRateLimitError
    LLMClientError <|-- LLMAuthError
    LLMClientError <|-- LLMAPIError
...
| 属性 | 型 | 制約 | 意図 |
|----|----|----|----|
| `_llm_client` | `AbstractLLMClient` | DI で注入、None 不可 | Protocol 差し替えを可能にする（DIP）。`ClaudeCodeLLMClient` / `CodexLLMClient` が concrete 実装 |
...
1. 各 `criterion` に対して `_build_messages(record.content, criterion)` を呼び `tuple[LLMMessage, LLMMessage]` を取得
2. `await self._llm_client.complete((system_msg, user_msg), max_tokens=512)` を呼び `LLMResponse` を取得
3. `_parse_response(response.content)` で `(ValidationStatus, reason)` を抽出し `CriterionValidationResult` を収集
...
`_build_messages(content, criterion) -> tuple[LLMMessage, LLMMessage]`: §確定 B の構造化プロンプトを構築。`(system_msg, user_msg)` のタプルを返す。

`_parse_response(raw: str) -> tuple[ValidationStatus, str]`: `raw` が空文字の場合は即 `LLMValidationError(kind='parse_failed')` raise。それ以外は `json.loads(raw)` → `status` / `reason` フィールド抽出。`json.JSONDecodeError` / キー不在の場合も `kind='parse_failed'` raise。
...
| `kind` | `Literal["llm_call_failed", "parse_failed"]` | 必須 | エラー種別。`llm_call_failed`: `LLMClientError` サブクラス起源 / `parse_failed`: JSON パース失敗起源 |
| `llm_error_kind` | `str` | 必須（`parse_failed` 時は `""`）| `LLMClientError` サブクラス識別。値域: `"timeout"` / `"rate_limit"` / `"auth"` / `"api_error"` / `"empty_response"` / `""` |
| `provider` | `str` | 必須 | `LLMClientError.provider` の値（`"claude-code"` / `"codex"`）。`parse_failed` 時は現在のプロバイダ名 |
...
`ValidationService._build_messages()` が構築する `tuple[LLMMessage, LLMMessage]` の構造:

| LLMMessage | role | 内容 | Injection 対策 |
|-----------|------|------|---------------|
| system_msg | `MessageRole.SYSTEM` | 評価者としての役割指示 + 出力フォーマット指定（JSON: `{"status": "PASSED\|FAILED\|UNCERTAIN", "reason": "<str>"}` のみ出力するよう指示）| ユーザー入力を含まない固定テキスト。実行時に変更不可 |
| user_msg | `MessageRole.USER` | `<criterion block>` + `--- BEGIN CONTENT ---` / `{content}` / `--- END CONTENT ---` | content を delimiter で囲みスコープを限定 |

`user_msg.content` の構造:
...
**凍結事項**:
- system_msg のテキストは実装 PR で確定（設計書では構造のみ凍結）
- Content delimiter は `--- BEGIN CONTENT ---` / `--- END CONTENT ---` 固定
- `AbstractLLMClient.complete()` に渡す `messages` は `(system_msg, user_msg)` の 2 要素 tuple
- LLM 応答の期待 JSON schema: `{"status": "PASSED" | "FAILED" | "UNCERTAIN", "reason": "<str, 0〜1000文字>"}`
- `max_tokens=512`（構造化 JSON 出力専用。reason 1000 文字 + JSON ラッパーで十分）

### 確定 C: AbstractLLMClient DI とプロバイダ管理

`ValidationService` は `AbstractLLMClient` を **DI で受け取るだけ** であり、プロバイダ選択・設定・認証を一切管理しない。

| 項目 | 旧設計（PR #147 クローズ済み）| 新設計 |
|-----|--------------------------|------|
| DI 構造 | 2段階（ValidationService → AbstractLLMValidationPort → AbstractLLMClient）| 1段階（ValidationService → AbstractLLMClient 直接）|
| LLM 呼び出し | `LLMValidationAdapter` が `anthropic` / `openai` SDK を直接使用 | `ClaudeCodeLLMClient` / `CodexLLMClient` が CLIサブプロセスで呼び出し |
...
**Fail Secure 規約**:
- `_llm_client` が None の場合は `LLMValidationError` を即 raise。評価をバイパスして PASSED を返すことを禁止
- `LLMClientError` サブクラスは即 `LLMValidationError(kind='llm_call_failed')` に変換（握り潰し禁止）
...
| 3 | `DELETE FROM criterion_validation_results WHERE deliverable_record_id=record.id` | 旧評価結果削除（FK 制約のため child 先に削除）|
| 4 | `DELETE FROM deliverable_records WHERE id=record.id` | 旧レコード削除 |
| 5 | `INSERT INTO deliverable_records VALUES (...)` | 新レコード挿入 |
| 6 | `INSERT INTO criterion_validation_results VALUES (...) * N` | 新評価結果挿入（N 件）|
| 7 | `COMMIT` | 確定 |

ロールバック: ステップ 2〜6 のいずれかで例外発生時は `ROLLBACK` → `RepositoryError` にラップして raise。
...
| MSG-AIVM-002 | `stderr` | `[FAIL] LLM validation response could not be parsed: expected JSON with 'status' and 'reason' fields.` / `Next: Check LLM model output format or update prompt structure in ValidationService._build_messages.` |

**注意**:
- `{provider}` には `LLMClientError.provider` の値（`"claude-code"` / `"codex"`）を展開
- `{error_type}` には例外クラス名のみ（`LLMTimeoutError` / `LLMRateLimitError` / `LLMAuthError` / `LLMAPIError` 等）を展開。スタックトレース・認証情報は含めない（§確定 A09 ログ制約）
...
旧設計の `ValidationService → AbstractLLMValidationPort → AbstractLLMClient` は、`LLMValidationAdapter` が infrastructure 実装と LLM API client の両方を担う過剰な間接層だった。新設計では `ValidationService` が `AbstractLLMClient` を直接 DI で受け取る。プロンプト構築・応答パースは `ValidationService` 内のプライベートメソッドとして実装することで、application service の凝集度を高め、テスト容易性も向上する。
...
- AbstractLLMClient Protocol（llm-client feature / Issue #144）: `backend/src/bakufu/application/ports/llm_client.py`
- LLMClientError 階層（llm-client feature）: `LLMClientError(message, provider)` / `LLMTimeoutError` / `LLMRateLimitError` / `LLMAuthError` / `LLMAPIError(status_code)`
- Claude Code CLI: `claude -p <prompt> --system-prompt <system> --model <model> --output-format stream-json --verbose --tools ""`
- Codex CLI: `codex exec --json --skip-git-repo-check --ephemeral --dangerously-bypass-approvals-and-sandbox <prompt>`
