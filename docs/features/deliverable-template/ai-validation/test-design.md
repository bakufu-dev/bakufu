...
| REQ-AIVM-001〜003（ValidationService + Repository 結合） | `ValidationService` + `SqliteDeliverableRecordRepository` | TC-IT-VS-001〜004 | 結合 | 正常系 / 異常系 | AC#16, 17 |
...
- **D-3 確定（Gate 非生成）**: TC-UT-VS-006 で ValidationService が ExternalReviewGate を生成しないことを物理確認（IT レベル確認: TC-IT-VS-003/004 で FAILED/PASSED 両パスを DB + Gate 非生成で結合確認）
- **AC#16/17 IT フルフロー（FAILED/PASSED 両パス）**: TC-IT-VS-003/004 で ValidationService + Repository 結合による FAILED/PASSED ステータスの DB 永続化と Gate 非生成を物理確認（旧 system-test-design.md TC-E2E-DT-010/011 を IT 降格）
...
| TC-IT-VS-003 | フルフロー（FAILED パス）— required=true FAILED → `validation_status==FAILED` DB 確認 + ValidationService が Gate を生成しないことを確認（AC#16/17 + R1-G）| 正常系 | DB 空（in-memory SQLite）。mock provider が `required=true` 基準 1 件に対して `ChatResult(response='{"status": "FAILED", "reason": "基準未達"}', session_id=None, compacted=False)` を返す。`criteria` に `required=true` AcceptanceCriterion を 1 件 | 1) `DeliverableRecordFactory.build(validation_status=PENDING)` で record 構築 → 2) `ValidationService.validate_deliverable(record, criteria)` を呼ぶ → 3) 返却 record の `validation_status` を確認 → 4) `repository.find_by_id(record.id)` で DB 保存を確認 → 5) ExternalReviewGate の生成がないことを確認（ValidationService は Gate を生成しない: R1-G） | (1) 返却 `DeliverableRecord.validation_status == FAILED` (2) `repository.find_by_id(record.id)` で取得した record も `FAILED` (3) ValidationService が ExternalReviewGate を生成しない（返却値が `DeliverableRecord` のみ、Gate 生成の副作用なし）|
| TC-IT-VS-004 | フルフロー（PASSED パス）— 全 required=true PASSED → `validation_status==PASSED` DB 確認 + Gate 非生成確認（AC#16/17 + R1-G）| 正常系 | DB 空（in-memory SQLite）。mock provider が `required=true` 基準 2 件全てに対して `ChatResult(response='{"status": "PASSED", "reason": "基準を満たす"}', session_id=None, compacted=False)` を返す。`criteria` に `required=true` AcceptanceCriterion を 2 件 | 1) `DeliverableRecordFactory.build(validation_status=PENDING)` で record 構築 → 2) `ValidationService.validate_deliverable(record, criteria)` を呼ぶ → 3) 返却 record の `validation_status` を確認 → 4) `repository.find_by_id(record.id)` で DB 保存を確認 → 5) ExternalReviewGate が生成されていないことを確認 | (1) 返却 `DeliverableRecord.validation_status == PASSED` (2) `repository.find_by_id(record.id)` で取得した record も `PASSED` (3) ValidationService が ExternalReviewGate を生成しない（R1-G: Gate 生成責務は呼び出し元が担う、ValidationService は関与しない）(4) PASSED 時は呼び出し元が Gate を不要と判断するべきであり、ValidationService 内部では Gate を生成しない |
