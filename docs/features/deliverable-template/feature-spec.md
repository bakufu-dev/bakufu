...
### 確定 R1-E: composition 合成による AcceptanceCriteria の union

composition により構成された DeliverableTemplate（複合テンプレート）は、全 composition メンバーの AcceptanceCriteria を union して新たな受入条件セットを形成する。

| 凍結項目 | 内容 |
|---|---|
| **(1) union の順序** | composition リストの順序に従い、先頭から順に AcceptanceCriteria を追加する。複合テンプレート自身が直接定義する acceptance_criteria は最後に追加される |
| **(2) id の一意性** | union 後の AcceptanceCriterion は id（UUID）で一意に識別される。同一 id が複数の composition メンバーから持ち込まれた場合は業務エラーとして拒否される |
| **(3) required の伝播** | required=true の AcceptanceCriterion は union 後も required=true を維持する。いずれかのメンバーで required=true であれば複合テンプレートの AcceptanceCriterion も required=true となる |
...
| 6 | composition に複数の有効な DeliverableTemplateRef を設定した複合テンプレートを作成できる。当該複合テンプレートの AcceptanceCriteria は composition メンバーの AcceptanceCriteria を union したものになる（業務ルール R1-E） | UC-DT-003 | TC-UT-DT-008, TC-UT-DT-009 |
| 7 | union 後の AcceptanceCriteria に同一 id（UUID）が複数存在する場合はエラーが返される（業務ルール R1-E (2)） | UC-DT-003 | TC-UT-DT-010 |
| 8 | composition メンバーに required=true の AcceptanceCriterion が含まれる場合、複合テンプレートの当該 AcceptanceCriterion も required=true になる（業務ルール R1-E (3)） | UC-DT-003 | TC-UT-DT-011 |
...
|---|------|-----------|----------------|
| OQ-1 | 推移的循環参照の検出をどの層（application 層 vs 専用 domain service）が担うか | 未決定。本書は「application 層が全グラフを展開して検証する」と暫定凍結（R1-B）。詳細は domain/`detailed-design.md` で確定 | domain（#115） |
...
| OQ-3 | ai-validation（Issue #123）の PASS / FAIL 判定結果を DeliverableTemplate / AcceptanceCriterion に書き込むか、別 Aggregate（DeliverableRecord 等）に保持するか | 未決定。ai-validation sub-feature の設計で確定する | ai-validation（#123） |
...
| DeliverableTemplate.schema | JSON Schema 定義 または ガイドライン文字列 | 中（組織内部の品質基準が含まれる場合あり。外部公開しない前提） |
| DeliverableTemplate.acceptance_criteria | AcceptanceCriterion のリスト（UUID + 自然言語 + required フラグ） | 中（同上） |
...
**注**: DeliverableTemplate.schema が SECRET や API キーを誤って含む可能性はゼロではないが、schema フィールドは CEO が意図的に記述する品質基準であり、secret の混入リスクは low と判断する。masking 対象の追加が必要になった場合は別 Issue で対応する。
