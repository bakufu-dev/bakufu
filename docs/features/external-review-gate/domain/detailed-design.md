...
| `required_deliverable_criteria` | `tuple[AcceptanceCriterion, ...]` | 0 件以上、不変（**§確定D' criteria_immutable**）| Gate 生成時に Stage.required_deliverables から引き込んだ AcceptanceCriterion snapshot（空タプル可）。`AcceptanceCriterion` は Issue #115 実体化済み VO（id: UUID / description: str 1〜500 / required: bool） |
...
**不変条件（model_validator(mode='after')）**: **6 種**
...
6. **`_validate_criteria_immutable`** — Gate 生成時の `required_deliverable_criteria` を以後変更しない（`_rebuild_with_state` で criteria 引数を渡さない契約。§確定 D' 参照）
...
| `kind` | `Literal['decision_already_decided', 'decided_at_inconsistent', 'snapshot_immutable', 'feedback_text_range', 'audit_trail_append_only', 'criteria_immutable']` | 違反種別 |
...
### 確定 D': `required_deliverable_criteria` 不変条件（§確定 R1-J 詳細）

`_validate_criteria_immutable` は **コンストラクタ後の `required_deliverable_criteria` 改変を禁止**:

| 入力経路 | 動作 |
|---|---|
| コンストラクタ（新規生成 or 復元）| 任意の `tuple[AcceptanceCriterion, ...]` を受け取る（空タプル可）、構築時には何もしない（criteria 設定そのもの）|
| `_rebuild_with_state` 内で `required_deliverable_criteria` を新値で上書き | 構造的にこの経路を作らない契約（`_rebuild_with_state` の引数に `required_deliverable_criteria` を含めない設計）|
| 万一の改変試行（直接代入は frozen で拒否、`model_validator` 経由でも検出）| `ExternalReviewGateInvariantViolation(kind='criteria_immutable')` で raise（MSG-GT-008）|

§確定 D（`deliverable_snapshot` 不変）と完全対称の構造。

##### `_rebuild_with_state` の契約（criteria を渡さない — §確定 D と合わせた更新版）
...
**全 4 method で `deliverable_snapshot` および `required_deliverable_criteria` を `_rebuild_with_state` に渡さない**ことで、snapshot / criteria 双方の不変性を構造的に保証。
...
| 構造的不変条件違反 | `ExternalReviewGateInvariantViolation` | `model_validator(mode='after')` | `decided_at_inconsistent` / `snapshot_immutable` / `feedback_text_range` / `audit_trail_append_only` / `criteria_immutable` |
...
| MSG-GT-008 | `ExternalReviewGateInvariantViolation(kind='criteria_immutable')` | `[FAIL] Gate required_deliverable_criteria is immutable after construction: gate_id={gate_id}` / `Next: required_deliverable_criteria is frozen at Gate creation; do not pass it to _rebuild_with_state. Issue a new Gate if criteria must change.` |
