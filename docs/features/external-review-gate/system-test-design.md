...
| TC-E2E-ERG-HTTP-003 | CEO | 二重決定拒否（HTTP 黒箱）| 1) PENDING Gate を DB に seed → 2) `POST /api/gates/{id}/approve` + `Authorization: Bearer <reviewer_id>` で APPROVED に遷移（HTTP 経由で前提状態を確立）→ 3) 同 Gate に再度 `POST /api/gates/{id}/approve`（同 reviewer）| HTTP 409 `conflict`（レスポンス body に `[FAIL]` + `Next:` の 2行構造、`decision_already_decided` 業務ルール R1-B 検証）|
