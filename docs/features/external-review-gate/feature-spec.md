...
| http-api | [`http-api/`](http-api/) | CEO が approve / reject / cancel を実行する HTTP エンドポイント（M3、Issue #61）|
...
| UC-ERG-006 | CEO | Gate の詳細を取得したとき、Stage に紐づく受入基準（required_deliverable_criteria）を一覧で確認できる | 必須 | http-api |

## 6. スコープ

### In Scope

- ExternalReviewGate 業務概念全体で観察可能な業務ふるまい（UC-ERG-001〜006）
- ふるまいの呼び出し失敗時に観察される拒否シグナル（業務ルール違反）
- 業務概念単位の E2E 検証戦略 → [`system-test-design.md`](system-test-design.md)

### Out of Scope（参照）

- Gate 操作 HTTP API → [`http-api/`](http-api/) sub-feature（Issue #61、M3 完了）
...
### 確定 R1-J: required_deliverable_criteria は Gate 生成時に凍結（R1-D と対称）

Gate 生成時、application 層（`GateService.create()`）が Stage.required_deliverables に紐づく各 `DeliverableTemplate.acceptance_criteria` を引き込み、`required_deliverable_criteria: tuple[AcceptanceCriterion, ...]` として Gate に渡す。Gate はこれを snapshot として保持し、以後変更不可（`_validate_criteria_immutable` 不変条件、§確定 R1-D の `deliverable_snapshot` と同パターン）。

**dangling ref（テンプレート不在）時の挙動（確定）**: `required_deliverable_template_ids` に含まれる `DeliverableTemplateId` の実体が見つからない場合（例: テンプレートが事前に削除された場合）、MVP ではその ID を **silently skip** して残りのテンプレートから criteria を収集する。Issue #122 §dangling-ref 設計判断と整合（被参照チェックは MVP スコープ外 YAGNI）。Gate 生成は失敗させない防衛的処理。完全な被参照チェックは別 Issue で対応する。

**`GateService.create()` 呼び出し統合**: `GateService.create()` の実際の呼び出し経路（`Task.request_external_review` 発火後の連携）は **Issue #107-D（または後続の統合 Issue）** で実装する。本 Issue #121 のスコープは Gate 構造への `required_deliverable_criteria` 追加と `GateService.create()` メソッド実装までとし、呼び出し統合は別 PR で行う。

目的: 「いつ作られた Gate に対して、どの基準でレビューしたか」を記録すること。後からテンプレートの基準が更新されても、Gate 内の snapshot は当時の基準を保持する。
...
| 16 | Gate 生成時、Stage.required_deliverables に紐づく各 DeliverableTemplate の acceptance_criteria が `required_deliverable_criteria` として Gate に snapshot 凍結される（業務ルール R1-J）。Stage に required_deliverables が存在しない場合は空タプルが設定される | UC-ERG-001 | TC-UT-GT-026〜028（[`domain/test-design.md`](domain/test-design.md)）|
| 17 | CEO が `GET /api/gates/{id}` でレスポンスに `required_deliverable_criteria` 一覧が含まれることを確認できる（Reviewer が「何を見るべきか」を把握できる）| UC-ERG-006 | TC-IT-ERG-HTTP-033（[`http-api/test-design.md`](http-api/test-design.md)）|

E2E（受入基準 14）は [`system-test-design.md`](system-test-design.md) で詳細凍結。受入基準 1〜12 は domain sub-feature の IT / UT で検証（[`domain/test-design.md`](domain/test-design.md)）。受入基準 15 は repository sub-feature の IT で検証（[`repository/test-design.md`](repository/test-design.md)）。受入基準 16〜17 は domain / http-api sub-feature の IT / UT で検証。
