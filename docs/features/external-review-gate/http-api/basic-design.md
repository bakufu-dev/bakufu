...
| REQ-ERG-HTTP-001〜006 | `ExternalReviewGateSchemas` | `backend/src/bakufu/interfaces/http/schemas/external_review_gate.py` | Pydantic v2 request / response model、VO 変換、マスク解除済み値の response 化 |
...
| 項目 | 内容 |
|---|---|
| 入力 | `GET /api/gates?decision=PENDING`。`decision` は MVP では `PENDING` のみ受理。reviewer は認証済み subject から取得し、query/body の自己申告 ID は受理しない |
...
| 項目 | 内容 |
|---|---|
| 入力 | `GET /api/tasks/{task_id}/gates`。reviewer は認証済み subject から取得する |
...
| 項目 | 内容 |
|---|---|
| 入力 | `GET /api/gates/{id}`。viewer は認証済み subject から取得する |
...
| 項目 | 内容 |
|---|---|
| 入力 | `POST /api/gates/{id}/approve`、Body `{"comment": str | null}`。actor は認証済み subject から取得する |
...
| 項目 | 内容 |
|---|---|
| 入力 | `POST /api/gates/{id}/reject`、Body `{"feedback_text": str}`。`feedback_text` は 1〜10000 文字。actor は認証済み subject から取得する |
...
| 項目 | 内容 |
|---|---|
| 入力 | `POST /api/gates/{id}/cancel`、Body `{"reason": str | null}`。actor は認証済み subject から取得する |
...
| MSG-ERG-HTTP-002 | エラー | Reviewer is not authorized for this gate | 認証済み subject が Gate reviewer と一致しない |
...
| security | 認証 subject / CSRF Origin middleware | 既存 http-api foundation を使用 | 自己申告 reviewer/actor ID を禁止し、状態変更 API は Origin 許可一覧を通す |
...
classDiagram
    ...
    ExternalReviewGatesRouter --> AuthenticatedSubject
...
- DB masking は Repository 責務。HTTP response は Issue #61 の契約に従い、認可済み reviewer にマスク解除後の原文を返す。
...
| 想定攻撃者 | 攻撃経路 | 保護資産 | 対策 |
|---|---|---|---|
| **T1: BOLA** | 他 reviewer の `gate_id` を推測して単件取得 / 判断 | Deliverable snapshot / audit_trail / decision | 認証済み `subject.owner_id` と `gate.reviewer_id` の一致を service で必須化し、不一致は 403。query/body の `reviewer_id` / `viewer_id` / `actor_id` は認可根拠にしない |
| **T2: 既決 Gate の再判断** | APPROVED 後に reject / cancel を再送 | Gate の判断一回性 | Domain state machine の `decision_already_decided` を 409 に変換 |
| **T3: secret 露出** | snapshot / feedback / audit comment に webhook URL が混入 | webhook URL / API key | Repository masking は DB 保護、API response は Issue #61 に従いマスク解除後の原文を返す。表示権限は subject 認可で閉じる |
| **T4: audit 改ざん** | 詳細取得を audit なしで返す | 閲覧監査ログ | `GET /api/gates/{id}` は `record_view` 保存後の Gate を返す |
| **T5: CSRF / Cross-Origin 状態変更** | ブラウザから approve / reject / cancel を不正送信 | Gate decision / audit_trail | `POST` 3 本は既存 `CsrfOriginMiddleware` の Origin 許可一覧検証対象。Cookie セッション導入時は SameSite=Strict と CSRF token を追加する |
