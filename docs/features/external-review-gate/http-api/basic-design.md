...
| REQ-ERG-HTTP-001〜006 | `ExternalReviewGateSchemas` | `backend/src/bakufu/interfaces/http/schemas/external_review_gate.py` | Pydantic v2 request / response model、VO 変換、HTTP レスポンス masking |
...
| 項目 | 内容 |
|---|---|
| 入力 | `GET /api/gates?reviewer_id=<UUID>&decision=PENDING`。`decision` は MVP では `PENDING` のみ受理 |
| 処理 | `reviewer_id` の PENDING Gate を `created_at DESC, id DESC` で取得する。reviewer 不在確認は行わず、Repository 結果に従う |
...
| 項目 | 内容 |
|---|---|
| 入力 | `GET /api/tasks/{task_id}/gates?reviewer_id=<UUID>` |
| 処理 | `task_id` の Gate 履歴を `created_at ASC, id ASC` で取得し、`gate.reviewer_id == reviewer_id` の行だけ返す |
...
| 項目 | 内容 |
|---|---|
| 入力 | `GET /api/gates/{id}?viewer_id=<UUID>` |
| 処理 | Gate を取得し、`viewer_id == gate.reviewer_id` を検証する。成功時は `record_view(viewer_id)` を呼び audit_trail に VIEWED を追記して保存する |
...
| 項目 | 内容 |
|---|---|
| 入力 | `POST /api/gates/{id}/approve`、Body `{"actor_id": UUID, "comment": str | null}` |
| 処理 | Gate を取得し、`actor_id == gate.reviewer_id` を検証する。PENDING のみ `gate.approve(actor_id, comment or "")` を実行し保存する |
...
| 項目 | 内容 |
|---|---|
| 入力 | `POST /api/gates/{id}/reject`、Body `{"actor_id": UUID, "feedback_text": str}`。`feedback_text` は 1〜10000 文字 |
| 処理 | Gate を取得し、`actor_id == gate.reviewer_id` を検証する。PENDING のみ `gate.reject(actor_id, feedback_text)` を実行し保存する |
...
| 項目 | 内容 |
|---|---|
| 入力 | `POST /api/gates/{id}/cancel`、Body `{"actor_id": UUID, "reason": str | null}` |
| 処理 | Gate を取得し、`actor_id == gate.reviewer_id` を検証する。PENDING のみ `gate.cancel(actor_id, reason or "")` を実行し保存する |
...
| ID | 種別 | メッセージ（要旨） | 表示条件 |
|---|---|---|---|
| MSG-ERG-HTTP-001 | エラー | Gate not found | 指定 Gate が存在しない |
| MSG-ERG-HTTP-002 | エラー | Reviewer is not authorized for this gate | `actor_id` / `viewer_id` が Gate reviewer と一致しない |
| MSG-ERG-HTTP-003 | エラー | Gate has already been decided | APPROVED / REJECTED / CANCELLED に再判断を要求 |
| MSG-ERG-HTTP-004 | エラー | Request validation failed | UUID / Body / query validation 失敗 |
...
| security | `mask()` | 既存 | HTTP レスポンスで `body_markdown` / `feedback_text` / audit comment を再マスク |
...
classDiagram
    ...
    class ExternalReviewGateService {
        -_repo
        -_session
        +list_pending(reviewer_id)
        +list_by_task(task_id, reviewer_id)
        +get_and_record_view(gate_id, viewer_id)
        +approve(gate_id, actor_id, comment)
        +reject(gate_id, actor_id, feedback_text)
        +cancel(gate_id, actor_id, reason)
    }
...
- reviewer / actor の認可境界は application service に閉じる。
- 状態遷移は Domain に「やれ」と命じ、HTTP 層は decision を直接書き換えない。
- DB masking は Repository 責務、HTTP response masking は defense-in-depth として schema serializer に閉じる。
...
sequenceDiagram
    ...
    Router->>Service: approve(gate_id, actor_id, comment)
    Service->>Repo: find_by_id(gate_id)
    Repo-->>Service: gate
    Service->>Service: assert actor_id == gate.reviewer_id
    Service->>Gate: approve(actor_id, comment, decided_at)
...
| 想定攻撃者 | 攻撃経路 | 保護資産 | 対策 |
|---|---|---|---|
| **T1: BOLA** | 他 reviewer の `gate_id` を推測して単件取得 / 判断 | Deliverable snapshot / audit_trail / decision | `viewer_id` / `actor_id` と `gate.reviewer_id` の一致を service で必須化し、不一致は 403 |
| **T2: 既決 Gate の再判断** | APPROVED 後に reject / cancel を再送 | Gate の判断一回性 | Domain state machine の `decision_already_decided` を 409 に変換 |
| **T3: secret 露出** | snapshot / feedback / audit comment に webhook URL が混入 | webhook URL / API key | Repository masking に加え、HTTP response serializer で `mask()` を再適用 |
