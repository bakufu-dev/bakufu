...
| クラス | フィールド | 型 | 制約 / 意図 |
|---|---|---|---|
| `AcceptanceCriterionResponse` | `id` | `str` | `AcceptanceCriterion.id` を str 化（UUID v4）|
| | `description` | `str` | AcceptanceCriterion.description（開発者定義テンプレートメタデータ、masking 不要）|
| | `required` | `bool` | AcceptanceCriterion.required |
...
| | `required_deliverable_criteria` | `list[AcceptanceCriterionResponse]` | Gate 生成時の AcceptanceCriterion snapshot（受入基準 #17）。空リスト可。order_index ASC 順（domain の tuple 元順序を保持）|
...
| `required_deliverable_criteria` | `array[AcceptanceCriterionResponse]` | 必須 | Gate 生成時の受入基準 snapshot（空配列可）。order_index ASC 順（受入基準 #17）|
