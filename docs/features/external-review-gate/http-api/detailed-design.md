...
HTTP schema serializer は `deliverable_snapshot.body_markdown` / `feedback_text` / `audit_trail[].comment` に `mask()` を再適用しない。ただし Repository の `MaskedText.process_result_value` は復号処理を持たず、保存時に不可逆マスクされた値をそのまま返す。したがって HTTP 出力契約は「Domain で未保存の raw 値は raw、Repository から復元した保存済み secret は redacted」であり、原文復号を要求しない。
...
### 確定 H: OWASP API Security Top 10 2023 を脅威棚卸しの基準にする

basic-design.md §OWASP API Security Top 10 2023 対応表を実装 PR のセキュリティチェックリストにする。API1 / API5 は reviewer 境界、API2 は token 運用、API3 は schema `extra="forbid"`、API4 は文字数上限、API6 は一回性と CSRF、API8 / API9 は app wiring と API inventory、API7 / API10 は外部通信なしの非該当根拠で閉じる。
