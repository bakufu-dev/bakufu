...
<!-- 配置先: docs/features/external-review-gate/test-design.md -->
<!-- 対象範囲: REQ-GT-001〜006 / MSG-GT-001〜007 / 受入基準 1〜15 / 詳細設計 §確定 A〜K / state machine 7 遷移 + 5 不変条件 + audit_trail append-only + snapshot 不変 + record_view 冪等性なし + auto-mask -->

本 feature は domain 層の Aggregate Root（ExternalReviewGate）+ VO（AuditEntry）+ enum（ReviewDecision / AuditAction）+ 例外（ExternalReviewGateInvariantViolation）に閉じる **M1 7 兄弟目（最後）**。HTTP API / CLI / UI の公開エントリポイントは持たないため、E2E は本 feature 範囲外（後続 `feature/external-review-gate-ui` / `feature/admin-cli` で起票）。本 feature のテストは **ユニット主体 + Aggregate 内 module 連携 + state machine 全 7 遷移網羅 + 監査要件物理保証** で構成する。
...
| REQ-GT-006（不変条件 5 種） | `_validate_*` helper 経路 | TC-UT-GT-007〜010 | ユニット | 正常系 / 異常系 | 7, 8, 9, 10 |
...
| frozen 不変性 | `gate.decision = X` 直接代入拒否 | TC-UT-GT-019 | ユニット | 異常系 | 13 |
| frozen 構造的等価 | 同一属性 Gate 2 インスタンスが `==` True | TC-UT-GT-012 | ユニット | 正常系 | 13 |
| `extra='forbid'` | 未知フィールド拒否 | TC-UT-GT-020 | ユニット | 異常系 | （T1 防御） |
...
| AC-15（カバレッジ）| `pytest --cov=bakufu.domain.external_review_gate` | （CI ジョブ）| — | — | 15 |
...
- REQ-GT-001〜006 すべてに最低 1 件のテストケース
...
- **5 不変条件 helper**: `_validate_decided_at_consistency`（TC-UT-GT-007）/ `_validate_snapshot_immutable`（TC-UT-GT-008）/ `_validate_audit_trail_append_only`（TC-UT-GT-009）/ `_validate_feedback_text_range`（TC-UT-GT-010）/ `_validate_decision_immutable`（TC-UT-GT-005）全 5 種に独立 unit
...
- **MSG 2 行構造 + Next: hint（§確定 I、room §確定 I 踏襲）**: TC-UT-GT-021〜025 で全 5 MSG-GT で `assert "Next:" in str(exc)` を CI 強制
- 受入基準 1〜13 すべてに unit/integration ケース（14/15 は CI ジョブ）
- 確定 A〜K すべてに証拠ケース、孤児要件ゼロ
...
|--------|-----|------------------|
| `GateFactory` | `ExternalReviewGate`（valid デフォルト = `decision=PENDING`、`feedback_text=''`、`audit_trail=[]`、`decided_at=None`） | `True` |
...
該当なし — 理由: domain 層、CLI / HTTP API / UI なし。後続 `feature/external-review-gate-ui` / `feature/admin-cli` で起票。
...
- **5 不変条件 helper** 全種に独立 unit ケース（decided_at_consistency / feedback_text_range / snapshot_immutable / audit_trail_append_only / decision_immutable）
...
- **MSG 2 行構造 + Next: hint**: TC-UT-GT-021〜025 で全 5 MSG-GT で `assert "Next:" in str(exc)` を CI 強制
- 受入基準 1〜13 すべてに unit/integration ケース（14/15 は CI ジョブ）
- 確定 A〜K すべてに証拠ケース
