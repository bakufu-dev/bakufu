...
本書は **実装直前の構造契約・確定文言・キー構造を凍結する**。Sub-issue 実装 PR は本書を改変せず参照する。設計変更が必要なら本書を先に更新する PR を立てる。

**書くこと**:
- 各 Aggregate / Entity / VO の属性・型・制約（構造契約の詳細）
- feature-spec.md §7 確定 R1-X で凍結された業務ルールを実装方針として展開する `§確定 A` / `§確定 B` ...
- 設計判断の補足（"なぜこの API 形か" "なぜこのデフォルト値か"）
- MSG 確定文言（実装者が改変できない契約）
- 永続化キー構造 / API エンドポイント詳細

**書かないこと**:
- 業務ルールの議論（採用根拠は feature-spec.md §7 で凍結済み、本書はそこを引用してさらに実装方針を展開する）
...
<!-- feature-spec.md §確定 R1-A 等で凍結した内容のうち、構造契約レベルでさらに細部を確定するものをここで凍結。 -->
