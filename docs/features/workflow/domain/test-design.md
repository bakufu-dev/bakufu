...
- 孤児要件ゼロ
- **⚠️ MSG-WF-013 設計ギャップ**: `kind='duplicate_required_deliverable'` の具体的メッセージ文言（`[FAIL] ... \nNext: ...` 2行形式）が `detailed-design.md` に未定義（MSG-WF-001〜012 止まり）。実装工程で設計書更新が必要。TC-UT-WF-063 は現在 `kind` の照合のみ実施し、文言照合は MSG-WF-013 定義後に追加する
...
| TC-UT-WF-063 | `Stage` に重複 `template_ref.template_id` | 異常系 | 同一 `template_id` を持つ `DeliverableRequirement` 2 件 | `StageInvariantViolation(kind='duplicate_required_deliverable')` を raise。Workflow 構築前に Stage レベルで検出される（R1-17）。⚠️ MSG-WF-013 未定義のため `message` 文言照合は保留（未決課題参照） |
...
| # | タスク | 起票先 | 備考 |
|---|-------|--------|------|
| 1 | **MSG-WF-013 設計書未定義** — `StageInvariantViolation(kind='duplicate_required_deliverable')` の `message` 文言（`[FAIL] ... \nNext: ...` 2行形式）が `detailed-design.md` に未記載 | `docs/features/workflow/domain/detailed-design.md` を更新して MSG-WF-013 を追加後、TC-UT-WF-063 の MSG 文言照合も追加 | 実装工程（Issue #117 実装 PR）で `detailed-design.md` §MSG 一覧を更新し、`kind='duplicate_required_deliverable'` のメッセージを凍結してからテスト実装へ進むこと |
| 2 | 該当なし — 外部 I/O ゼロのため characterization 不要 | — | 後続 `feature/persistence`（DB 永続化）/ `feature/http-api`（HTTP 境界の payload schema）/ `feature/discord-notifier`（実 webhook 送信）/ `feature/workflow-presets`（プリセット JSON 読み込み）が起票時に Workflow 起点の characterization が発生する見込み |
...
- [ ] **MSG-WF-013 未決**（未決課題 #1）が明記され、TC-UT-WF-063 の `message` 文言照合が保留となっていることを確認
