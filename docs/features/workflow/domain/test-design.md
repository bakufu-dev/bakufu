# テスト設計書 — workflow / domain

<!-- feature: workflow / sub-feature: domain -->
<!-- 配置先: docs/features/workflow/domain/test-design.md -->
<!-- 対象範囲: REQ-WF-001〜007 / R1-17（required_deliverables） / MSG-WF-001〜012 / 脅威 T1, T2, T3 / 受入基準 1〜10 / 詳細設計 確定 A〜G / DAG 不変条件 7 種 / SSRF G1〜G10 / シークレットマスキング / DeliverableRequirement VO -->

本 sub-feature は domain 層の Aggregate Root（Workflow）と内部 Entity（Stage / Transition）と VO（CompletionPolicy / NotifyChannel）と例外（WorkflowInvariantViolation / StageInvariantViolation）に閉じる。HTTP API / CLI / UI の公開エントリポイントは持たないため、E2E は本 sub-feature 範囲外（親 [`../system-test-design.md`](../system-test-design.md) で担当）。本 sub-feature のテストは **ユニット主体 + 結合は Aggregate 内 module 連携の往復シナリオ + V モデル開発室レンダリング例の from_dict 構築**で構成する。

empire の test-design.md（`docs/features/empire/domain/test-design.md`）と同じ規約を踏襲（外部 I/O ゼロ・factory に `_meta.synthetic = True` の `WeakValueDictionary` レジストリ）。

受入基準 1〜10 は本 sub-feature の IT / UT で検証する（親 [`../feature-spec.md §9`](../feature-spec.md) を参照）。受入基準 11（永続化再起動）は親 [`../system-test-design.md`](../system-test-design.md) が担当。受入基準 12（webhook token マスキング）は [`../repository/test-design.md`](../repository/test-design.md) が担当。
...
| REQ-WF-007-④ / R1-17 | Stage 自身: `required_deliverables` 空 tuple → 成功 | TC-UT-WF-061 | ユニット | 正常系 | R1-17 |
| REQ-WF-007-④ / R1-17 | Stage 自身: `required_deliverables` 1 件 → 成功 | TC-UT-WF-062 | ユニット | 正常系 | R1-17 |
| REQ-WF-007-④ / R1-17 | Stage 自身: `required_deliverables` 内 `template_id` 重複 → `duplicate_required_deliverable` | TC-UT-WF-063 | ユニット | 異常系 | R1-17 |
| R1-17（DeliverableRequirement VO） | `DeliverableRequirement(template_ref, optional=False)` 構築 | TC-UT-VO-WF-005 | ユニット | 正常系 | R1-17 |
| R1-17（DeliverableRequirement VO） | `DeliverableRequirement(template_ref, optional=True)` 構築 | TC-UT-VO-WF-006 | ユニット | 正常系 | R1-17 |
| R1-17（DeliverableRequirement VO） | `DeliverableRequirement` frozen 不変性 | TC-UT-VO-WF-007 | ユニット | 異常系 | — |
| R1-17（from_dict ペイロード） | `from_dict` で `required_deliverables` 含む Stage ペイロードを構築 | TC-UT-WF-064 | ユニット | 正常系 | R1-17 |
...
| MSG-WF-013 | `[FAIL] Stage {stage_id} required_deliverables has duplicate template_id: {template_id}` | TC-UT-WF-063 | ユニット | 異常系 | R1-17 |
...
- MSG-WF-001〜013 すべてに静的文字列照合（MSG-WF-012 は TC-UT-WF-019 で、MSG-WF-013 は TC-UT-WF-063 で文言完全一致照合）
- **R1-17（required_deliverables）**: `DeliverableRequirement` VO 構築・frozen・重複 template_id 検査（TC-UT-WF-061〜064 / TC-UT-VO-WF-005〜007）
- 受入基準 1〜10 すべてに unit/integration ケース
...
受入基準 11（再起動跨ぎ保持）/ 12（webhook token 永続化マスキング）は本 sub-feature の IT / UT では検証しない（親 [`../system-test-design.md`](../system-test-design.md) / [`../repository/test-design.md`](../repository/test-design.md) が担当）。

## 外部 I/O 依存マップ

| 外部 I/O | 用途 | raw fixture | factory | characterization 状態 |
|--------|-----|------------|---------|---------------------|
| **該当なし** | Workflow は domain 層単独で外部 I/O を持たない（HTTP / DB / ファイル / 時刻 / LLM / Discord いずれも未依存）。`NotifyChannel.target` の URL 文字列は **VO 内に保持される文字列**であって本 sub-feature では通信しない（実際の Discord webhook 送信は `feature/discord-notifier` 責務） | — | — | **不要（外部 I/O ゼロ）** |
...
- [`../feature-spec.md §8`](../feature-spec.md) §制約・前提 で「ネットワーク: 該当なし」と凍結
- T3（SSRF/A10）対策の URL allow list は **文字列バリデーション** であって通信ではない。実 webhook 送信時の URL 解釈・SSRF 防御は後続 `feature/discord-notifier` の責務として残し、本 sub-feature では VO レベルの allow list 形式照合のみテストする
...
| `StageFactory` | `Stage`（valid デフォルト = `kind=WORK` / `required_role={DEVELOPER}` / `required_deliverables=()` / `notify_channels=[]`） | `True` |
...
| `DeliverableRequirementFactory` | `DeliverableRequirement`（valid デフォルト = `optional=False` + 新規 `DeliverableTemplateRef`） | `True` |
...
- E2E は親 [`../system-test-design.md`](../system-test-design.md) が担当（TC-E2E-WF-001, 002）
- 受入基準 10「V モデル開発室を `from_dict` で構築できる」は、本 sub-feature では **結合テスト** の TC-IT-WF-001 として整理する（公開 I/F 経由ではないため）

## 結合テストケース

domain 層単独の本 sub-feature では「結合」を **Aggregate 内 module 連携（Workflow + Stage + Transition + NotifyChannel + CompletionPolicy + 例外の往復シナリオ）+ V モデル開発室レンダリング例の bulk-import** と定義する。外部 LLM / Discord / GitHub / DB は本 sub-feature では使わないためモック不要。
...
**注**: 本 sub-feature では結合テストも `tests/integration/test_workflow.py` ではなく `tests/domain/test_workflow.py` 内の「往復シナリオ」「from_dict 結合」セクションとして実装してよい（empire と同方針）。

## ユニットテストケース

`tests/factories/workflow.py` の factory 経由で入力を生成する。raw fixture は本 sub-feature では外部 I/O ゼロのため存在しない。設計書凍結例（V モデル）は JSON 固定。
...
| TC-UT-WF-061 | `Stage(required_deliverables=())` | 正常系 | 空 tuple の `required_deliverables` | 構築成功（R1-17: 空リストは合法） |
| TC-UT-WF-062 | `Stage(required_deliverables=(req,))` | 正常系 | 1 件の `DeliverableRequirement` を含む | 構築成功、`len(stage.required_deliverables) == 1` |
| TC-UT-WF-063 | `Stage` に重複 `template_ref.template_id` | 異常系 | 同一 `template_id` を持つ `DeliverableRequirement` 2 件 | `StageInvariantViolation(kind='duplicate_required_deliverable')` を raise。Workflow 構築前に Stage レベルで検出される（R1-17）。`message` が `[FAIL] Stage {stage_id} required_deliverables has duplicate template_id: {template_id}`（MSG-WF-013）と完全一致。`detail` は `{"stage_id": str, "template_id": str}` のキーのみ（A09 whitelist）。 |
...
| TC-UT-WF-064 | `from_dict` で `required_deliverables` 含む Stage ペイロードを構築 | 正常系 | Stage に `"required_deliverables": [{"template_ref": {"template_id": "<uuid>", "minimum_version": {"major": 1, "minor": 0, "patch": 0}}, "optional": false}]` を含む payload | 構築成功、`stages[n].required_deliverables` に 1 件の `DeliverableRequirement` が含まれる（R1-17 / 確定 D ペイロード形式更新） |
...
`detailed-design.md` §確定 G の G1〜G10 を**全て独立 unit ケース**で検証する。1 つでも抜けると SSRF/A10 の防衛線に穴が空く。
...
| TC-UT-WF-052 | G8（query 禁止） | 異常系 | `target='https://discord.com/api/webhooks/123/abc?override=x'` / `'?'`（空 query） | `pydantic.ValidationError`、`parsed.query == ''` 検査、detail に G8 違反 |
| TC-UT-WF-053 | G9（fragment 禁止） | 異常系 | `target='https://discord.com/api/webhooks/123/abc#frag'` / `'#'`（空 fragment） | `pydantic.ValidationError`、`parsed.fragment == ''` 検査、detail に G9 違反 |
...
| テストID | 対象 | 種別 | 入力 | 期待結果 |
|---------|-----|------|------|---------|
| TC-UT-WF-055 | `NotifyChannel(kind='slack', target=...)` | 異常系 | `kind='slack'`、target は任意 | `pydantic.ValidationError`、`kind` Literal['discord'] 違反 |
...
| TC-UT-WF-063 | MSG-WF-013 | 同一 `template_id` を持つ `DeliverableRequirement` 2 件 | `[FAIL] Stage <stage_id> required_deliverables has duplicate template_id: <template_id>` 完全一致、`detail` キーは `stage_id` / `template_id` のみ（A09 whitelist） |
...
| TC-UT-VO-WF-005 | `DeliverableRequirement(template_ref=..., optional=False)` | 正常系 | `DeliverableTemplateRef` + `optional=False`（デフォルト） | 構築成功、`req.optional is False`（R1-17） |
| TC-UT-VO-WF-006 | `DeliverableRequirement(template_ref=..., optional=True)` | 正常系 | `DeliverableTemplateRef` + `optional=True` | 構築成功、`req.optional is True`（R1-17） |
| TC-UT-VO-WF-007 | `DeliverableRequirement` frozen 不変性 | 異常系 | 構築後に `req.optional = False` 直接代入 | `pydantic.ValidationError` または `TypeError`（frozen=True 制約） |

## カバレッジ基準

- REQ-WF-001 〜 007 の各要件が**最低 1 件**のテストケースで検証されている（マトリクス参照）
- DAG 不変条件 7 種それぞれが独立した unit ケースで検証されている（REQ-WF-005-①〜⑦）
- MSG-WF-001 〜 013 の各文言が**静的文字列で照合**されている（TC-UT-WF-019, 037 〜 047, 063）
- **R1-17（required_deliverables）**: `DeliverableRequirement` VO 構築（TC-UT-VO-WF-005〜007）/ Stage 不変条件 `duplicate_required_deliverable`（TC-UT-WF-061〜063）/ from_dict ペイロード形式（TC-UT-WF-064）が各々最低 1 件で検証
- 受入基準 1 〜 10 の各々が**最低 1 件のユニット/結合ケース**で検証されている（E2E は親 system-test-design.md 担当）
...
- C0 目標: `domain/workflow.py` で **95% 以上**（domain 層基準）
...
- V モデル開発室レンダリング例の構築実観測: `uv run python -c "import json; from bakufu.domain.workflow import Workflow; wf = Workflow.from_dict(json.load(open('backend/tests/fixtures/v_model_workflow.json'))); print(len(wf.stages), len(wf.transitions))"` で `13 15` が出力されることを目視
...
| # | タスク | 起票先 | 備考 |
|---|-------|--------|------|
| 1 | 該当なし — 外部 I/O ゼロのため characterization 不要 | — | 後続 `feature/persistence`（DB 永続化）/ `feature/http-api`（HTTP 境界の payload schema）/ `feature/discord-notifier`（実 webhook 送信）/ `feature/workflow-presets`（プリセット JSON 読み込み）が起票時に Workflow 起点の characterization が発生する見込み |

**本 feature 固有の申し送り**（`feature/discord-notifier` への申し送り）:
- NotifyChannel.target の URL allow list（G1〜G10）は **形式バリデーション** のみ。実 webhook 送信時の SSRF 防御（DNS rebinding / IPv4 mapped IPv6 / URL リダイレクト追跡時の再検査）は `feature/discord-notifier` 責務として残す（[detailed-design.md §確定 G `Phase 2 で再検証が必要な項目`](detailed-design.md)）
- シークレットマスキング 6 経路の責務分離: 本 sub-feature でカバーするのは VO `model_dump(mode='json')` / `model_dump_json()` 出力 と 例外 `message` / `detail` の 2 経路（TC-UT-WF-057〜059）。残り 4 経路（Outbox `payload_json` / `audit_log.args_json` / Conversation system message / 構造化ログ stdout・file）は `feature/persistence` / `feature/discord-notifier` / `feature/audit-log` の責務として申し送り

## レビュー観点（テスト設計レビュー時）

- [ ] DAG 不変条件 7 種すべてが REQ-WF-005-①〜⑦ として独立した unit ケースで検証されている
- [ ] MSG-WF-001〜013 の文言が静的文字列で照合される設計になっている（MSG-WF-012 = stage_not_found 含む、MSG-WF-013 = duplicate_required_deliverable 含む）
...
- [ ] Stage 自身の不変条件と Workflow 集約検査の両方でテストが用意されている（二重防護、TC-UT-WF-006a / 006b）
- [ ] 確定 F の 7 helper を直接 import する独立性検証（TC-UT-WF-060）が含まれている
- [ ] **NotifyChannel URL allow list G1〜G10 全項目**が独立した unit ケース（TC-UT-WF-034〜036, 048〜054）で検証されている
- [ ] **シークレットマスキング**が 3 経路（`model_dump(mode='json')` / `model_dump_json()` / 例外 message・detail）で検証されている
...
- [ ] **R1-17 / Issue #117**: `DeliverableRequirement` VO（TC-UT-VO-WF-005〜007）/ `required_deliverables` 不変条件（TC-UT-WF-061〜063）/ `from_dict` ペイロード（TC-UT-WF-064）が追加されている
- [ ] **MSG-WF-013 確定済み**（`[FAIL] Stage {stage_id} required_deliverables has duplicate template_id: {template_id}`）: TC-UT-WF-063 に文言完全一致照合と `detail` キーホワイトリスト（`{"stage_id": str, "template_id": str}`）確認が物理追加されていること
