...
> 関連: [basic-design.md](basic-design.md) / [`../../empire-repository/detailed-design.md`](../../empire-repository/detailed-design.md) **テンプレート真実源** / [`../domain/detailed-design.md`](../domain/detailed-design.md)（domain VO 凍結済み）
...
| `workflow_stages` | `infrastructure/persistence/sqlite/tables/workflow_stages.py`（新規） | `workflow_id: UUIDStr FK CASCADE` / `stage_id: UUIDStr` / `name: String(80)` / `kind: String(32)` / `roles_csv: String(255)` / `required_deliverables_json: JSONEncoded` / `completion_policy_json: JSONEncoded` / **`notify_channels_json: MaskedJSONEncoded`** / UNIQUE(workflow_id, stage_id) |
...
| 3 | workflow_stages bulk INSERT | `INSERT INTO workflow_stages (workflow_id, stage_id, name, kind, roles_csv, required_deliverables_json, completion_policy_json, notify_channels_json) VALUES ...`（workflow.stages 件数分、`required_deliverables_json` は `JSONEncoded` 経由、`notify_channels_json` は `MaskedJSONEncoded.process_bind_param` 経由でマスキング後 JSON） |
...
2. `stage_rows: list[dict[str, Any]]` — 各 Stage を `{'workflow_id': ..., 'stage_id': ..., 'name': ..., 'kind': ..., 'roles_csv': ..., 'required_deliverables_json': ..., 'completion_policy_json': ..., 'notify_channels_json': ...}` に変換（`required_deliverables_json` は `stage.required_deliverables` を `[{"template_ref": {"template_id": "...", "minimum_version": {...}}, "optional": bool}, ...]` 形式の JSON 配列に変換）
...
戻り値: `Workflow(id=..., name=..., stages=[...], transitions=[...], entry_stage_id=...)`。Aggregate Root の不変条件（DAG / 一意性 / capacity）は Workflow 構築時の `model_validator(mode='after')` + `dag_validators` で再走（Repository は再 validate しない契約だが、構築は valid な状態で行われる）。

##### `required_deliverables_json` のシリアライズ契約（A08 Unsafe Deserialization 対応）

| 経路 | 変換 |
|---|---|
| `_to_row` | `stage.required_deliverables` の各 `DeliverableRequirement` を `dr.model_dump(mode='json')` で `list[dict]` に変換。`JSONEncoded` TypeDecorator が `json.dumps` で DB 保存 |
| `_from_row` | DB の JSON 文字列を `json.loads` → `list[dict]` → `[DeliverableRequirement.model_validate(d) for d in ...]` → `tuple[DeliverableRequirement, ...]` に変換。**生 dict を `Stage` コンストラクタに直接渡すことは禁止**（Pydantic `model_validate` を必ず経由してバリデーション済み VO として復元すること）|

`DeliverableRequirement.model_validate` の検査内容: `template_ref.template_id` が UUID 形式、`template_ref.minimum_version` が SemVer 形式（`major/minor/patch` が 0 以上整数）、`optional` が `bool`。型違反は `pydantic.ValidationError` を raise — application 層が catch し、上位へ伝播させる（§確定 H / §確定 I と同一契約）。
...
`docs/design/domain-model/storage.md` §逆引き表に Workflow 関連 3 行追加:

| 行 | 内容 |
|---|---|
| `workflow_stages.notify_channels_json` | `MaskedJSONEncoded`、Discord webhook token マスキング、Schneier 申し送り #6 + workflow §確定 H の Repository 経路実適用 |
...
| 採用 | 不採用 | 理由 |
|---|---|---|
| **(a) `roles_csv: String(255)`** にカンマ区切り（**sorted で決定論化**） | (b) 別テーブル `workflow_stage_roles(workflow_id, stage_id, role)` 正規化 | (b) は workflow_stage 1 件あたり SELECT が 4 段（workflows / stages / transitions / stage_roles）に増えて N+1 リスク。MVP で role 件数は最大 5 程度でカンマ区切りカーディナリティが小さい |
| | (c) `JSONEncoded` で `["AGENT", "REVIEWER"]` 配列保存 | (c) は SQL レベル LIKE 検索ができない。MVP ではまだ検索しないが、将来「role を含む Stage を全 Workflow から横断検索」が想定される。CSV なら `LIKE '%AGENT%'` で索引可能 |
...
`roles_csv.split(",")` → 各文字列を `Role(s)` で enum に戻す → `frozenset(...)` で frozenset 化。空文字列は `Stage.required_role` の `model_validator` で `empty_required_role` として弾かれる（domain で凍結済み）ため、Repository が空文字列を許す処理を入れない（Fail Fast）。

##### 制約サイズ

`String(255)` の根拠: Role enum は最大 5 件（domain §Role 凍結）、enum 値は最大 32 文字、`5 × 32 + 4(comma) = 164` < 255 で十分余裕。後続 PR が Role enum を拡張しても 7 件以下なら破綻しない。

##### Role enum 値が区切り文字 `,` を含まない契約（CSV シリアライズ可逆性の前提）

CSV split/join が round-trip で壊れないのは **Role enum 値そのものに `,` が含まれない**ことを前提とする。domain で凍結済み: `domain/value_objects.py` の `Role(StrEnum)` 定義が **ASCII 大文字 + アンダースコアのみ**（10 値、すべて `[A-Z_]+` 範囲）で、区切り文字 `,` は構造的に出現し得ない。

| 経路 | 前提が崩れる条件 | 防衛 |
|---|---|---|
| `_to_row`（join） | Role 値に `,` を含む値が後続 PR で追加される | domain で `Role(StrEnum)` を凍結（enum 値の追加 PR は同 feature の設計書修正と連動）+ 本詳細設計の本節を参照する責務 |
...
後続 PR が Role enum 値を追加・変更する場合、本節を**逆引き参照**してエスケープ不要を確認する責務を持つ。enum 値に `,` を含めたい場合は、本詳細設計の §確定 G 採用判定（CSV vs 別テーブル）を再議論する PR を立てる必要がある（CSV エスケープ実装は不採用）。
...
| | (c) 別テーブル `workflow_stage_notify_channels` 正規化 | NotifyChannel は順序保持必要（list 凍結）、別テーブル化すると `ORDER BY` 必須 + JOIN 増 |
...
domain sub-feature で **Workflow 構築時の Aggregate 不変条件**として既に凍結済み（`dag_validators._validate_entry_stage_in_stages`）。本 PR ではその凍結を信頼して FK を張らない。

##### 申し送り（Workflow 後続申し送り #2）: PostgreSQL 移行時の対応指針
...
**本 PR では (ii) で凍結**、移行時に再議論する事項を [`../../../design/migration-plan.md`](../../../design/migration-plan.md) §TODO-MIG-001 に記録済み（申し送りの行き先を本 PR スコープで確定）。後続 PR が「設計書のどこに方針が書かれていたか」を逆引きする経路を物理的に保証する。

##### `migration-plan.md` への流入元としての位置づけ

| 流入元 | 受け皿 |
|---|---|
| 本 §確定 J（workflows.entry_stage_id の DB FK 不採用） | [`../../../design/migration-plan.md`](../../../design/migration-plan.md) §TODO-MIG-001 |
...
### 確定 K: テスト責務の 4 ファイル分割（empire-repo PR #29 Norman 教訓）
...
| `test_workflow_repository/test_save_semantics.py` | delete-then-insert の物理保証、`roles_csv` の sorted 決定論、`completion_policy_json` の往復 |
| `test_workflow_repository/test_constraints_arch.py` | UNIQUE 制約 / FK CASCADE / ORDER BY 物理保証 / pyright Protocol 充足 |
| `test_workflow_repository/test_masking.py` | `notify_channels_json` の masking 経路（TC-IT-WFR-013 / 014）— Workflow が masking gateway 初実適用 PR のため独立 |

各ファイルは 200 行を目安、500 行ルール厳守。

### 確定 L: `deliverable_template` → `required_deliverables_json` Alembic migration 方針（Issue #117 連動）

Issue #117 で `Stage.deliverable_template: str` が廃止され `Stage.required_deliverables: tuple[DeliverableRequirement, ...]` に置き換えられた。`workflow_stages` テーブルのカラムも同期して以下の migration を実施する。

#### migration スクリプト方針（凍結）

| 操作 | SQL 相当 | 備考 |
|---|---|---|
| 旧カラム削除 | `ALTER TABLE workflow_stages DROP COLUMN deliverable_template` | SQLite は `DROP COLUMN` を 3.35.0+ でサポート。`pyproject.toml` の `requires-python = ">=3.12"` により Python 3.12 同梱 SQLite は **3.42.0 以上が保証**される（3.35.0 を大幅に上回るため追加要件なし） |
| 新カラム追加 | `ALTER TABLE workflow_stages ADD COLUMN required_deliverables_json TEXT NOT NULL DEFAULT '[]'` | SQLAlchemy の `JSONEncoded` TypeDecorator として定義。デフォルト `'[]'` で既存行を空リスト状態に移行 |

#### Alembic revision 配置

Issue #117 実装 PR（workflow repository の実装変更）に同梱し、単一 revision として発行する。`op.drop_column` → `op.add_column` の順。rollback（`downgrade`）は `required_deliverables_json` を削除し `deliverable_template TEXT NOT NULL DEFAULT ''` を追加する（既存データ復元は不要、ダウングレード後は空文字が入る）。

#### 既存データ移行

MVP 時点では本番データなし（開発環境のみ）のため、既存行への `deliverable_template` → `required_deliverables_json` のデータ変換は不要。将来本番環境で `deliverable_template` に値が入っているケースが発生した場合は、migration スクリプト内で `UPDATE` によるデータ変換（Markdown → 空リスト相当の JSON）を検討する。現時点では `DEFAULT '[]'` のみで充足と凍結。
...
| `kind` | enum |
| `roles_csv` | enum 列挙 |
| `required_deliverables_json` | JSON 化された DeliverableRequirement リスト（`template_id` UUID + `optional` bool）、secret 流入想定なし |
| `completion_policy_json` | VO（kind enum + description 自由記述）、secret 流入想定なし |

過剰マスキング（`required_deliverables_json` まで `MaskedJSONEncoded` にする等）は CI Layer 2 が「対象なし」を assert することで防ぐ（empire-repo §確定 E）。

### なぜ entry_stage_id の FK を張らないか

§確定 J 参照。短く言うと**循環参照 + portability 問題**。Aggregate 不変条件で守る方式は domain sub-feature で凍結済みで本 PR はその上に乗る。
...
| `required_deliverables_json` | `JSONEncoded` | NOT NULL DEFAULT '[]' | `list[DeliverableRequirement]` の JSON 表現。`DeliverableRequirement` は `template_ref`（`template_id` UUID + `minimum_version` SemVer）と `optional` bool を持つ。空リストは `'[]'` |
...
`entry_stage_id` と同じく循環参照回避。Workflow 構築時の `dag_validators._validate_transition_refs` で `from_stage_id` / `to_stage_id` ∈ stages の id 集合を Aggregate 不変条件として保証（domain sub-feature 凍結済み）。
...
- [`../../persistence-foundation/`](../../persistence-foundation/) — M2 永続化基盤（PR #23 マージ済み、`MaskedJSONEncoded` / `MaskingGateway` 実装）
- [`../../empire-repository/`](../../empire-repository/) — **テンプレート真実源**（PR #29 / #30 マージ済み、§確定 A〜F + §Known Issues §BUG-EMR-001 規約）
- [`../domain/`](../domain/) — Workflow domain 設計（PR #16 マージ済み、Aggregate 不変条件 + §確定 G で `frozenset[Role]` / `list[NotifyChannel]` / `CompletionPolicy` 凍結）
- [`../../../design/domain-model/aggregates.md`](../../../design/domain-model/aggregates.md) — Workflow 凍結済み設計
- [`../../../design/domain-model/storage.md`](../../../design/domain-model/storage.md) — 逆引き表（本 PR で Workflow 3 行追加）
- [`../../../design/threat-model.md`](../../../design/threat-model.md) — A02 / A04 / A08 対応根拠
