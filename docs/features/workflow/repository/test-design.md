...
> 対象範囲: REQ-WFR-001〜005 / 受入基準 1〜13（§確定 A〜K）/ Repository ポート + delete-then-insert + domain↔row 変換（§G/H/I）+ ORDER BY 規約 + CI 三層防衛 + partial-mask テンプレート
...
| REQ-WFR-003（chain 一直線） | revision 0001 → 0002 → 0003 が一直線（CI 検査で head 分岐なし、`0003.down_revision == "0002_empire_aggregate"`） | TC-IT-WFR-022 | 結合 | 正常系 | 11 |
...
- **受入基準 1〜13 の全件に unit/integration ケース**（§確定 A〜K の全証拠ケース含む）
...
### Alembic 0003 revision（受入基準 11、§確定 E）
...
- 受入基準 1 〜 13 の各々が**最低 1 件のユニット/結合ケース**で検証されている（E2E は親 system-test-design.md が担当）
- 確定 A〜K すべてに証拠ケース
