...
- TC-E2E-WF-003: HTTP API 経由での Workflow 構築 → 永続化 → 取得（`workflow/http-api/` 完成後）
...
- notify_channels 不在 Workflow でのラウンドトリップは TC-E2E-WF-001 / 002 でカバー（§確定 H §不可逆性による制約のため、EXTERNAL_REVIEW Stage を含む E2E ラウンドトリップは不可）
...
- TC-E2E-WF-003, 004 は将来の sub-feature 追加時に本書を更新する別 PR で起票
