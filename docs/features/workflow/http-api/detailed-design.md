...
- `preset_name` が None の場合: `name` / `stages` / `transitions` / `entry_stage_id` が全て設定されていなければ 422
...
| `notify_channels` | `list[str]` | **GET**: DB から復元した masked 文字列（`<REDACTED:DISCORD_WEBHOOK>` 含む）のまま返却（不可逆性）。**POST/PATCH**: in-memory `NotifyChannel` を `NotifyChannel.model_dump(mode='json')['target']` でシリアライズすることで domain の `field_serializer` による masking を適用し、raw token をレスポンスに含めない |
...
| `WorkflowIrreversibleError` | `WorkflowService.update`（find_by_id で pydantic.ValidationError 発生時 — notify_channels masked）| 409 | `conflict` | MSG-WF-HTTP-008 |
...
| `workflow_archived_handler` | `WorkflowArchivedError` | MSG-WF-HTTP-002 + HTTP 409 |
| `workflow_irreversible_handler` | `WorkflowIrreversibleError` | `ErrorResponse(code="conflict", message=MSG-WF-HTTP-008)` + HTTP 409 |
...
**`workflow_archived_handler` の判定ルール**:
- `WorkflowArchivedError` は `kind: str` 属性を持つ（現在は `"update"` 固定）
- `kind="update"` → 409, MSG-WF-HTTP-002（PATCH で archived Workflow を操作）

## 確定 D: プリセット定義構造

プリセットは **アプリ内 static データ** として `bakufu/application/presets/workflow_presets.py` にモジュールレベル定数として定義する（DDD 原則: schemas は interfaces 層のため application 層配置が正しい）。DB クエリ / ファイル読み込みは不使用。
...
| `WorkflowArchivedError` | `Exception` | `workflow_id: str`, `kind: str` | update で archived=True の Workflow を操作（`kind="update"` 固定）|
| `WorkflowIrreversibleError` | `Exception` | `workflow_id: str` | update で pydantic.ValidationError（notify_channels masked 状態）が発生した場合。WorkflowRepository.find_by_id が masked URL を復元しようとして NotifyChannel バリデーションに失敗した際に raise |
...
| `update` | `workflow_id: WorkflowId, name: str \| None, stages: list \| None, transitions: list \| None, entry_stage_id: StageId \| None` | `Workflow` | `WorkflowNotFoundError` / `WorkflowArchivedError(kind="update")` / `WorkflowIrreversibleError` / `WorkflowInvariantViolation` |
...
| MSG-WF-HTTP-008 | `conflict` | `"Workflow contains masked notify_channels and cannot be modified. Please recreate the workflow with new webhook URLs."` | 409 |
...
| Q-OPEN-2 | 【確定】プリセット定義の配置先: `bakufu/application/presets/workflow_presets.py`（DDD 原則: schemas は interfaces 層のため application 層配置が正しい。§確定D で凍結）| 本 PR（Issue #58）|
