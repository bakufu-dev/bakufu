...
E2E（受入基準 11, 23）は [`system-test-design.md`](system-test-design.md) で詳細凍結。受入基準 1〜10 は domain sub-feature の IT / UT で検証（[`domain/test-design.md`](domain/test-design.md)）。受入基準 12 は repository sub-feature の IT で検証。受入基準 13〜21 は http-api sub-feature の IT / UT で検証（[`http-api/test-design.md`](http-api/test-design.md)）。
