...
> 関連 Issue: [#9 feat(workflow): Workflow + Stage + Transition Aggregate (M1)](https://github.com/bakufu-dev/bakufu/issues/9) / [#31 feat(workflow-repository): Workflow SQLite Repository (M2)](https://github.com/bakufu-dev/bakufu/issues/31)
...
| http-api | (将来) | UI / 外部クライアントから Workflow を操作・取得する経路 |
...
- Workflow の HTTP API → 将来の `workflow/http-api/` sub-feature
...
E2E（受入基準 11）は [`system-test-design.md`](system-test-design.md) で詳細凍結。受入基準 1〜10 は domain sub-feature の IT / UT で検証（[`domain/test-design.md`](domain/test-design.md)）。受入基準 12 は repository sub-feature の IT で検証。
