...
| 10 | V モデル開発室の JSON プリセット（14 Stage / 16 Transition）から Workflow を一括構築できる（業務ルール R1-2〜9 全充足）。Stage 14「リリース前承認」（`kind=EXTERNAL_REVIEW`, `notify_channels=[]`）を含む。M7 で MVP 受入基準 #5（UI 承認）確認のため追加（[`workflow_presets.py`](../../../../backend/src/bakufu/application/presets/workflow_presets.py) 参照）| UC-WF-006 | TC-IT-WF-001（[`domain/test-design.md`](domain/test-design.md)） |
...
| Q-WF-003 | 【確定 — M7 Issue #168】V-model プリセットに Stage 14「リリース前承認」（`kind=EXTERNAL_REVIEW`, `required_role=["LEADER"]`, `notify_channels=[]`, `required_gate_roles=[]`）と Transition 16（Stage 13 → Stage 14, condition=APPROVED）を追加する。目的: MVP 受入基準 #5「UI で承認 / 差し戻し操作ができる」を M7 E2E 検証で確認するため EXTERNAL_REVIEW Stage が 1 件以上必要。Discord 通知は Phase 2（M6-A post-MVP [R133]）のため `notify_channels=[]`。`workflow_presets.py` 変更後は `TC-IT-WF-001` も 14 Stage / 16 Transition で再確認すること | Issue #168 で実装 |
