...
CEO は Workflow をアーカイブ（論理削除）できる。アーカイブ済み Workflow は更新操作（PATCH）を受け付けない。ただし取得（GET）は可能。アーカイブされた Workflow を Room への新規割り当て（POST /api/rooms/{room_id}/workflows）に指定することは禁止される。
...
| 22 | アーカイブ済み Workflow を Room に割り当てようとすると 409 で拒否される（業務ルール R1-14）| UC-WF-008, R1-14 | TC-IT-WFH-010 |
| 23 | HTTP API 経由 Workflow CRUD が再起動跨ぎで保持される（E2E）| UC-WF-008〜010 | TC-E2E-WF-003（[`system-test-design.md`](system-test-design.md)）|

E2E（受入基準 11, 23）は [`system-test-design.md`](system-test-design.md) で詳細凍結。受入基準 1〜10 は domain sub-feature の IT / UT で検証（[`domain/test-design.md`](domain/test-design.md)）。受入基準 12 は repository sub-feature の IT で検証。受入基準 13〜22 は http-api sub-feature の IT / UT で検証（[`http-api/test-design.md`](http-api/test-design.md)）。
