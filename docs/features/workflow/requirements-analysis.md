...
> 凍結済み設計: [`docs/design/domain-model/aggregates.md`](../../design/domain-model/aggregates.md) §Workflow / [`value-objects.md`](../../design/domain-model/value-objects.md) §Workflow 構成要素
...
| 可搬性 | 純 Python のみ |
| セキュリティ | `from_dict` 経路で外部 JSON を受け取るが、Pydantic 型強制で `Role` 名や UUID 形式の不正値を Fail Fast で拒否。詳細は [`threat-model.md`](../../design/threat-model.md) §A04 |
...
| 10 | V モデル開発室レンダリング例（[`transactions.md`](../../design/domain-model/transactions.md)）を `from_dict` で構築できる | TC-UT-WF-010（E2E 相当） |
...
| Stage.notify_channels | Discord webhook URL 等 | **中**（URL 自体は秘匿性中、配信時にマスキング対象。詳細は [`storage.md`](../../design/domain-model/storage.md)）|
