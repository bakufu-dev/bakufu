...
| UC-WSB-003 | CEO | Agent のステータス（idle / running / blocked）が変化したとき、UI で確認したい | **M5 Phase 2**（`AgentStatusChangedEvent` クラス定義のみ M4・型定義として凍結、`AgentService.update_status()` 未実装のため Service publish 統合は M5）| domain / http-api |
| UC-WSB-004 | CEO | Directive が完了（DONE / FAILED）したとき、UI で確認したい | **M5 Phase 2**（`DirectiveCompletedEvent` クラス定義のみ M4・型定義として凍結、`DirectiveService.complete()/fail()` 未実装のため Service publish 統合は M5）| domain / http-api |
...
- MVP 対象の Domain Event クラス定義（Task / ExternalReviewGate / Agent / Directive）— **Agent / Directive は M4 でクラス定義・型定義のみ凍結。実 publish なし（M5 Phase 2 まで Dead Code）**
- `EventBusPort` インターフェース（application/ports）
- `InMemoryEventBus` 実装（infrastructure）
- 各 ApplicationService への `event_bus.publish()` 統合 — **M4 対象: TaskService（`cancel()` / `unblock_retry()` / `commit_deliverable()`）/ ExternalReviewGateService（`approve()` / `reject()`）のみ。AgentService / DirectiveService は M5 Phase 2**
...
- `AgentService.update_status()` / `DirectiveService.complete()` / `DirectiveService.fail()` の実装および `publish()` 統合 → **M5 Phase 2**（これらメソッドが M4 時点で未実装のため。`AgentStatusChangedEvent` / `DirectiveCompletedEvent` クラス定義は M4 でも作成するが実 publish は行わない）
...
| 4 | Agent のステータス変化後、接続中クライアントに `{"event_type": "agent.status_changed", ...}` JSON が届く | UC-WSB-003（**M5 Phase 2** — `AgentService.update_status()` 実装後に検証）| TC-ST-WSB-004（M5 Phase 2）|
...
|---|---|---|---|
| **A**: domain（Issue #158） | UC-WSB-001〜002（M4 必須）/ UC-WSB-003〜004（クラス定義のみ M4・Service 統合は M5 Phase 2）| DomainEvent / EventBusPort / InMemoryEventBus / TaskService・ExternalReviewGateService publish 統合 | `http-api-foundation` に依存 |
| **B**: http-api（Issue #159） | UC-WSB-005（M4 必須）/ UC-WSB-001〜002（WebSocket 配信側）| ConnectionManager / `GET /ws` / EventBus→WebSocket bridge / lifespan 統合 | Issue #158 完了後に着手 |
