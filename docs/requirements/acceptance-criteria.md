...
各受入基準は [`../acceptance-tests/scenarios/`](../acceptance-tests/) の SC-MVP-NNN シナリオで観察される。

| 受入基準 # | カバーするシナリオ ID | M7 スコープ |
|---|---|---|
| 1, 2, 3, 4, 5 (UI承認), 7, 9, 17 | SC-MVP-001（V モデル開発室で Task 完走）| ✅ M7 |
| 6 (Internal差し戻し), 18 | SC-MVP-002（InternalReview差し戻し → 複数ラウンド完走）| ✅ M7 |
| 5 (差し戻し UI操作), 6 (External差し戻し) | SC-MVP-009（ExternalReview Gate UI 差し戻し） — Issue #172 で追跡 | M7 スコープ外 |
| 8 | SC-MVP-003（再起動跨ぎでの全状態復元） | M7 スコープ外 |
| 10 | SC-MVP-008（`just check-all` 緑） | M7 スコープ外 |
| 11 | SC-MVP-004（BLOCKED Task の admin 救済） | M7 スコープ外 |
| 12 | SC-MVP-005（dead-letter event の admin 救済） | M7 スコープ外 |
| 13 | SC-MVP-007（Admin CLI の audit_log 記録） | M7 スコープ外 |
| 14, 15, 16 | SC-MVP-006（secret マスキング / TLS / 添付安全） — Issue #172 で追跡 | M7 スコープ外 |

**M7 検証スコープ**: SC-MVP-001（受入基準 #1〜#5(UI承認)/#7/#9/#17）と SC-MVP-002（受入基準 #6/#18）の 2 シナリオが M7 の完了基準。残シナリオ（SC-MVP-003〜009）は Issue #172 で追跡し、v0.1.0 リリース前に完了させること。
