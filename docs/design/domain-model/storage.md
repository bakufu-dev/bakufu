...
|---|---|---|---|---|
| `Conversation.messages[].body_markdown` | `conversation_messages` | `infrastructure/persistence/sqlite/tables/conversation_messages.py` | `MaskedText` | `feature/conversation-repository`（後続 — **§BUG-TR-002 凍結**: Task Aggregate PR #42 に `conversations` 属性なし。task-repository PR #52 スコープから除外。Task domain が `conversations` 属性を獲得した将来 PR で配線完了予定） |
...
| **Directive 残カラム（`directives` の `text` 以外）** | 同左 | `tables/directives.py` | **masking 対象なし**（`id` / `target_room_id` / `created_at` / `task_id` は `UUIDStr` / `UTCDateTime` のみ。`directives.text` 以外のカラムが secret 6 種に該当しないことを CI Layer 2 で arch test で保証、過剰マスキング防止） | `feature/directive-repository`（Issue #34） |
| **Task 残カラム（`tasks` の `last_error` 以外 / `task_assigned_agents` 全カラム / `deliverables` の `body_markdown` 以外 / `deliverable_attachments` 全カラム）** | 同左 4 テーブル | `tables/tasks.py` / `tables/task_assigned_agents.py` / `tables/deliverables.py` / `tables/deliverable_attachments.py` | **masking 対象なし**（`UUIDStr` / `String` / `Integer` / `DateTime` のみ。2 masking カラム以外が secret 6 種に該当しないことを CI Layer 2 で arch test で保証、過剰マスキング防止）。`conversations` / `conversation_messages` テーブルは §BUG-TR-002 凍結済みのため除外 | `feature/task-repository`（Issue #35） |
| `ExternalReviewGate.deliverable_snapshot.body_markdown`（Gate snapshot 本文、Agent 出力 / secret 混入経路） | `external_review_gates` | `infrastructure/persistence/sqlite/tables/external_review_gates.py`（カラム: `snapshot_body_markdown`） | **`MaskedText`** | `feature/external-review-gate-repository`（Issue #36、**external-review-gate §確定 G 実適用**、persistence-foundation #23 で hook 構造提供 → **本 PR で配線完了**） |
| `AuditEntry.comment`（Gate audit コメント、CEO 入力経路 → secret 混入リスク） | `external_review_audit_entries` | `infrastructure/persistence/sqlite/tables/external_review_audit_entries.py`（カラム: `comment`） | **`MaskedText`** | `feature/external-review-gate-repository`（Issue #36、**CEO 入力経路 masking 実適用**、persistence-foundation #23 で hook 構造提供 → **本 PR で配線完了**） |
| **ExternalReviewGate 残カラム（`external_review_gates` の `snapshot_body_markdown` 以外 / `external_review_gate_attachments` 全カラム / `external_review_audit_entries` の `comment` 以外）** | 同左 3 テーブル | `tables/external_review_gates.py` / `tables/external_review_gate_attachments.py` / `tables/external_review_audit_entries.py` | **masking 対象なし**（`UUIDStr` / `String` / `Text` / `Integer` / `DateTime` のみ。2 masking カラム以外が secret 6 種に該当しないことを CI Layer 2 で arch test で保証、過剰マスキング防止） | `feature/external-review-gate-repository`（Issue #36） |
| **ExternalReviewGate criteria（`external_review_gate_criteria` 全カラム）** | `external_review_gate_criteria` | `tables/external_review_gate_criteria.py` | **masking 対象なし**（`UUIDStr` / `String` / `Boolean` / `Integer` のみ。`AcceptanceCriterion.description` は `deliverable-template/feature-spec.md §13` で機密レベル「低」と業務判定済み + PR #137 `acceptance_criteria_json` 凍結と同一業務判断。CI Layer 1+2 で `MaskedText` / `MaskedJSONEncoded` 不在を物理保証） | `feature/121-gate-deliverable-criteria`（Issue #121） |
| **DeliverableTemplate 全カラム（`deliverable_templates` 全カラム）** | `deliverable_templates` | `tables/deliverable_templates.py` | **masking 対象なし**（`UUIDStr` / `String(80)` / `Text` / `JSONEncoded` のみ。テンプレートメタデータ・スキーマ・受入基準は Schneier #6 secret カテゴリ非該当。`schema` は `Text`（type カラム判別により条件付き json.dumps/loads）、`acceptance_criteria_json` / `composition_json` は `JSONEncoded`。CI Layer 1+2 で `MaskedJSONEncoded` / `MaskedText` 不在を物理保証） | `feature/deliverable-template-repository`（Issue #119） |
| **RoleProfile 全カラム（`role_profiles` 全カラム）** | `role_profiles` | `tables/role_profiles.py` | **masking 対象なし**（`UUIDStr` / `String(32)` / `JSONEncoded` のみ。ロールプロファイルとテンプレート参照リストは Schneier #6 secret カテゴリ非該当。`deliverable_template_refs_json` は `JSONEncoded`。CI Layer 1+2 で `MaskedJSONEncoded` / `MaskedText` 不在を物理保証） | `feature/deliverable-template-repository`（Issue #119） |
| `DeliverableRecord.content`（成果物本文コピー、Agent 出力に secret 混入の可能性）| `deliverable_records` | `infrastructure/persistence/sqlite/tables/deliverable_records.py` | **`MaskedText`**（同じデータ源である `deliverables.body_markdown` が `MaskedText` 適用済み。コピー先の `content` も同一リスクが存在するため同等の保護が必要。`feature-spec.md §13` の「機密レベル: 低」判断はテンプレート静的定義データ向けであり、runtime Agent 出力には適用不可。SEC-2 解消）| `feature/123-ai-validation`（Issue #123、**本 PR で配線完了**）|
| **DeliverableRecord 残カラム（`deliverable_records` の `content` 以外）** | `deliverable_records` | `infrastructure/persistence/sqlite/tables/deliverable_records.py` | **masking 対象なし**（`UUIDStr` / `String(20)` / `DateTime` のみ。`id` / `deliverable_id` / `template_ref_*` / `task_id` / `validation_status` / `produced_by` / `created_at` / `validated_at` は Schneier #6 secret カテゴリ非該当。CI Layer 1+2 で `MaskedText` / `MaskedJSONEncoded` 不在を物理保証）| `feature/123-ai-validation`（Issue #123）|
| **CriterionValidationResult 全カラム（`criterion_validation_results` 全カラム）** | `criterion_validation_results` | `infrastructure/persistence/sqlite/tables/criterion_validation_results.py` | **masking 対象なし**（`id` / `deliverable_record_id` / `criterion_id` は `UUIDStr`、`status` は `String(20)`、`reason` は `Text`（LLM の評価理由。Agent 生成テキストだが criterion 評価コメントに過ぎず、APIキー・トークン等の secret は通常混入しない）、`created_at` は `DateTime`。万一 secret が混入した場合は親テーブルの `content` MaskedText で先行マスキング済みのデータに基づく評価理由であるため二次漏洩リスクは限定的。CI Layer 1+2 で `MaskedText` / `MaskedJSONEncoded` 不在を物理保証）| `feature/123-ai-validation`（Issue #123）|
