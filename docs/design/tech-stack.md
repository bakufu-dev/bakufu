...
| backend コンテナ | **`backend/Dockerfile`**（python:3.12-slim ベース） | 開発モード（`override`）ではソースを bind mount してホットリロード、本番ビルドでは `uv sync --no-dev` で最小レイヤーを維持。2 ステージビルドで builder / runtime を分離し、uv バイナリを builder からコピーすることで runtime に pip を持ち込まない |
| frontend コンテナ | **`frontend/Dockerfile`**（node:20-slim + pnpm ベース） | 開発モードでは Vite 開発サーバ（`pnpm dev --host 0.0.0.0`）を起動し HMR を有効化。`server.host: 0.0.0.0` でコンテナ外のブラウザからアクセス可能。本番ビルドでは `pnpm build` 後の静的ファイルを nginx で配信 |
| データ永続化 | **named volume `bakufu-data`** | SQLite ファイル（`bakufu.db`）および content-addressable 添付ファイル（`attachments/`）を Docker named volume に配置。コンテナ再作成後もデータが保持される。volume はホスト側ディレクトリ bind mount も可（`.env` の `BAKUFU_DATA_DIR` で制御） |
| 環境変数管理 | **`backend/.env.example` → `backend/.env`**（gitignore 済み） | `.env.example` が必須環境変数を列挙する雛形（シークレット値はプレースホルダー）。開発者は `cp backend/.env.example backend/.env` して実値を設定。`docker compose` は `env_file: backend/.env` で読み込む。**`.env` を git に追加することを `.gitignore` + gitleaks で二重に防止** |
| ポート割り当て | backend `8000`、frontend `5173` | backend は `BAKUFU_BIND_HOST=0.0.0.0` をコンテナ内専用設定にし、ホスト側は `127.0.0.1:8000` にバインド（ホスト側から `localhost:8000` でアクセス、外部 IP 非公開）。frontend Vite デフォルト `5173` をホスト `127.0.0.1:5173` にバインド |
| 開発 / 本番分離 | **`docker-compose.yml`（基底）+ `docker-compose.override.yml`（開発専用）** | `docker-compose.yml` が本番相当の最小定義。`override` で開発専用設定（ソース bind mount・Vite HMR・uvicorn `--reload`）を上書き。`docker compose up` は自動的に両ファイルを合成する（公式挙動）。CI では `docker compose -f docker-compose.yml up` で override を排除 |
| ヘルスチェック | backend `GET /health` 200、frontend Vite 起動確認 | `docker-compose.yml` の `healthcheck` に設定。`docker compose up --wait` で backend が HEALTHY になるまで待機。CI でのスモークテストに活用 |
| just 統合 | `just up` / `just down` / `just logs` | `justfile` に docker compose コマンドを alias として定義。`uv run python -m bakufu.main`（ネイティブ起動）と `just up`（コンテナ起動）を同一 `justfile` で使い分け可能にする |

#### ローカル起動方法（開発者ガイド）

| 手順 | コマンド | 備考 |
|----|----|----|
| 1. 環境変数準備 | `cp backend/.env.example backend/.env` | API キー等を `.env` に記入 |
| 2. コンテナ起動 | `just up`（または `docker compose up --build`） | 初回はイメージビルドを含む |
| 3. ヘルス確認 | `curl http://localhost:8000/health` | `{"status":"ok"}` が返ること |
| 4. UI アクセス | ブラウザで `http://localhost:5173` | Vite HMR 有効 |
| 5. 停止 | `just down`（または `docker compose down`） | `--volumes` オプションで永続データも削除可 |

ネイティブ起動（docker なし）も継続サポートする: `uv run python -m bakufu.main`（backend）、`pnpm dev`（frontend）。ローカルファーストの要件上、docker は **開発者体験の向上手段** であって、必須依存にはしない。
...
| Podman / Podman Compose | コンテナ起動 | Docker Engine との互換性を優先。Apple Silicon + Linux + Windows の全環境で Docker Desktop が現状最も事例が豊富 |
