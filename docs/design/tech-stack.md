...
| 状態管理 | **Zustand v4**（クライアント UI 状態）+ **TanStack Query v5**（サーバ状態）| サーバ状態（Task / Gate / Room）は TanStack Query が一元管理。WebSocket 接続状態等のクライアント UI 状態は Zustand の軽量ストア。Redux はオーバースペック（MVP スケール）で不採用 |
| WebSocket | ネイティブ `WebSocket` API + `useWebSocketBus` hook | リアルタイムイベント受信。切断時は exponential backoff で自動再接続（`[1000, 2000, 4000, 8000, 16000, 30000]` ms）。追加ライブラリ不採用（socket.io は不要な overhead）|
| Markdown レンダリング | **react-markdown v9** + **rehype-sanitize v6** | AI 生成の `deliverable_snapshot.body_markdown` を Markdown → HTML にレンダリング。`rehype-sanitize` で XSS サニタイズ必須（LLM 出力に `<script>` 等が混入し得るため。`threat-model.md §A3` 対応）|
...
#### §確定 SE-A: StageWorker 非同期実行設計（M5-A stage-executor Issue #163 確定）

StageWorker は asyncio.Queue + asyncio.Semaphore の組み合わせで並行数制御を実現する。

| 設計要素 | 採用 | 不採用候補 | 根拠 |
|---|---|---|---|
| キューイング | **asyncio.Queue** | concurrent.futures.ThreadPoolExecutor / asyncio.TaskGroup | 単一 asyncio イベントループ内でノンブロッキングなバッファリングが可能。FastAPI / uvicorn と追加スレッドなしで統合できる（追加依存ゼロ）|
| 並行数制御 | **asyncio.Semaphore（`BAKUFU_MAX_CONCURRENT_STAGES`）** | ThreadPoolExecutor max_workers / asyncio.gather limit | Queue consumer が acquire → dispatch → release を繰り返すことで、上限超過時のキューイングが自然に成立する。実装が単純で障害点が少ない |
| MVP 並行数 | **1（シリアル実行）**、環境変数 `BAKUFU_MAX_CONCURRENT_STAGES` で変更可能（未設定 / 不正値時は 1 にフォールバック）| 複数（並列実行）| MVP の検証対象は「Vモデル工程と外部レビューゲートの正しさ」。並列実行には git worktree 分離が前提となり Phase 2 以降で設計する |

出典: [asyncio Queue](https://docs.python.org/3/library/asyncio-queue.html) / [asyncio Synchronization Primitives](https://docs.python.org/3/library/asyncio-sync.html)

#### §確定 SE-B: Bootstrap Stage 6.5 StageWorker 登録（M5-A stage-executor Issue #163 確定）

`backend/src/bakufu/infrastructure/bootstrap.py` に `_stage_6_5_stage_worker()` メソッドを追加し、以下の起動順序を保証する:

| Stage | 内容 | 順序根拠 |
|---|---|---|
| Stage 6 | Outbox Dispatcher 起動（既存）| Stage 6.5 より前: Stage 実行が発火する Domain Event を配送する Outbox が先に起動している必要がある |
| **Stage 6.5** | **StageWorker 起動（本確定）**| Stage 6 完了後 / Stage 8 より前 |
| Stage 8 | FastAPI listener 起動（既存）| HTTP API で Directive を受け付ける前に StageWorker が待機状態にある |

StageWorker は `asyncio.create_task` で起動する。FastAPI event loop と同一ループで動作し、追加スレッドは不要。Bootstrap が shutdown シグナルを受信した際は `StageWorker.stop()` を呼び出し、実行中の Stage subprocess を SIGTERM して graceful drain する。

#### §確定 WS-A: WebSocket / EventBus 技術選定（M4 websocket-broadcast Issue #157 確定）

| カテゴリ | 採用 | 不採用候補 | 根拠 |
|---|---|---|---|
| WebSocket ライブラリ | **FastAPI ネイティブ WebSocket**（`from fastapi import WebSocket`）+ **`websockets`**（`uvicorn[standard]` に同梱）| `starlette.websockets`（低レベル直接使用）/ `socket.io` | FastAPI の WebSocket サポートは Starlette を薄くラップし、型安全な DI / router 統合が可能。`websockets` は `uvicorn[standard]` に含まれるため **追加依存ゼロ**（`pyproject.toml` 変更なし） |
| EventBus 実装（MVP） | **`InMemoryEventBus`**（インプロセス asyncio）| Redis Pub/Sub / RabbitMQ / Kafka | ローカルファースト MVP に外部 MQ は過剰（YAGNI）。`asyncio.gather()` でプロセス内ブロードキャストを実現。イベント損失は許容（再接続時 REST API でリフレッシュ）|
| EventBus 接続管理 | **`ConnectionManager`**（interfaces/http 層の薄いラッパ）| 省略（ws.py に直書き）| WebSocket 接続の add / remove / broadcast を 1 クラスに集約。`WebSocketDisconnect` を ConnectionManager が隠蔽し、router / bridge が詳細を知らなくて済む（Tell, Don't Ask）|
| Server-Sent Events（SSE）| **不採用** | SSE（`text/event-stream`）| WebSocket は双方向通信の拡張余地を持つ（将来のエージェント↔UI インタラクション）。SSE は単方向のみ。MVP での実装コストは同等のため WebSocket を採用 |
| Domain Event 定義方式 | **Pydantic BaseModel 継承**（`DomainEvent` 基底）| `dataclasses.dataclass` | 既存コードベース全体が Pydantic v2 を使用。`model_dump()` で WebSocket メッセージへの変換が容易。バリデーションが自動 |

Domain Event → WebSocket 配信の全体フロー（[`docs/features/websocket-broadcast/`](../features/websocket-broadcast/) 設計詳細参照）:
...
sequenceDiagram
    ...
    participant UI as Browser WebSocket

    Svc->>EB: publish(DomainEvent)
    EB->>CM: handler(event)（asyncio.gather）
    CM->>UI: send_text(event.to_ws_message())
```
...
| backend コンテナ | **`backend/Dockerfile`**（python:3.12-slim / 2 ステージビルド） | builder ステージで `uv sync --frozen` を実行し venv を構築、runtime ステージに venv のみコピーすることで pip を runtime に持ち込まない。開発 override では `src/` を bind mount し、`BAKUFU_DEV_RELOAD=true` 環境変数を渡すことで `bakufu.main` の `_uvicorn_starter` が `asyncio.create_subprocess_exec(sys.executable, "-m", "uvicorn", ..., "--reload", "--reload-dir", "/app/backend/src")` により uvicorn を**子プロセスとして**起動しホットリロードを実現する（**BUG-006**: `uvicorn.run()` を `run_in_executor` 経由で呼ぶと uvicorn 内部の `signal.signal()` がワーカースレッドから呼ばれ `ValueError: signal only works in main thread` が発生するため子プロセス方式に切り替え。子プロセスは独立したメインスレッドを持つため `signal.signal()` 制約を回避できる）。**非 root ユーザー `bakufu`（uid=1000）で実行必須**（Dockerfile 末尾の `USER bakufu` + `/app/data` への `chown` で volume 上ファイル所有者を uid=1000 に固定し、root 所有ファイルをホスト側に作らない） |
...
| 開発専用オーバーライド | **`docker-compose.override.yml`** | backend `src/` bind mount（`./backend/src:/app/backend/src`）・`BAKUFU_DEV_RELOAD=true` 環境変数・Vite HMR・frontend `src/` bind mount・デバッグポートを定義。`bakufu.main` の `_uvicorn_starter` が `BAKUFU_DEV_RELOAD=true` を検知し `asyncio.create_subprocess_exec` で `python -m uvicorn bakufu.interfaces.http.app:app --reload --reload-dir /app/backend/src --host <host> --port <port>` を子プロセス起動、親プロセスは `await proc.wait()` で待機する。`docker compose up` は自動的に両ファイルを合成する（Docker Compose 公式挙動）。CI では `-f docker-compose.yml` のみ指定し override を排除する |
...
| backend 起動コマンド | **`uv run python -m bakufu.main`** | `bakufu.main`（`backend/src/bakufu/main.py`）は Bootstrap 経由で uvicorn を起動する wrapper。マイグレーション実行等の 8 段階コールドスタートを経由させる必要があるため、**親プロセスの entrypoint として** `uvicorn bakufu.interfaces.http.app:app` を直接呼ぶことは禁止し、必ず `bakufu.main` 経由を強制する。**dev_reload 時に `_uvicorn_starter` が子プロセスとして uvicorn を起動する経路は正規経路として許可**。起動シーケンス: **親プロセス Bootstrap stage 1–8 → stage 8 で `_uvicorn_starter` を実行 → 通常時: `uvicorn.Server(config).serve()` をプロセス内起動 / `BAKUFU_DEV_RELOAD=true` 時: `asyncio.create_subprocess_exec` で子プロセス（uvicorn + --reload）を起動 → 親プロセスは `await proc.wait()` で待機** |
