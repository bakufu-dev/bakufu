...
| Anthropic API | **anthropic** SDK（`>=0.40.0`）| 公式 Python SDK、`AsyncAnthropic` で非同期 HTTP 直接呼び出し。`AbstractLLMClient` の Anthropic 実装に使用（Issue #144）|
| OpenAI API | **openai** SDK（`>=1.30.0`）| 公式 Python SDK、`AsyncOpenAI` で非同期 HTTP 直接呼び出し。`AbstractLLMClient` の OpenAI 実装に使用（Issue #144）|
...
|---|---|---|
| **用途** | AI 評価・分類・自然言語処理（短時間テキスト処理）| coding agent 実行（コード生成 / 実行、長時間タスク）|
