...
test.describe("TC-E2E-CD-009: WebSocket リアルタイム更新 (P0)", () => {
  test(
    "WS task.state_changed イベント受信 → StatusBadge aria-label がページリロードなしで更新される",
    async ({ page }) => {
      // -------------------------------------------------------------------
      ...
      // -------------------------------------------------------------------
      await expect(
        page.locator('output[aria-label="サーバーと接続済み"]'),
...
test.describe("TC-E2E-CD-010: WebSocket 切断/再接続", () => {
  test(
    "WS 切断 → ConnectionIndicator「再接続中...」→ 再接続 → 「接続済み」の状態遷移を確認",
    async ({ page }) => {
      // -------------------------------------------------------------------
      ...
      // -------------------------------------------------------------------
      await expect(
        page.locator('output[aria-label*="再接続中"]'),
