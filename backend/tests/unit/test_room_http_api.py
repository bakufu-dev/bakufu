"""room / http-api ユニットテスト (TC-UT-RM-HTTP-001~006, 010).

Per ``docs/features/room/http-api/test-design.md`` §ユニットテストケース.
...
Covers:
  ...
  TC-UT-RM-HTTP-005  room_invariant_violation_handler [FAIL]/Next: 除去 + kind 分岐 (MSG-RM-HTTP-005/007)
...
# ---------------------------------------------------------------------------
# TC-UT-RM-HTTP-001: RoomCreate スキーマ検証
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TC-UT-RM-HTTP-002: RoomUpdate スキーマ検証
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TC-UT-RM-HTTP-003: AgentAssignRequest スキーマ検証
# ---------------------------------------------------------------------------
...
class TestAgentAssignRequestSchema:
    ...
    def test_all_valid_roles_pass(self) -> None:
        ...
        from bakufu.interfaces.http.schemas.room import AgentAssignRequest, _VALID_ROLES  # type: ignore[attr-defined]
...
class TestRoomResponseSchema:
    ...
    def test_room_response_prompt_kit_prefix_markdown_flattened(self) -> None:
        """prompt_kit.prefix_markdown が prompt_kit_prefix_markdown にフラット化される (_flatten_room)."""
        from bakufu.interfaces.http.schemas.room import RoomResponse
    ...
    def test_room_list_response_total_matches_items_len(self) -> None:
        ...
        rooms = [RoomResponse.model_validate(self._make_mock_room(name=f"部屋{i}")) for i in range(3)]
...
# ---------------------------------------------------------------------------
# TC-UT-RM-HTTP-006: RoomService.__init__ 4 repo 構造
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TC-UT-RM-HTTP-010: interfaces/http/ 依存方向静的解析 — ast.walk() 全ノード走査
# ---------------------------------------------------------------------------
