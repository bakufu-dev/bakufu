...
def make_tool_call_chat_result(
    ...
    """submit_verdict ToolCall を含む ChatResult を生成する（§確定 D 新形式）。

    ChatResult.tool_calls に ToolCall を格納する新形式（M5-B 設計書 §確定D更新後）。
    """
    from bakufu.domain.value_objects.chat_result import ToolCall
    ...
    return ChatResult(
        ...
        compacted=False,
        tool_calls=(
            ToolCall(name="submit_verdict", input={"decision": decision, "reason": reason}),
        ),
    )
