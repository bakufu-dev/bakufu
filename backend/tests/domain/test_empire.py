...
``docs/features/empire/test-design.md``. Each test docstring carries the trace
anchor (TC-ID, REQ-ID, MSG-ID where applicable).

The integration scenarios live in this file rather than under ``integration/``
...
# ===========================================================================
# REQ-EM-001 — Construction
# ===========================================================================


def test_empire_constructs_with_valid_id_and_name() -> None:
...
def test_empire_construction_preserves_input_name() -> None:
    """TC-UT-EM-001: Empire.name reflects input after NFC+strip pipeline."""
    empire = make_empire(name="山田の幕府")
    assert empire.name == "山田の幕府"


# --- TC-UT-EM-002 — name boundary values --------------------------------
@pytest.mark.parametrize("valid_length", [1, 80])
def test_empire_accepts_name_at_lower_and_upper_boundaries(valid_length: int) -> None:
...
def test_empire_rejects_name_outside_boundaries(invalid_name: str) -> None:
...
# --- TC-UT-EM-003 — NFC + strip -----------------------------------------
def test_empire_normalizes_decomposed_name_to_nfc() -> None:
    ...
    # voicing mark). Pure katakana like テスト has no decomposition, so cannot
    # demonstrate the NFC pipeline.
    composed = "がが"  # NFC composed form (U+304C twice)
    decomposed = unicodedata.normalize("NFD", composed)
    assert decomposed != composed  # sanity: the input is actually decomposed
    empire = make_empire(name=decomposed)
    assert empire.name == composed


def test_empire_strips_surrounding_whitespace_from_name() -> None:
...
def test_hire_agent_appends_to_agents_list() -> None:
...
def test_hire_agent_does_not_mutate_original_aggregate() -> None:
    """TC-UT-EM-004: original Empire's agents stay empty after hire_agent."""
    empire = make_empire()
    agent = make_agent_ref()
    empire.hire_agent(agent)
    assert empire.agents == []


def test_hire_agent_supports_three_consecutive_distinct_hires() -> None:
    ...
    assert {ref.agent_id for ref in final.agents} == {a1.agent_id, a2.agent_id, a3.agent_id}


def test_hire_agent_rejects_duplicate_agent_id() -> None:
    """TC-UT-EM-006: rehiring the same agent_id raises agent_duplicate."""
    empire = make_empire()
    agent = make_agent_ref()
    after_first_hire = empire.hire_agent(agent)
    with pytest.raises(EmpireInvariantViolation) as excinfo:
        # Re-hire with the *same* agent_id but otherwise different fields to
        # prove the duplicate check keys solely on agent_id.
...
def test_hire_agent_capacity_boundary_succeeds_at_limit() -> None:
...
def test_hire_agent_capacity_overflow_raises_capacity_exceeded() -> None:
...
def test_establish_room_appends_to_rooms_list() -> None:
...
def test_establish_room_does_not_mutate_original_aggregate() -> None:
    """TC-UT-EM-008: original Empire's rooms stay empty after establish_room."""
    empire = make_empire()
    room = make_room_ref()
    empire.establish_room(room)
    assert empire.rooms == []


def test_establish_room_rejects_duplicate_room_id() -> None:
    """TC-UT-EM-009: re-establishing the same room_id raises room_duplicate."""
    empire = make_empire()
    room = make_room_ref()
    after_first_establish = empire.establish_room(room)
    with pytest.raises(EmpireInvariantViolation) as excinfo:
        duplicate = RoomRef(room_id=room.room_id, name="別名")
        after_first_establish.establish_room(duplicate)
    assert excinfo.value.kind == "room_duplicate"


def test_establish_room_capacity_boundary_succeeds_at_limit() -> None:
...
def test_establish_room_capacity_overflow_raises_capacity_exceeded() -> None:
...
def test_archive_room_marks_target_as_archived() -> None:
...
def test_archive_room_leaves_other_rooms_unchanged() -> None:
...
def test_archive_room_raises_for_unknown_room_id() -> None:
...
def test_archive_room_does_not_physically_delete() -> None:
...
# ===========================================================================
# Pre-validate (REQ-EM-005, Confirmation A) — original Empire stays unchanged
# on failure across every behavior.
# ===========================================================================


def test_hire_agent_failure_leaves_original_empire_unchanged() -> None:
...
def test_establish_room_failure_leaves_original_empire_unchanged() -> None:
...
def test_archive_room_failure_leaves_original_empire_unchanged() -> None:
...
# ===========================================================================
# Frozen / extra='forbid' (REQ-EM-005)
# ===========================================================================


def test_empire_rejects_attribute_assignment_after_construction() -> None:
    """TC-UT-EM-017: Empire is frozen — direct attribute set raises."""
    empire = make_empire()
    with pytest.raises(ValidationError):
        empire.name = "改竄"  # type: ignore[misc]  # frozen=True forbids assignment


def test_room_ref_rejects_attribute_assignment_after_construction() -> None:
...
def test_agent_ref_rejects_attribute_assignment_after_construction() -> None:
...
def test_empire_model_validate_rejects_unknown_field() -> None:
...
def test_msg_em_001_for_oversized_name_matches_exact_wording() -> None:
...
def test_msg_em_001_for_whitespace_only_reports_post_strip_length() -> None:
...
def test_msg_em_002_for_duplicate_agent_includes_agent_id() -> None:
...
def test_msg_em_003_for_duplicate_room_includes_room_id() -> None:
...
def test_msg_em_004_for_archive_unknown_room_includes_room_id() -> None:
...
def test_msg_em_005_for_capacity_overflow_uses_invariant_violation_prefix() -> None:
...
# ===========================================================================
# Integration scenarios (TC-IT-EM-001 / 002) — Aggregate-internal round trip.
# ===========================================================================


def test_full_lifecycle_empire_hire_establish_archive_round_trip() -> None:
    ...
    # Final aggregate has 1 agent, 1 archived room.
...
def test_full_lifecycle_keeps_intermediate_aggregates_immutable() -> None:
    ...
    # Earlier aggregates are unchanged by later operations.
    assert empire.agents == [] and empire.rooms == [] and len(after_hire.rooms) == 0


def test_failed_hire_does_not_block_subsequent_establish_or_archive() -> None:
    ...
    # 2) establish_room now succeeds against the unchanged aggregate.
    ...
    final_room = after_archive.rooms[0]
    assert (
        len(empire.agents) == 1 and len(after_establish.rooms) == 1 and final_room.archived is True
    )
