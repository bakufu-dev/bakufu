...
class HttpErrorHandlers:
    ...
    async def http_exception_handler(request: Request, exc: Exception) -> JSONResponse:
        ...
        if status == 404:
            # MSG-HAF-001: exc.detail ではなく確定文言を使う。
    ...
    @staticmethod
    async def directive_invariant_violation_handler(
        request: Request, exc: Exception
    ) -> JSONResponse:
    ...
    async def external_review_gate_authorization_handler(
        ...
        if not isinstance(exc, ExternalReviewGateAuthorizationError):
            raise TypeError(
                f"Expected ExternalReviewGateAuthorizationError, got {type(exc).__name__}"
            )
        return _error_response(
            FORBIDDEN,
    ...
    async def external_review_gate_invariant_violation_handler(
        ...
        if not isinstance(exc, ExternalReviewGateInvariantViolation):
            raise TypeError(
                f"Expected ExternalReviewGateInvariantViolation, got {type(exc).__name__}"
            )
        raw = str(exc)
        cleaned = _FAIL_PREFIX_RE.sub("", raw).split("\nNext:")[0].strip()
        return _error_response(VALIDATION_ERROR, cleaned, 422)
