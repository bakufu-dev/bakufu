...
async def approve_gate(
    ...
    Application 層の ``ExternalReviewGateService.apply_post_approve_task_state_update``
    に集約。Workflow Transition を辿って Task を DONE か次 Stage に進める。
    """
    ...
    async with session.begin():
        ...
        await service.apply_post_approve_task_state_update(
            gate=gate,
            reviewer_owner_id=reviewer_owner_id,
            now=now,
        )
