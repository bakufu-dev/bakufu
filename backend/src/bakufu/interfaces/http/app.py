...
class BakufuHttpApplication:
    """FastAPI app 初期化境界を classmethod に閉じる。"""
    ...
    async def lifespan(cls, app: FastAPI) -> AsyncGenerator[None, None]:
        """startup で session_factory を保持、shutdown で dispose (確定 B)。"""
...
create_app = BakufuHttpApplication.create
app = BakufuHttpApplication.create()
