"""StageExecutorService — Stage 実行オーケストレータ（REQ-ME-001〜005）。

WORK / INTERNAL_REVIEW / EXTERNAL_REVIEW の 3 分岐を担い、LLMProviderError 5 分類
に対するリトライ戦略を実装する。UoW（``async with self._session.begin()``）は
...
#: retry_blocked_task の enqueue_fn 型エイリアス。
...
class StageExecutorService:
    ...
    **セキュリティ（セキュリティ設計 T1）**:
    ...
    async def dispatch_stage(self, task_id: TaskId, stage_id: StageId) -> None:
        ...
        読み取りフェーズ（短トランザクション）→ 処理（LLM 呼び出しは長時間のためトランザクション外）
        → 書き込みフェーズ（短トランザクション）の順序で進む。

        **Fail Fast（§確定 F）**: Task.status が IN_PROGRESS でない場合はログを出して即 return。
        TaskRepository 障害は上位に伝播させる（Bootstrap レベルの障害）。
        """
        # 読み取りフェーズ
        task, stage, workflow, agent = await self._load_context(task_id, stage_id)
        if task is None:
            logger.error(
                "[FAIL] dispatch_stage: task not found (task_id=%s)",
                task_id,
            )
            return
        if task.status != TaskStatus.IN_PROGRESS:
            logger.error(
                "[FAIL] dispatch_stage: task=%s is not IN_PROGRESS (status=%s); skipping",
                task_id,
                task.status,
            )
            return
        if stage is None:
            logger.error(
                "[FAIL] dispatch_stage: stage=%s not found in workflow for task=%s",
                stage_id,
                task_id,
            )
            return
        ...
        **Fail Fast（§確定 F）**:
        ...
        async with self._session.begin():
            ...
            if task.status != TaskStatus.BLOCKED:
                msg = (
                    f"[FAIL] Task {task_id} is not BLOCKED (current status: {task.status.value})\n"
    ...
    async def _execute_work_stage(
        ...
        エラー時は _handle_llm_error に委譲（REQ-ME-004）。
        """
        if agent is None:
            logger.error(
        ...
        try:
            ...
            try:
                ...
                await self._handle_llm_error(task, retry_exc)
                return
            ...
            result = await self._retry_rate_limited(
                task=task,
                messages=messages,
            ...
            await self._handle_llm_error(task, exc)
            return
    ...
    async def _request_external_review(self, task: Task) -> None:
        ...
        async with self._session.begin():
            ...
            if current_task is None or current_task.status != TaskStatus.IN_PROGRESS:
                logger.warning(
                    "[WARN] task=%s state changed before EXTERNAL_REVIEW transition",
                    task.id,
                )
                return
    ...
    # ------------------------------------------------------------------
    # プライベートメソッド — LLM エラー処理
    # ------------------------------------------------------------------

    async def _handle_llm_error(self, task: Task, exc: LLMProviderError) -> None:
        ...
        logger.error("%s", msg)
        await self._block_task(task, reason=error_kind, error_summary=masked_summary)
    ...
    async def _retry_rate_limited(
        ...
        成功した場合は ChatResult を返す。3 回失敗した場合は Task.block() を呼び出し
        None を返す。
        """
        ...
        *,
        ...
        async with self._session.begin():
            ...
            if current_task is None:
                logger.error(
                    "[FAIL] _block_task: task=%s disappeared before block()",
                    task.id,
                )
                return
            if current_task.status not in (
                ...
                logger.warning(
                    "[WARN] _block_task: task=%s status=%s; cannot block from this state",
                    task.id,
                    current_task.status,
                )
                return
