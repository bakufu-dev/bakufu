...
開発初期。MVP（V モデル開発室 1 部屋でタスク完走 + 外部レビューゲート）に向けた設計フェーズ。詳細は Vモデル工程ごとの設計書群（[`docs/analysis/`](docs/analysis/) / [`docs/requirements/`](docs/requirements/) / [`docs/design/`](docs/design/)）および各 feature の設計書（[`docs/features/`](docs/features/)）を参照。
